@echo off
rem YH fat_stops A/B — Mag9 w/o TSLA.
rem Pattern suggests tighter stop_pct OR time-stop (honest direction).
rem Note: prior yh_param_hint_ab tested EXPAND stop (0.923) and dismissed it;
rem this suite tests the opposite pattern lean (tighten / time-stop), ≤1–2 alts.
rem
rem Arms:
rem   01_control          — stop_pct=0.934 (production)
rem   02_stop_tighten_945 — stop_pct=0.945 (tighter multiplier)
rem   03_time_stop_45     — time_stop_days=45 (bars-held TIME exit)
rem
rem Output: drive\paul_experiments\yh_fat_stops_ab\<arm>\
rem
