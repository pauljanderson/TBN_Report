#!/usr/bin/env python3
"""
Generate ThinkorSwim studies for StockBee Momentum Burst (SB) Closed trades.

Per seed symbol:
  - SMA20 (red), SMA50 (white), SMA100 (yellow), SMA200 (purple/violet)
  - Orange SIGNAL bubbles on SIGNAL_DATE (day-1 expansion bar)
  - Green In/Out arrows+bubbles from Closed (open In from Open CSV)
  - Red stop-loss segments: STOP_PRICE (LOD) from open..close
  - Cyan dashed target segments: TARGET_PRICE from open..close
  - OUT bubble includes EXIT_TYPE (STOP_LOSS / GAP_DOWN / TARGET / TIME / NO_FT)

Output:
  drive/paul_studies/SB/closed_<stamp>/SB_<SYM>_closed_<stamp>.ts

Usage:
  python tos/gen_sb_closed_studies.py
  python tos/gen_sb_closed_studies.py --stamp 260801122358
  python tos/gen_sb_closed_studies.py -o drive/paul_studies/SB/closed_260801122358
"""
...
DRIVE = _ROOT / "drive"
STUDIES_ROOT = DRIVE / "paul_studies" / "SB"
_STAMP_RE = re.compile(r"SB_Closed_(\d{12})\.csv$", re.I)

# Locked seed universe (Theory / run_stockbee_burst.bat).
DEFAULT_SYMBOLS = [
    "NVDA",
    "TSLA",
    "AMD",
...
def discover_latest_sb_closed_stamp(drive: Path = DRIVE) -> str:
    stamps: list[str] = []
    for path in drive.glob("SB_Closed_*.csv"):
    ...
    if not stamps:
        raise FileNotFoundError(f"No SB_Closed_*.csv stamps under {drive}")
    return max(stamps)


def _parse_trade(raw: dict[str, str], *, is_open: bool) -> SbTrade | None:
    ...
    return SbTrade(
        ...
        signal_ymd=_ymd_int(raw.get("SIGNAL_DATE")),
        pct_day=_f(raw.get("PCT_DAY")),
        dcr=_f(raw.get("DCR")),
...
def load_closed(path: Path) -> list[SbTrade]:
    if not path.is_file():
...
def load_open(path: Path) -> list[SbTrade]:
    if not path.is_file():
        return []
    rows: list[SbTrade] = []
    with path.open(newline="", encoding="utf-8-sig") as fh:
...
def markers_and_levels(
    ...
    closed: list[SbTrade],
    opens: list[SbTrade],
) -> tuple[list[Marker], list[LevelSeg], list[LevelSeg]]:
    ...
    def add_marker(
        ...
        *,
        ...
        if not ymd:
            return
        # Allow signal + entry on consecutive days; dedupe same kind+ymd+bubble.
        key = (kind, ymd, bubble)
    ...
    for t in trades:
        ...
        audit = (
            f"pct={t.pct_day:.3f} dcr={t.dcr:.3f} risk={t.risk_pct:.4f}"
            if t.pct_day or t.dcr or t.risk_pct
            else label
        )
        add_marker(
            t.signal_ymd,
        ...
        if t.open_ymd and t.stop > 0:
            stops.append(
                LevelSeg(
                    ...
                    price=t.stop,
                    note=f"{label} LOD stop {t.open_ymd}->{close_bound}",
                )
            )
...
def build_study_lines(
    ...
    *,
    ...
    signals = [m for m in markers if m.kind == "signal"]
    entries = [m for m in markers if m.kind == "entry"]
    exits = [m for m in markers if m.kind == "exit"]

    lines: list[str] = [
        f"# SB closed {symbol} — stamp {stamp} (StockBee Momentum Burst)",
        "# SMA20=red, SMA50=white, SMA100=yellow, SMA200=purple (Color.VIOLET)",
        "# Orange SIG = SIGNAL_DATE (day-1 4%+/range-expansion bar)",
        "# Green IN = next-open fill; Green OUT = exit (bubble shows EXIT_TYPE)",
        "# Red stop = LOD STOP_PRICE; Cyan dashed = TARGET_PRICE (+10%)",
        f"# Counts: sig={len(signals)} in={len(entries)} out={len(exits)} "
        f"stop_segs={len(stops)} tgt_segs={len(targets)}",
        "",
        "declare upper;",
        "",
        "input showSMA = yes;",
        ...
        "",
        "# ---- SMAs (shared visual DNA with RL/YH closed studies) ----",
    ...
    # ---- SIGNAL markers (orange) ----
    if not signals:
        lines.append("# (no SIGNAL_DATE markers)")
        lines.append("")
    else:
        lines.append("# ===================== SIGNALS (SIGNAL_DATE) =====================")
        for i, m in enumerate(signals, 1):
            note = f"  # {m.note}" if m.note else ""
            lines.append(f"def sg{i}Hit = {event_hit_expr(m.ymd)};{note}")
            bubble = (m.bubble or "SIG >").replace('"', "'")
            lines.append(
                f'AddChartBubble(showSignals and sg{i}Hit, high, "{bubble}", '
                f"Color.ORANGE, yes);"
            )
        lines.append("")
    ...
    emit_levels(
        stops,
        prefix="sl",
        label="stop loss (LOD STOP_PRICE)",
...
def write_readme(
    ...
    *,
    ...
    lines = [
        "# SB Closed ThinkorSwim studies (StockBee Momentum Burst)",
        "",
        f"Single-stamp SB trade studies for the locked 18-seed universe, "
        ...
        "Output folder:",
        "",
        f"`drive/paul_studies/SB/closed_{stamp}/`",
        "",
        "## What's drawn",
        "",
        "| Element | Meaning |",
        ...
        "| Orange SIG bubbles | `SIGNAL_DATE` — day-1 expansion bar |",
        "| Green In/Out arrows | SB Closed (+ Open → In only) |",
        "| OUT bubble text | Includes `EXIT_TYPE` |",
        "| Red stop segments | LOD `STOP_PRICE` open→close |",
        "| Cyan dashed targets | `TARGET_PRICE` (+10%) open→close |",
        "",
        "## Files",
        "",
        "| Symbol | SIG | IN | OUT | SL | TGT | File |",
        "|--------|----:|---:|----:|---:|----:|------|",
    ]
    for sym in symbols:
        c = per_sym.get(sym)
        if not c:
            continue
        lines.append(
            f"| {sym} | {c.get('sig', 0)} | {c.get('in', 0)} | {c.get('out', 0)} | "
            f"{c.get('stops', 0)} | {c.get('targets', 0)} | "
            f"`SB_{sym}_closed_{stamp}.ts` |"
        )
    if failed:
        lines.extend(["", "## Failed", ""])
    ...
    lines.extend(
        [
            ...
            "",
            "1. Open a **daily** chart for the symbol (e.g. SNOW).",
            "2. Studies → Edit Studies → **Create…**",
            "3. Open the matching `.ts` file in a text editor, copy all, paste into the study editor.",
            f"4. Name it e.g. `SB SNOW closed {stamp}` and apply.",
            "5. Toggle inputs: `showSMA`, `showSignals`, `showEntries`, "
            "`showExits`, `showStopLoss`, `showTargets`.",
            "",
            "Regenerate:",
            "",
            "```",
            f"python tos/gen_sb_closed_studies.py --stamp {stamp}",
            "```",
            "",
            "## Sources",
            "",
    ...
    if open_path is not None:
        lines.append(f"- `{open_path.as_posix()}` (open In + stop/target)")
    lines.extend(
        [
            "- Seed universe: `run_stockbee_burst.bat` / Theory 18 seeds",
            "",
            "Sibling systems under `drive/paul_studies/` "
            "(`RL/`, `YH/`, `SB/`, …).",
            "",
            "Process notes: `drive/paul_experiments/tbn_new_systems/"
            "stockbee_momentum_burst/60_seed_tos.md`.",
            "",
        ]
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--stamp",
        default=None,
        help="SB Closed stamp YYYYMMDDHHMMSS (default: newest under drive/)",
    )
    ap.add_argument(
        "-o",
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory (default: drive/paul_studies/SB/closed_<stamp>)",
    )
    ap.add_argument(
        "--closed-csv",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Override Open CSV path (default: SB_Open_<stamp>.csv if present)",
    )
    ap.add_argument(
        "--symbols",
        default=None,
    ...
    stamp = args.stamp or discover_latest_sb_closed_stamp(DRIVE)
    closed_path = args.closed_csv or DRIVE / f"SB_Closed_{stamp}.csv"
    open_path = args.open_csv
    if open_path is None:
        cand = DRIVE / f"SB_Open_{stamp}.csv"
        if cand.is_file():
            open_path = cand
    ...
    for sym in symbols:
        try:
            markers, stops, targets = markers_and_levels(sym, closed, opens)
            ...
            fname = f"SB_{sym}_closed_{stamp}.ts"
            path = out_dir / fname
            path.write_text("\n".join(lines) + "\n", encoding="utf-8")
            written.append(sym)
            per_sym[sym] = {
                "sig": sum(1 for m in markers if m.kind == "signal"),
            ...
            print(
                f"  {sym}: sig={per_sym[sym]['sig']} in={per_sym[sym]['in']} "
