<#
.SYNOPSIS
    After rocket_brt and run_audit.ps1, copy selected latest BRT, IND, and Rocket Launcher CSVs to stable names in drive\.

.DESCRIPTION
    BRT run: latest yyMMddHHmmss from drive\BRT_{Closed|Open|Scanner|Watchlist}_<ts>.csv only
    ...
    MTS run: latest yyMMddHHmmss from MTS_Closed|Open|Scanner|Watchlist_<ts>.csv.

    Rocket Launcher / audit: timestamp from drive\last_run_ts.txt (same as portfolio_audit.awk).

    Copies only:
      BRT_Closed|Open|Scanner|Watchlist_<brtTs>.csv  -> BRT_LatestRun_*.csv
      YH_Closed|Open|Scanner|Watchlist_<yhTs>.csv  -> YH_LatestRun_*.csv
      IND_Closed|Open|Scanner|Watchlist|indicators_while_held|EquityCurve_Aggressive_<indTs>.csv -> IND_LatestRun_*.csv
      MTS_Closed|Open|Scanner|Watchlist_<mtsTs>.csv  -> MTS_LatestRun_*.csv
      RL_Closed|Open|Scanner|Watchlist_<rlTs>.csv    -> RL_LatestRun_*.csv
...
$BrtStems = @("Closed", "Open", "Scanner", "Watchlist")
$YhStems = @("Closed", "Open", "Scanner", "Watchlist")
$MtsStems = @("Closed", "Open", "Scanner", "Watchlist")
$IndStems = @("Closed", "Open", "Scanner", "Watchlist", "indicators_while_held", "EquityCurve_Aggressive")
$RlStems = @("Closed", "Open", "Scanner", "Watchlist")
