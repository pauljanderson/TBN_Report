...
REM Merge audit CSVs into drive\all.csv (or all_yh / all_vec / all_wpbr / all_rs for mode filters).
...
REM   concat.bat 26062211         merge BRT+IND+YH+VEC+WPBR+MTS+RS *_Audit_Report_26062211*.csv -> all.csv
...
if "!MODE!"=="yh" (
  ...
  if "!TS_FILTER!"=="" (
    ...
    set "PATS=BRT_Audit_Report_!TS_FILTER!*.csv;IND_Audit_Report_!TS_FILTER!*.csv;YH_Audit_Report_!TS_FILTER!*.csv;VEC_Audit_Report_!TS_FILTER!*.csv;WPBR_Audit_Report_!TS_FILTER!*.csv;PBR_Audit_Report_!TS_FILTER!*.csv;MTS_Audit_Report_!TS_FILTER!*.csv;RS_Audit_Report_!TS_FILTER!*.csv"
  )
)
