...
rem Adopt: drive\paul_experiments\rsi_atr5_dailyrun_20260914\
rem
...
rem   rsi_min_atr_pct=5, rsi_time_stop_days=20, rsi_entry_on=next_open
...
if not defined RSI_MIN_ATR_PCT set "RSI_MIN_ATR_PCT=5"
...
echo [RSI] freeze ob=%RSI_OB% os=%RSI_OS% exit=%RSI_EXIT% max_trigger=%RSI_MAX_TRIGGER% min_atr=%RSI_MIN_ATR_PCT% ts=%RSI_TIME_STOP_DAYS%d entry_on=%RSI_ENTRY_ON% notional=%RSI_SHEET_NOTIONAL% workers=%RSI_WORKERS%
...
if "%RSI_PASS_SYMBOLS%"=="1" (
  "%PY%" stock_analysis\rocket_tbn.py data\newdata\data -o drive -w %RSI_WORKERS% --no-regression %RSI_AGG_FLAG% --initial-capital 500000 --aggressive-max-multiple 2.0 --margin-utilization 0.6 -v rsi_mode=true -v vz_mode=false -v wrl_mode=false -v brt_zones=false -v yh_zones=false -v wpbr_zones=false -v rl_mode=false -v relative_strength_enabled=false -v rs_mode=false -v mvcp_mode=false -v sb_mode=false -v qull_mode=false -v indicator_buy=off -v rsi_ob=%RSI_OB% -v rsi_os=%RSI_OS% -v rsi_exit=%RSI_EXIT% -v rsi_max_trigger=%RSI_MAX_TRIGGER% -v rsi_min_atr_pct=%RSI_MIN_ATR_PCT% -v rsi_time_stop_days=%RSI_TIME_STOP_DAYS% -v rsi_entry_on=%RSI_ENTRY_ON% -v rsi_sheet_notional=%RSI_SHEET_NOTIONAL% -s "!RSI_SYMBOLS!" !RSI_FORWARD!
) else (
  "%PY%" stock_analysis\rocket_tbn.py data\newdata\data -o drive -w %RSI_WORKERS% --no-regression %RSI_AGG_FLAG% --initial-capital 500000 --aggressive-max-multiple 2.0 --margin-utilization 0.6 -v rsi_mode=true -v vz_mode=false -v wrl_mode=false -v brt_zones=false -v yh_zones=false -v wpbr_zones=false -v rl_mode=false -v relative_strength_enabled=false -v rs_mode=false -v mvcp_mode=false -v sb_mode=false -v qull_mode=false -v indicator_buy=off -v rsi_ob=%RSI_OB% -v rsi_os=%RSI_OS% -v rsi_exit=%RSI_EXIT% -v rsi_max_trigger=%RSI_MAX_TRIGGER% -v rsi_min_atr_pct=%RSI_MIN_ATR_PCT% -v rsi_time_stop_days=%RSI_TIME_STOP_DAYS% -v rsi_entry_on=%RSI_ENTRY_ON% -v rsi_sheet_notional=%RSI_SHEET_NOTIONAL% !RSI_FORWARD!
)
exit /b %errorlevel%
