#!/usr/bin/env python3
"""One-knob A/Bs under CONTROL = Neutral-after-OB + exit RSI≥80 + min ATR%≥4.

Control freezes EXIT_rsi80 (lean from rsi_ob_neutral_rsi75_atr4_20260910) on top of
the prior rsi75+atr4 combo. Arms differ by exactly one knob. Research only.

Usage:
  python tools/rsi_ob_neutral_rsi80_atr4_ab_20260910.py
"""
...
STAMP = "rsi_ob_neutral_rsi80_atr4_20260910"
OUT_DIR = ROOT / "drive" / "paul_experiments" / STAMP
PRIOR_TUNE = "rsi_ob_neutral_rsi75_atr4_20260910"
...
ORIGINAL_REQUEST = "lets add EXIT_rsi80 to the run"
...
def fixed_arms() -> list[Arm]:
    """Pre-declared one-knob arms vs rsi80+atr4 control (no corr pick)."""
    return [
        Arm(
            "CONTROL",
            "none",
            "Frozen combo: Neutral-after-OB buy, min ATR%≥4 at trigger, sell RSI≥80, next open, no time stop.",
        ),
        Arm(
            "EXIT_rsi75",
            "exit_rsi",
            "Ablate rsi80 lean: sell at prior freeze exit RSI≥75 (still ATR%≥4).",
            exit_rsi=75.0,
        ),
        Arm(
            "EXIT_rsi70",
            "exit_rsi",
            "Ablate to classic OB: sell when RSI≥70 (still ATR%≥4).",
            exit_rsi=70.0,
        ),
        Arm(
            "EXIT_rsi85",
            "exit_rsi",
            "One notch higher exit: sell when RSI≥85.",
            exit_rsi=85.0,
        ),
        Arm(
            "ENTRY_min_atr3",
            "min_atr_pct",
            "Ablate atr4 downward: require ATR%≥3 at trigger (still exit 80).",
            min_atr_pct=3.0,
        ),
        Arm(
            "ENTRY_min_atr5",
            "min_atr_pct",
            "Nudge atr4 upward: require ATR%≥5 at trigger (still exit 80).",
            min_atr_pct=5.0,
        ),
        Arm(
            "ENTRY_no_atr_gate",
            "min_atr_pct",
            "Ablate atr4 entirely: no ATR% gate (still exit 80).",
            min_atr_pct=0.0,
        ),
        Arm(
            "TIME_stop_20d",
            "time_stop_days",
            "Flatten at 20 calendar days if still not exit-RSI (control exit 80 + atr4).",
            time_stop_days=20,
        ),
        Arm(
            "TIME_stop_40d",
...
def pick_extra_arms(corr_is: pd.DataFrame) -> list[Arm]:
    ...
    Skip ATR% gates (already frozen at 4). Cap extras to avoid combinatorial explode.
    """
    skip = {
        ...
        return intro + "".join(
            [
                book_table_for_slice(
                    "IS",
                    "IS (entry &lt; 2024-01-01)",
                    "In-sample book under the rsi80+atr4 control.",
                ),
                book_table_for_slice(
                    "OOS",
...
<body>
<h1>RSI Neutral-after-OB — CONTROL = exit RSI≥80 + min ATR%≥4 — one-knob A/Bs</h1>
...
<p>We already froze “sell at RSI 75” plus “only buy livelier names (ATR%≥4)” as a baseline.
On that book, selling a bit later — at RSI <strong>80</strong> — looked like a quality lean.
You asked to put that exit into the freeze itself and re-run one-change tests around the new
combo (exit 80 + ATR%≥4). Ablations include going back to exit 75 or classic 70, nudging to 85,
ATR gates, and optional time stops. Later years (2024+) are a check only; we did not retune
to make them look better. Research only — not gold, not wired to DailyRun.</p>
</div>
<div class="insight">
...
<p class="caveat"><strong>Selection honesty:</strong> control = prior lean <code>EXIT_rsi80</code>
from <code>{PRIOR_TUNE}</code> (itself built on the rsi75+atr4 combo), chosen after an
in-sample one-knob horse-race. That is in-sample selection. We re-scored FULL/IS/OOS under
the frozen rsi80+atr4 combo; OOS was not used to pick or retune.</p>
</div>
<h2>1. Arms (one knob each vs rsi80+atr4 control)</h2>
...
def write_baseline(path: Path, arms: list[Arm]) -> None:
    lines = [
        ...
        "## In plain English",
        "",
        "Promote the prior lean EXIT_rsi80 into the freeze (on top of atr4), then re-run",
        "one-change A/Bs around exit RSI≥80 + min ATR%≥4. Judge quality, not trade count.",
        "",
        "## Control freeze",
        "",
        ...
        "",
        f"Control adopts **EXIT_rsi80** as a research lean from `{PRIOR_TUNE}` ",
        "(which itself froze EXIT_rsi75 + ENTRY_min_atr4 from an earlier tune stamp). ",
        "EXIT_rsi80 was chosen after an in-sample one-knob horse-race (selection bias). ",
        "This stamp **re-scores** FULL/IS/OOS under the frozen rsi80+atr4 combo and runs ",
...
def write_summary(path: Path, arms: list[Arm], books: dict[str, list[dict]]) -> None:
    ...
    lines = [
        f"# SUMMARY — {STAMP}",
        "",
        "Research only. Not gold. Not DailyRun. Control = exit RSI≥80 + min ATR%≥4 ",
        f"(prior lean EXIT_rsi80 from `{PRIOR_TUNE}` on top of rsi75+atr4 combo; ",
        ...
        "## In plain English",
        "",
        "We put EXIT_rsi80 into the freeze with atr4 and re-tested one change at a time. ",
...
def enrich_control_closed() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    enrich_script = ROOT / "tools" / "rsi_ob_neutral_rsi80_atr4_enrich_20260910.py"
    import subprocess
    r = subprocess.run(
...
def main() -> int:
    ...
    arms = fixed_arms()
    print(f"Phase 1: {len(arms)} fixed arms (control = rsi80 + atr4)", flush=True)
    by_arm = run_all(arms)
