#!/usr/bin/env python3
"""Save/reconcile AMZN (if needed) then AMD vs variant C stamp 260722134152."""
...
def find_paste_line(markers: list[str], ticker: str) -> int:
    hits = []
    with TRANSCRIPT.open(encoding="utf-8", errors="ignore") as fh:
        for i, line in enumerate(fh, 1):
            ...
            if all(m in line for m in markers):
                hits.append(i)
            elif any(m in line for m in markers) and "Break out upper" in line:
                hits.append(i)
    for line_no in reversed(hits):
        body = extract_user_body(line_no)
        if not body:
            continue
        if "Break out upper" in body and "Entry Date" in body:
            head = body[:300]
            if "Date\tOpen\tHigh" in head and head.find("Break out upper") > 500:
                continue
            return line_no
    raise SystemExit(f"{ticker} paste not found; hits={hits[-5:]}")
...
def needs_amzn_refresh() -> bool:
    st = BASE / "AMZN" / "AMZN_wpbr_reconcile_status.md"
    z = BASE / "AMZN" / "zones.tsv"
    js = BASE / "AMZN" / "_reconcile_summary.json"
    if not z.is_file():
        return True
    zt = z.read_text(encoding="utf-8")
    if "29.69" not in zt or "236.50" not in zt:
        return True
    if not st.is_file() or STAMP not in st.read_text(encoding="utf-8"):
        return True
    if js.is_file():
        j = json.loads(js.read_text(encoding="utf-8"))
        if j.get("stamp") == STAMP and j.get("pivots"):
            return False
    return True


def focus_notes(sym: str, pivot_prefix: str, zh_target: float, retest_target: str) -> list[str]:
    import pandas as pd

    notes = []
    df = pd.read_csv(DATA / f"{sym}.csv", index_col=0, parse_dates=True)
    eng_all, _ = build_eng(df)
    sheet_z = load_sheet_zones(BASE / sym)
    target = None
    for z in sheet_z:
        if z.get("zhigh") is not None and abs(float(z["zhigh"]) - zh_target) < 1e-9:
            target = z
            break
    if not target:
        for z in sheet_z:
            if (z.get("pivot") or "").startswith(pivot_prefix):
                target = z
                break
    if not target:
        notes.append(f"Could not locate pivot ~{pivot_prefix} / zh={zh_target}")
        return notes
    ...
    if e:
        zh_ok = abs(e["zhigh"] - zh_target) <= 0.02
        rt_ok = e["retest"] == retest_target
        notes.append(
            f"Engine under C: z=({e['zlow']},{e['zhigh']}) retest `{e['retest']}` "
            f"signal `{e['signal']}` — zh=={zh_target} {'YES' if zh_ok else 'NO'}, "
            f"retest=={retest_target} {'YES' if rt_ok else 'NO'}"
        )
    else:
...
def update_markten_status(results: dict[str, dict], vc: dict, amzn_focus: list[str]) -> Path:
    ...
    lines.append("|---|---|---|---|---|---|---|---:|---|")
    for sym in ("AU", "NVDA", "NFLX", "AMZN", "AMD"):
        r = results[sym]
        if "fair" in r:
            fair = r["fair"]
            note = []
    ...
    for s in ("AU", "NVDA", "NFLX", "AMZN", "AMD"):
...
def reconcile_one(sym: str, markers: list[str], vc: dict, focus=None) -> dict:
    line_no = find_paste_line(markers, sym)
    print(f"{sym} transcript line: {line_no}")
    body = extract_user_body(line_no)
    if not body or "Break out upper" not in body:
        raise SystemExit(f"{sym} paste body invalid")
    save_zones_trades(sym, body)
    ...
    notes = focus() if focus else None
    if notes:
        for n in notes:
            print("  FOCUS:", n.encode("ascii", "replace").decode())
    write_ticker_status(sym, r, vc, extra_notes=notes)
    return r
...
def main() -> int:
    ...
    if needs_amzn_refresh():
        print("AMZN needs refresh")
        r_amzn = reconcile_one(
            "AMZN",
            ["29.69", "28.81"],
            vc,
            focus=lambda: focus_notes("AMZN", "2024-12", 236.5, "2025-11-13"),
        )
        amzn_focus = focus_notes("AMZN", "2024-12", 236.5, "2025-11-13")
    else:
        print("AMZN already reconciled on this stamp")
        r_amzn = analyze("AMZN")
        amzn_focus = focus_notes("AMZN", "2024-12", 236.5, "2025-11-13")
        write_ticker_status("AMZN", r_amzn, vc, extra_notes=amzn_focus)

    r_amd = reconcile_one("AMD", ["4.05", "3.93"], vc)
    ...
    update_markten_status(results, vc, amzn_focus)
    return 0
