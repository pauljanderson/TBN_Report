"""Reconcile AMZN BRT sheet (transcript paste) vs engine stamp 260720143523.

Layers: zones (+/- $0.02), breakouts/retests, closed trades (+/- $0.05).
Also accepts an optional AMZN-only re-run stamp via --trades-stamp for trades
when the multi-symbol preferred stamp has 0 AMZN closed rows.
"""
...
AMZN_CSV = ROOT / "data" / "newdata" / "data" / "AMZN.csv"
...
def extract_paste() -> str:
    ...
    for line in TRANSCRIPT.read_text(encoding="utf-8").splitlines():
        ...
        if (
            ...
            and ("AMZN\nDate" in text or "AMZN\r\nDate" in text or text.lstrip().startswith("AMZN"))
        ):
            latest = text
    if not latest:
        raise SystemExit("AMZN paste not found in transcript")
    if "<user_query>" in latest:
        latest = latest[latest.find("<user_query>") + len("<user_query>") :]
    if "</user_query>" in latest:
        latest = latest[: latest.find("</user_query>")]
    latest = latest.strip()
    if latest.startswith("AMZN\n") or latest.startswith("AMZN\r\n") or latest.startswith("AMZN"):
        # Drop leading symbol label line if present
        first, _, rest = latest.partition("\n")
        if first.strip() == "AMZN" and rest:
            latest = rest
    return latest
...
def ensure_exports(text: str):
    ...
    (TOOLS / "amzn_brt_sheet_zones.tsv").write_text(zones.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "amzn_brt_sheet_breakouts.tsv").write_text(bos.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "amzn_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")
    (OUT / "amzn_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")

    zero_dates = []
    with (OUT / "AMZN_sheet_ohlc.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "AMZN_sheet_zones.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "AMZN_sheet_breakouts.csv").open("w", newline="", encoding="utf-8") as f:
        if bo_rows:
    ...
    with (OUT / "AMZN_sheet_trades.csv").open("w", newline="", encoding="utf-8") as f:
        if tr_rows:
...
def reconcile_zones(unique_sheet, stamp: str):
    eng_path = ROOT / "drive" / f"BRT_ZONES_AMZN_{stamp}.csv"
    rows = []
    ...
    with (OUT / "AMZN_zones_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        f"# AMZN zone reconcile (sheet paste 2026-07-20)",
        "",
        f"- Engine: `BRT_ZONES_AMZN_{stamp}.csv`",
        "- Sheet: `tools/amzn_brt_sheet_zones.tsv` / `AMZN_sheet_zones.csv`",
    ...
    (OUT / "AMZN_zones_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
...
def reconcile_breakouts(bo_rows, stamp: str):
    ...
    with (OUT / "AMZN_breakouts_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# AMZN breakout/retest reconcile",
        "",
        "- Sheet TSV: `tools/amzn_brt_sheet_breakouts.tsv`",
        f"- Engine: `BRT_breakout_and_retest_{stamp}.csv` (AMZN filter)",
        f"- Window: {WIN_START} .. {WIN_END}",
        ...
        f"- Engine BO (AMZN total): **{len(eng_all)}**",
        f"- Engine BO (AMZN in window): **{len(eng_bo)}**",
    ...
    if so:
        ...
        lines += [
            ...
            "No engine-only BOs; every engine AMZN BO matched (retests included).",
            "",
        ]
    else:
    ...
    (OUT / "AMZN_breakouts_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "sheet": len(sheet_bo),
        "engine": len(eng_bo),
        "exact": len(exact),
...
def reconcile_trades(tr_rows, stamp: str):
    ...
    with (OUT / "AMZN_trades_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# AMZN BRT trades reconcile - sheet vs engine",
        "",
        f"- Sheet: `tools/amzn_brt_sheet_trades.tsv` ({len(load_sheet_trades(tr_rows))} closed rows pasted)",
        ...
        f"- Engine AMZN trades in window: **{len(eng_tr)}**",
        "",
        "## Match summary",
    ...
    if matched:
        lines += [
            ...
            "",
            "| sheet trigger | sheet entry | eng CA | eng entry | dentry | sheet exit | eng exit | exit type |",
            "|---|---:|---|---:|---:|---|---|---|",
        ]
    ...
    if so:
        ...
        lines += [
            ...
            "`<=260719*`). Dropped in `260720*` engines (42->27 AMZN zones). Older stamp",
    ...
    if len(eng_tr) == 0:
        note = (
            f"**NOTE:** Preferred multi-symbol stamp `{STAMP}` has **0** AMZN closed trades "
            f"(zones/BOs present). This trades layer used stamp `{stamp}`. "
            "Root cause hypothesis: post-2026-07-19 multi-symbol runs drop AMZN (and ~12 other symbols) "
            "from closed/open outputs despite writing zones/breakouts - likely portfolio/resume path "
            "or symbol-loop skip; AMZN-only re-run recommended for trade parity."
        )
        lines += ["## Engine availability note", "", note, ""]

    (OUT / "AMZN_trades_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "stamp": stamp,
        "sheet": len(sheet_tr),
        "sheet_after": len(sheet_after),
...
def write_summary(exports, z, b, t, zero_dates):
    lines = [
        "# AMZN BRT sheet vs engine - reconcile summary",
        "",
        ...
        f"- Engine AMZN.csv: present, trading-day-only (0 zero-OHLC bars)",
        "",
        "## Counts",
        "",
    ...
    lines.append(
        ...
        "and was dropped starting `260720082240` (42->27 AMZN zones). Sheet still follows the older zone set for this touch."
    )
    lines.append(
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `AMZN_sheet_ohlc.csv`, `AMZN_sheet_zones.csv`, `AMZN_sheet_breakouts.csv`, `AMZN_sheet_trades.csv`",
        "- `AMZN_zones_diff.md`, `AMZN_breakouts_diff.md`, `AMZN_trades_diff.md`",
        "- Match detail CSVs: `AMZN_*_match_detail.csv`",
        "",
    ]
    (OUT / "AMZN_reconcile_summary.md").write_text("\n".join(lines), encoding="utf-8")
    print("\n".join(lines))
...
def pick_trades_stamp(cli_stamp: str | None) -> str:
    if cli_stamp:
        return cli_stamp
    # Prefer preferred stamp if it has AMZN trades; else newest prior stamp with AMZN.
    ...
    print(f"[trades] preferred {STAMP} has 0 AMZN; using {cands[0][0]} ({cands[0][1]} trades)")
    return cands[0][0]
