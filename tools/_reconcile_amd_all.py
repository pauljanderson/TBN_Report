"""Reconcile AMD BRT sheet (transcript paste) vs engine stamp 260720143523.

Layers: zones (+/- $0.02), breakouts/retests, closed trades (+/- $0.05).
Also accepts an optional AMD-only re-run stamp via --trades-stamp for trades
when the multi-symbol preferred stamp has 0 AMD closed rows.
"""
...
AMD_CSV = ROOT / "data" / "newdata" / "data" / "AMD.csv"
...
def extract_paste() -> str:
    """Prefer the latest full AMD paste (OHLC + zones + BOs + trades)."""
    latest = None
    for line in TRANSCRIPT.read_text(encoding="utf-8").splitlines():
        ...
        has_sections = (
            ...
            and "Date\tOpen\tHigh\tLow\tClose" in text
        )
        is_amd = (
            "here is some data for your AMD reconiciling" in text
            or "AMD\nDate\tOpen" in text
            or "AMD\r\nDate\tOpen" in text
            or ("1/4/2010\t$9.7900" in text and has_sections)
        )
        if has_sections and is_amd:
            latest = text
    if not latest:
        raise SystemExit("AMD paste not found in transcript")
    if "<user_query>" in latest:
        latest = latest[latest.find("<user_query>") + len("<user_query>") :]
    if "</user_query>" in latest:
        latest = latest[: latest.find("</user_query>")]
    latest = latest.strip()
    # Drop timestamp wrapper / intro line(s) before OHLC header
...
def ensure_exports(text: str):
    ...
    (TOOLS / "amd_brt_sheet_zones.tsv").write_text(zones.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "amd_brt_sheet_breakouts.tsv").write_text(bos.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "amd_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")
    (OUT / "amd_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")

    zero_dates = []
    with (OUT / "AMD_sheet_ohlc.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "AMD_sheet_zones.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "AMD_sheet_breakouts.csv").open("w", newline="", encoding="utf-8") as f:
        if bo_rows:
    ...
    with (OUT / "AMD_sheet_trades.csv").open("w", newline="", encoding="utf-8") as f:
        if tr_rows:
...
def reconcile_zones(unique_sheet, stamp: str):
    eng_path = ROOT / "drive" / f"BRT_ZONES_AMD_{stamp}.csv"
    rows = []
    ...
    with (OUT / "AMD_zones_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        f"# AMD zone reconcile (sheet paste 2026-07-20)",
        "",
        f"- Engine: `BRT_ZONES_AMD_{stamp}.csv`",
        "- Sheet: `tools/amd_brt_sheet_zones.tsv` / `AMD_sheet_zones.csv`",
    ...
    (OUT / "AMD_zones_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
...
def reconcile_breakouts(bo_rows, stamp: str):
    ...
    with (OUT / "AMD_breakouts_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# AMD breakout/retest reconcile",
        "",
        "- Sheet TSV: `tools/amd_brt_sheet_breakouts.tsv`",
        f"- Engine: `BRT_breakout_and_retest_{stamp}.csv` (AMD filter)",
        f"- Window: {WIN_START} .. {WIN_END}",
        ...
        f"- Engine BO (AMD total): **{len(eng_all)}**",
        f"- Engine BO (AMD in window): **{len(eng_bo)}**",
    ...
    (OUT / "AMD_breakouts_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "sheet": len(sheet_bo),
        "engine": len(eng_bo),
        "exact": len(exact),
...
def reconcile_trades(tr_rows, stamp: str):
    ...
    with (OUT / "AMD_trades_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# AMD BRT trades reconcile — sheet vs engine",
        "",
        f"- Sheet: `tools/amd_brt_sheet_trades.tsv` ({len(load_sheet_trades(tr_rows))} closed rows pasted)",
        ...
        f"- Engine AMD trades in window: **{len(eng_tr)}**",
        "",
        "## Match summary",
    ...
    if len(eng_tr) == 0:
        note = (
            f"**NOTE:** Preferred multi-symbol stamp `{STAMP}` has **0** AMD closed trades "
            f"(zones/BOs present). This trades layer used stamp `{stamp}`. "
            "Root cause hypothesis: post-2026-07-19 multi-symbol runs drop AMD (and ~12 other symbols) "
            "from closed/open outputs despite writing zones/breakouts — likely portfolio/resume path "
            "or symbol-loop skip; AMD-only re-run recommended for trade parity."
        )
        lines += ["## Engine availability note", "", note, ""]

    (OUT / "AMD_trades_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "stamp": stamp,
        "sheet": len(sheet_tr),
        "sheet_after": len(sheet_after),
...
def write_summary(exports, z, b, t, zero_dates, zones_stamp: str, bo_stamp: str, ohlc_check: dict):
    lines = [
        "# AMD BRT sheet vs engine — reconcile summary",
        "",
        ...
        f"- Engine AMD.csv: present, trading-day-only (0 zero-OHLC bars)",
    ...
    if t["engine"] == 0:
        lines.append(
            f"- **Trades:** stamp has **0** AMD closed trades despite BOs. "
            "Often `max_market_cap` default wipes AMD when yfinance leaves market_cap=None — use `max_market_cap=0` or checkpoint rewrite. "
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `AMD_sheet_ohlc.csv`, `AMD_sheet_zones.csv`, `AMD_sheet_breakouts.csv`, `AMD_sheet_trades.csv`",
        "- `AMD_zones_diff.md`, `AMD_breakouts_diff.md`, `AMD_trades_diff.md`",
        "- Match detail CSVs: `AMD_*_match_detail.csv`",
        "- Four-scenario cut-paste: `AMD_four_scenario_stats.md`",
        "",
    ]
    (OUT / "AMD_reconcile_summary.md").write_text("\n".join(lines), encoding="utf-8")
    print("\n".join(lines))


def check_2018_05_18(exports_zero_ok=None):
    """Verify corrected 5/18/2018 bar in sheet OHLC vs engine AMD.csv."""
    sheet_bar = None
    with (OUT / "AMD_sheet_ohlc.csv").open(encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            if r.get("date") == "2018-05-18":
                sheet_bar = (
                    float(r["open"]),
                    float(r["high"]),
                    float(r["low"]),
    ...
    eng_bar = None
    if AMD_CSV.exists():
        with AMD_CSV.open(encoding="utf-8", newline="") as f:
            for r in csv.DictReader(f):
                ...
                if d == "2018-05-18":
                    ...
                    break
    expect = (13.06, 13.26, 12.91, 13.00)
    status = "MISSING"
    if sheet_bar and eng_bar:
        if all(abs(a - b) < 0.015 for a, b in zip(sheet_bar, eng_bar)) and all(
            abs(a - b) < 0.015 for a, b in zip(sheet_bar, expect)
        ):
            status = "MATCH"
        else:
            status = "MISMATCH"
    elif sheet_bar:
        status = "MATCH" if all(abs(a - b) < 0.015 for a, b in zip(sheet_bar, expect)) else "MISMATCH"
    return {
        "sheet": sheet_bar,
        "engine": eng_bar,
        "expected": expect,
...
def pick_trades_stamp(cli_stamp: str | None) -> str:
    if cli_stamp:
        return cli_stamp
    # Prefer preferred stamp if it has AMD trades; else newest prior stamp with AMD.
    ...
    print(f"[trades] preferred {STAMP} has 0 AMD; using {cands[0][0]} ({cands[0][1]} trades)")
    return cands[0][0]
...
def main():
    ...
    ap.add_argument("--zones-stamp", default="260720165857")
    ap.add_argument("--bo-stamp", default="260720165857")
    args = ap.parse_args()

    text = extract_paste()
    exports = ensure_exports(text)
    ohlc_check = check_2018_05_18()
    ...
    tstamp = pick_trades_stamp(args.trades_stamp or "260720165857")
