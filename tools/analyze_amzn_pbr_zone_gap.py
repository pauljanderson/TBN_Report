#!/usr/bin/env python3
"""AMZN PBR zone gap: categorize engine-only vs sheet."""
...
def main() -> None:
    ...
    raw = compute_pbr_touch_stream(df, band_pct=0.015, breakout_confirmation=0.03)
    out = filter_pbr_output_for_compare(raw, df, min_date=SHEET_COMPARE_MIN_DATE)
    sheet = load_pbr_ground_truth()["AMZN"].zones
    engine = out["pbr_zone_events"]
    engine_raw_count = len(raw["pbr_zone_events"])

    sheet_pivots = {z.pivot_date for z in sheet}
    engine_by_pivot = {ev["pivot_monday"].replace("-", ""): ev for ev in engine}

    pre_sheet = [
        ev for ev in raw["pbr_zone_events"]
