#!/usr/bin/env python3
"""RL dip-band tighten A/B on tradable 764 tape (ENTRY).

Evidence: ImprovePriority 260828112205
  band_tighten_weak_fill (lean tighten): 303/2356 trades (13%) MFE<3% then STOP/TIME
  shallow_entry_sma50_fail: 71 quick STOPs with entry hugging SMA50

One ENTRY knob: tighten `rl_dip_pct` only (narrower SMA50 dip band).
Do not move slope / sma_qual / expansion / stop / target on this stamp.
Do not reuse tools/rl_stop_ab.py (old dip 1.041 freeze + house 59 grid).

Control: 1.055 (±5.5%) reuse tradable Closed 260828112205.
Candidates (pre-agreed, not a grid):
  1.041 (±4.1%) modest — old house freeze
  1.030 (±3.0%) stronger

Research-only. Not gold. Not DailyRun. Do not overwrite RL_universe.csv.

Usage:
  python tools/rl_dip_tighten_ab.py
  python tools/rl_dip_tighten_ab.py --summarize-only
"""
...
DRIVE = ROOT / "drive"
STAMP = "20260828"
OUT_DIR = DRIVE / "paul_experiments" / f"rl_dip_tighten_tradable_{STAMP}"
CONTROL_SRC = (
...
ARM_ORDER = {"control": 0, "dip041": 1, "dip030": 2}
...
def build_cmd(py: str, outdir: Path, workers: int, symbols: str, extra_v: list[str]) -> list[str]:
    ...
    i = 0
    while i < len(cmd):
        if cmd[i] == "-v" and i + 1 < len(cmd) and cmd[i + 1].startswith("rl_dip_pct="):
            i += 2
            continue
        out.append(cmd[i])
        i += 1
...
def _decision(verdicts: dict[str, dict[str, tuple[str, str]]]) -> str:
    vis_a, vis_b = verdicts["dip041"]["is"][0], verdicts["dip030"]["is"][0]
    voos_a, voos_b = verdicts["dip041"]["oos"][0], verdicts["dip030"]["oos"][0]
    keepish = {"KEEP", "LEAN KEEP"}
    if vis_a not in keepish and vis_b not in keepish:
        return (
            f"**DISMISS** both. IS 1.041={vis_a}; 1.030={vis_b}. "
            f"OOS report-only 1.041={voos_a}; 1.030={voos_b}. "
            "Do not search more dip values. Do not pile on slope on this stamp."
        )
    if (vis_a in keepish and voos_a == "DISMISS") or (vis_b in keepish and voos_b == "DISMISS"):
        return (
            f"**HOLD**. IS quality looked better (1.041: {vis_a}; 1.030: {vis_b}) but OOS softened "
            f"(1.041: {voos_a}; 1.030: {voos_b}). Do not retune OOS. Research-only."
        )
    return (
        f"IS 1.041={vis_a}; 1.030={vis_b}. OOS report-only 1.041={voos_a}; 1.030={voos_b}. "
...
def write_compare_html(
    ...
    for split_key, title in (("m_is", "IS"), ("m_oos", "OOS (report-only)"), ("m_full", "FULL book")):
        ...
        sections.append(
            f'<section><h2>Dip tighten — {title}</h2>'
            f'<p class="muted">Δ vs control (rl_dip_pct=1.055, ±5.5%). {note} Click column headers to sort.</p>'
    ...
    pairs = [
        ("1.041 − control", by_id[CONTROL_ID], by_id["dip041"]),
        ("1.030 − control", by_id[CONTROL_ID], by_id["dip030"]),
        ("1.030 − 1.041", by_id["dip041"], by_id["dip030"]),
    ]
    pw_sections = []
...
<title>RL dip tighten A/B — tradable {STAMP}</title>
<style>
...
<body>
<header>
<h1>RL dip tighten — tradable 764 (ENTRY)</h1>
<p class="muted">Stamp <code>rl_dip_tighten_tradable_{STAMP}</code>. One knob: <code>rl_dip_pct</code> tighten only
(1.055 → 1.041 / 1.030). Freeze: expansion=1.163, stop=0.934, target=1.20, slope off, trails off, post-TARGET bars=0.
Not gold / not DailyRun. IS = entry &lt; 2024-01-01; OOS report-only. Click column headers to sort.</p>
</header>
<main>
<div class="callout">
<strong>Evidence:</strong> ImprovePriority <code>260828112205</code>
<em>band_tighten_weak_fill</em> (303/2356 = 13% MFE&lt;3% then STOP/TIME) and
<em>shallow_entry_sma50_fail</em> (71 quick STOPs hugging SMA50).
Hypothesis: a narrower 50-SMA dip band drops shallow fills and lifts trade quality
(Avg%, WO_MAX, PF, WR) even if N falls. Pre-agreed: <strong>1.041</strong> (±4.1%) and
<strong>1.030</strong> (±3.0%) vs control <strong>1.055</strong> (±5.5%).
Do not A/B slope / sma_qual on this stamp. Judge quality over count.
<ul>{"".join(v_lis)}</ul>
...
def write_docs(packed: list[dict[str, Any]], verdicts: dict[str, dict[str, tuple[str, str]]]) -> None:
    by_id = {p["arm"]["id"]: p for p in packed}
    hyp = f"""# HYPOTHESIS — RL dip-band tighten (ENTRY)

| Field | Fill in |
|-------|---------|
| System / prefix | RL |
| Baseline stamp | tradable Closed `260828112205` (`rl_dip_pct=1.055`) |
| Universe | tradable 2010 / ADV$2m (764) |
| **Evidence** | ImprovePriority `260828112205` **band_tighten_weak_fill**: 303/2356 (13%) MFE<3% then STOP/TIME; **shallow_entry_sma50_fail**: 71 quick STOPs hugging SMA50 |
| **Hypothesis** | A narrower SMA50 dip band filters shallow/noise fills and lifts IS quality (Avg%, WO_MAX, PF, WR) without collapsing N |
| **Single knob** | `rl_dip_pct` tighten only (ENTRY) |
| Frozen settings | expansion=1.163, stop=0.934, target=1.20, slope=0, trails off, `rl_post_target_reentry_bars=0` |
| Alternatives | control **1.055** (±5.5%); **1.041** (±4.1%); **1.030** (±3.0%). Not 1.01. Not slope on this stamp |
...
"""
    ...
    lines = [
        f"# BASELINE — `rl_dip_tighten_tradable_{STAMP}`",
        "",
        "**Status:** RESEARCH only. Not gold. Not DailyRun. One ENTRY knob (dip tighten).",
        "",
        "| Field | Value |",
        "|---|---|",
        "| Universe | tradable 2010 / ADV$2m (764) |",
        "| Control | reuse `RL_Closed_260828112205` (`rl_dip_pct=1.055`, ±5.5%) |",
        "| Candidates | 1.041 (±4.1%); 1.030 (±3.0%) |",
        "| Frozen | expansion=1.163, stop=0.934, target=1.20, slope off, trails off, post-TARGET bars=0 |",
        "| Split | IS entry < 2024-01-01; OOS report-only |",
        "",
        "Do **not** retune on OOS. Do **not** pile on `rl_slope_threshold` in this stamp.",
        "Do **not** overwrite `RL_universe.csv`. Judge quality over N.",
        "",
    ...
    slines = [
        f"# SUMMARY — `rl_dip_tighten_tradable_{STAMP}`",
        "",
        "Dip-band tighten (`rl_dip_pct` 1.041 / 1.030 vs 1.055) on tradable 764. ENTRY. Research only.",
        "",
        "## IS",
        "",
    ]
...
def summarize(packed: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    print(f"[RL-DT] Wrote {OUT_DIR / 'compare.html'}", flush=True)
...
def main() -> int:
    ...
    if not trad:
        print("[RL-DT] Missing tradable universe", flush=True)
        return 1
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    arms = [
        {
            "id": "control",
            "label": "Control (dip=1.055, ±5.5%)",
            "role": "control",
            "symbols": trad,
            "extra_v": ["rl_dip_pct=1.055"],
            "live": False,
        },
        {
            "id": "dip041",
    ...
    if args.summarize_only:
        for arm in arms:
            ...
            if not closed:
                print(f"[RL-DT] Missing Closed for {arm['id']}", flush=True)
                return 1
            trades = load_trades(closed)
            runs.append(
                {
        ...
        print(
            f"[RL-DT] control reuse ok={ctrl_run['ok']} n={len(ctrl_run['trades'])} stamp={ctrl_run.get('stamp')}",
            flush=True,
        )
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as ex:
            ...
            for fut in as_completed(futs):
                ...
                run = fut.result()
                print(
                    f"[RL-DT] {arm['id']} ok={run['ok']} n={len(run['trades'])} "
    ...
    if not all(r.get("ok") for r in runs):
        print("[RL-DT] One or more arms failed", flush=True)
        for r in runs:
    ...
    if ntfy.is_file():
        a = next(p for p in packed if p["arm"]["id"] == "dip041")
        b = next(p for p in packed if p["arm"]["id"] == "dip030")
        subprocess.run(
            [
                py,
                str(ntfy),
                ...
                "-t",
                "RL dip tighten tradable",
                "-m",
                (
                    f"1.041 IS {result['verdicts']['dip041']['is'][0]} "
                    f"Avg={a['m_is']['avg_pnl']:.2f}% N={a['m_is']['n']} · "
                    f"1.030 IS {result['verdicts']['dip030']['is'][0]} "
                    f"Avg={b['m_is']['avg_pnl']:.2f}% N={b['m_is']['n']}"
                ),
            ],
            cwd=str(ROOT),
        )
