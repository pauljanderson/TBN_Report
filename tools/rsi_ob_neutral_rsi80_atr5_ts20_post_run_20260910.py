#!/usr/bin/env python3
"""Post-run (no charts, no Yahoo) on rsi80+atr5+ts20 CONTROL Closed book."""
...
OUT = ROOT / "drive" / "paul_experiments" / "rsi_ob_neutral_rsi80_atr5_ts20_20260910"
...
LAYMAN = (
    "This is the after-the-run checkup on the new baseline that freezes exit Relative Strength "
    "Index (RSI) ≥80 plus Average True Range percent (ATR%) ≥5 plus a 20-day calendar time stop "
    "(preference-adopted after it was DISMISS on quality under the prior atr5 / combo look). "
...
def _prepend_ask(path: Path) -> None:
    ...
    block = (
        ...
        "TIME_stop_20d was DISMISS on quality under "
        "<code>rsi_ob_neutral_rsi80_atr5_20260910</code> / combo look; Paul chose it by "
        "preference into the freeze — not a KEEP from metrics. "
