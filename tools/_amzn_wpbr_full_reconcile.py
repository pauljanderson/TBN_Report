#!/usr/bin/env python3
"""AMZN WPBR FULL reconcile: weekly pivots + zones + retests + rockets + trades.

Engine truth = live compute_wpbr_touch_stream(retest_mode=stop_looking) on AMZN.csv
(identical settings to MarkTen stamp 260722105625) + serialized WPBR_Closed from
that same MarkTen run. Sheet = user paste under drive/wpbr_sheet_reconcile/AMZN/.
"""
...
DATA = REPO / "data" / "newdata" / "data" / "AMZN.csv"
...
ENG_ZONES = (REPO / "drive" / "wpbr_sheet_reconcile" / "_markten_retest_2016"
             / "WPBR_ZONES_AMZN_260722105625.csv")
...
P(f"=== {SYMBOL} WPBR FULL RECONCILE (sheet vs engine stop_looking; MarkTen 260722105625) ===")
P(f"{SYMBOL} price bars={len(df)} ({idx.min().date()}..{idx.max().date()})")
P(f"Sheet: weekly={n_weekly} rows, zones={len(sheet_z)}")
P(f"Engine zones (all history)={len(eng)}; retest_mode={RETEST_MODE}")
P(f"Engine zones file exists: {ENG_ZONES.exists()} ({ENG_ZONES.name})")
P("")
...
for p in sorted(sheet_z):
    ...
    if det:
        line = (f"{p:>10} {fmt(chk['zone'])}{fmt(chk['bo'])}{fmt(chk['conf'])}{fmt(chk['next'])}"
...
eng_only_2016 = [p for p in eng_only_piv if p and p >= MIN_DATE]
if eng_only_2016:
    P(f"Engine-only pivots (>=2016, no sheet zone): {eng_only_2016}")
P("")
...
out[2] = f"Sheet: weekly={n_weekly} rows, zones={len(sheet_z)}, trades={len(sheet_trades)}"

eng_closed = pd.read_csv(ENG_CLOSED, dtype=str)
eng_closed = eng_closed[eng_closed["SYMBOL"] == SYMBOL].reset_index(drop=True)
eng_entry_dates = {nd(x) for x in eng_closed["DATE_OPENED"]}

P(f"Engine serialized {SYMBOL} closed trades (portfolio one-slot; start 2016-01-01):")
...
P("")
P("Engine raw WPBR signal fills (>=2016): " + " ".join(sorted(x for x in eng_fill_dates if x)))

rocket_sheet = sum(1 for p in sheet_z if p in eng and sheet_z[p]["rocket"])
rocket_ok = sum(1 for p in sheet_z if p in eng and sheet_z[p]["rocket"]
                and sheet_z[p]["rocket"] == eng[p]["signal"])
P("")
P(f"Rocket where sheet fires: {rocket_ok}/{rocket_sheet}")
...
summary = {
    ...
    "n_qual": n_qual,
    "n_zones": len(sheet_z),
    "n_trades": len(sheet_trades),
    "ohlc_n": len(ZONES.parent.joinpath("sheet_ohlc.tsv").read_text(encoding="utf-8").splitlines()) - 1
              if ZONES.parent.joinpath("sheet_ohlc.tsv").exists() else None,
    "wp_match": wp_match,
    "wp_zone": wp_zone,
    "stats": {k: list(v) for k, v in stats.items()},
    "retest_parity": r_match + r_blank,
    "retest_matched": matched,
    "retest_detail": {
        "date_match": r_match, "blank_match": r_blank,
        "eng_only": r_engonly, "sheet_only": r_sheetonly, "diff": r_diff,
    },
    "sig_hits": sig_hits,
    "ser_hits": ser_hits,
    "eng_closed": len(eng_closed),
    "rocket_ok": rocket_ok,
    "rocket_sheet": rocket_sheet,
    "mismatches": mismatches[:20],
    "zone_missing": zone_missing,
    "eng_only_2016": eng_only_2016,
    "sheet_only_trades": sheet_only,
    "sheet_trades": sheet_trades,
    "eng_trades": [
        {
            "entry": nd(r["DATE_OPENED"]), "entry_px": float(r["ENTRY_PRICE"]),
            "exit": nd(r["DATE_CLOSED"]), "exit_px": float(r["EXIT_PRICE"]),
            "pnl": r.get("PNL_PCT", ""), "xt": r.get("EXIT_TYPE", ""),
            "match": nd(r["DATE_OPENED"]) in sheet_entry_set,
        }
        for _, r in eng_closed.iterrows()
    ],
    "eng_fill_dates": sorted(x for x in eng_fill_dates if x),
    "eng_zones_file_exists": ENG_ZONES.exists(),
}
(OUT / "_reconcile_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
