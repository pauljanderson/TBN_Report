#!/usr/bin/env python3
"""Fund-scorecard gate AB on RL / RS / VZ Closed books (research-only).

Overlay filter by house scorecard **Quality pillar** (`score_quality` ≥ 60 / ≥ 70).
**Not** Composite, Valuation, or Growth Stability — see expanded stamp
`fund_score_gate_pillars_rl_rs_vz_20260831` / `tools/fund_score_gate_pillars_rl_rs_vz_ab.py`.

No engine rerun. Scores are a **point-in-time snapshot** (fund_scorecard_v1_20260830) —
applying them to historical entries is a **descriptive contaminated overlay / research
upper-bound**, not a clean historical gate. Label selection bias; OOS report-only.

Usage:
  python tools/fund_score_gate_rl_rs_vz_ab.py
"""
...
STAMP = "fund_score_gate_rl_rs_vz_20260831"
OUT = ROOT / "drive" / "paul_experiments" / STAMP
SCORES = ROOT / "drive" / "paul_experiments" / "fund_scorecard_v1_20260830" / "scores.csv"

# Prefer 764 tradable RL for breadth (not house 59).
RL_CLOSED = (
    ROOT
    / "drive"
    / "paul_experiments"
...
# DualPaul78 live Closed from vz_tradable stamp BASELINE (83-name Paul cut → 76 traded).
...
# Lean arms: control + Quality pillar ≥60 + ≥70 (NOT composite — see pillars stamp).
ARMS_SPEC = [
    {"id": "control", "label": "control (full book)", "pillar": None, "thr": None},
    {"id": "quality_ge_60", "label": "Quality pillar ≥ 60", "pillar": "score_quality", "thr": 60.0},
    {"id": "quality_ge_70", "label": "Quality pillar ≥ 70", "pillar": "score_quality", "thr": 70.0},
]
...
def build_compare_table(
    ...
    # Hard omit dollar book totals even if filter leaves Total PnL.
...
Contaminated overlay — today's Quality scores on historical entries.</p>
...
def system_verdict(arms: list[dict[str, Any]], control: dict[str, Any]) -> tuple[str, str]:
    """Pick KEEP/HOLD/DISMISS from IS quality; OOS softens → HOLD."""
    notes = []
    best_id = "control"
    best_tag = "HOLD"
    for a in arms:
        if a["id"] == "control":
            continue
        packed_cand = {"m_is": a["m_is"], "m_oos": a["m_oos"], "m_full": a["m_full"]}
        packed_ctrl = {
            "m_is": control["m_is"],
            "m_oos": control["m_oos"],
            "m_full": control["m_full"],
        ...
        notes.append(f"{a['id']}: IS={is_v} ({is_n}); OOS={oos_v} ({oos_n})")
        # Contaminated overlay: never stronger than LEAN KEEP; OOS soften → HOLD
        tag = is_v
        if is_v in ("KEEP", "LEAN KEEP") and oos_v == "DISMISS":
            tag = "HOLD"
            notes[-1] += " → overall HOLD (OOS softens; no OOS retune)"
        if tag in ("LEAN KEEP", "KEEP") and best_tag not in ("LEAN KEEP", "KEEP"):
            best_tag = "LEAN KEEP"  # research-only ceiling under contamination
            best_id = a["id"]
        elif tag == "DISMISS" and best_tag == "HOLD" and best_id == "control":
            # track dismissals separately in note only
            pass
    # If no arm LEAN KEEP and any DISMISS without KEEP → DISMISS theme; else HOLD
    is_tags = []
    for a in arms:
        if a["id"] == "control":
            continue
        ...
        is_v, _ = verdict_vs_control(packed_cand, packed_ctrl, "m_is")
        oos_v, _ = verdict_vs_control(packed_cand, packed_ctrl, "m_oos")
        if is_v in ("KEEP", "LEAN KEEP") and oos_v != "DISMISS":
            is_tags.append("LEAN KEEP")
        elif is_v == "DISMISS":
            is_tags.append("DISMISS")
        else:
            is_tags.append("HOLD")
    if "LEAN KEEP" in is_tags:
        overall = "LEAN KEEP"
...
def write_html(path: Path, packed_systems: list[dict[str, Any]], meta: dict[str, Any]) -> None:
    sections = []
    for sys_pack in packed_systems:
        sid = sys_pack["id"]
...
<title>Fund score gate RL/RS/VZ — {STAMP}</title>
<style>
...
<h1>House fund scorecard gate — RL / RS / VZ</h1>
<p class="meta"><strong>Research only.</strong> Not gold. Not DailyRun. Overlay on Closed CSVs — no engine rerun.
Stamp <code>{STAMP}</code>. Score source <code>{esc(str(SCORES.relative_to(ROOT)))}</code>
...
<li>Joining today's Quality onto past entries is a <strong>descriptive contaminated overlay</strong>
(research upper-bound), <em>not</em> a clean historical PIT gate.</li>
<li>True IS freeze of "Quality ≥ Q as-of entry" is unavailable without historical score panels.</li>
<li>Arms use fixed a-priori thresholds (60 / 70); Financial Health skipped (often NA). OOS report-only.</li>
<li>Selection bias: choosing to test Quality gates after building the scorecard is labeled; do not claim gold.</li>
</ul>
</div>

<div class="box">
<strong>Hypothesis</strong>
<p>Does filtering entries/symbols by house fund scorecard <em>Quality</em> improve trade quality
(Win%, Avg PnL%, WO_MAX, PF) vs the full Closed book?</p>
<strong>Arms (same all systems)</strong>
<ul>
<li><code>control</code> — full Closed book</li>
<li><code>quality_ge_60</code> — keep symbols with <code>score_quality ≥ 60</code></li>
<li><code>quality_ge_70</code> — keep symbols with <code>score_quality ≥ 70</code></li>
</ul>
<p>Missing Quality → fail gate. IS = entry &lt; 2024-01-01; OOS ≥ 2024-01-01.</p>
</div>

{"".join(sections)}

<p class="meta">Generated {date.today().isoformat()}. Tool <code>tools/fund_score_gate_rl_rs_vz_ab.py</code>.</p>
{SORTABLE_TABLE_SCRIPT}
</body></html>
"""
...
def write_baseline(path: Path, packed_systems: list[dict[str, Any]], meta: dict[str, Any]) -> None:
    lines = [
        f"# BASELINE — `{STAMP}`",
        "",
        "**Status:** RESEARCH only. **Not gold. Not DailyRun-wired.**",
        "",
        "## Hypothesis (one theme)",
        "",
        "Does filtering entries/symbols by house fund scorecard **Quality** improve trade quality",
        "on RL / RS / VZ Closed books vs full-book control?",
        "",
        "House proxies — **not** Fidelity / S&P Global parity. See",
        "`fund_scorecard_v1_20260830/BASELINE.md`.",
        "",
        "## Look-ahead honesty (critical)",
        "",
        "- Score source: `drive/paul_experiments/fund_scorecard_v1_20260830/scores.csv`",
        "  (snapshot as-of scorecard stamp — **not** point-in-time at entry).",
        "- Applying today's Quality to historical Closed trades = **descriptive contaminated overlay**",
        "  / research upper-bound. **Not** a clean historical gate.",
        "- Preferred lottery-style IS membership freeze from historical Quality is **unavailable**",
        "  without a PIT score panel; we document contamination instead of pretending.",
        "- Thresholds 60 / 70 chosen a priori (lean); not retuned on OOS.",
        "- Financial Health gate skipped (often NA). Valuation / composite left for later if needed.",
        "",
        "## Books",
        "",
    ]
    ...
    lines += [
        "",
        "## Arms (identical per system)",
        "",
        "| Arm | Rule |",
        "|-----|------|",
        "| `control` | Full Closed book |",
        "| `quality_ge_60` | Keep symbol iff `score_quality ≥ 60` (missing → drop) |",
        "| `quality_ge_70` | Keep symbol iff `score_quality ≥ 70` (missing → drop) |",
        "",
        "## Process",
        "",
        ...
        "## Artifacts",
        "",
        "- `compare.html` — per-system IS / OOS / FULL sortable tables",
        "- `metrics_all.csv` — flat metrics",
        "- `*_quality_ge_*_symbols.csv` — keep lists",
        "- `SUMMARY.md` — KEEP / HOLD / DISMISS",
        "",
...
def write_summary(path: Path, packed_systems: list[dict[str, Any]]) -> None:
    lines = [
        ...
        "## Verdicts",
        "",
        "| System | Book | Verdict | Note |",
        "|--------|------|---------|------|",
    ]
    for s in packed_systems:
        note = s["verdict_note"].replace("|", "/")
        lines.append(
    ...
    for s in packed_systems:
        lines.append(f"### {s['id']}")
        lines.append("")
        lines.append("| Arm | IS N | IS Avg% | IS WO_MAX | IS PF | OOS N | OOS Avg% | OOS WO_MAX | OOS PF |")
        lines.append("|-----|------|---------|-----------|-------|-------|----------|------------|--------|")
        for a in s["arms"]:
    ...
    worthwhile = []
    for s in packed_systems:
        worthwhile.append(f"- **{s['id']}:** {s['verdict']}")
    lines += [
        "## Are house scores worthwhile for these systems?",
        "",
        "Under a **contaminated** Quality overlay (research upper-bound):",
        "",
        *worthwhile,
        "",
        "- **LEAN KEEP** ⇒ Quality filter *looks* helpful on IS without OOS collapse — still research-only;",
        "  needs PIT scores / walk-forward before stronger claim.",
        "- **HOLD** ⇒ flat / mixed / OOS softens — do not adopt.",
        "- **DISMISS** ⇒ Quality gate hurts IS quality — house Quality not useful as a hard gate here.",
        "",
        "Overall: treat this stamp as a **screen for whether Quality is even directionally useful**.",
        "Do not wire DailyRun from contaminated overlays.",
        "",
    ]
    path.write_text("\n".join(lines), encoding="utf-8")
...
def run_system(sys_cfg: dict[str, Any], scores: pd.DataFrame) -> dict[str, Any]:
    ...
    verd, vnote = system_verdict(arms, control)
    try:
...
def ntfy(paths: list[Path]) -> None:
    ...
    cmd.extend(["-t", "Fund score gate AB done", "-m", f"{STAMP} RL/RS/VZ compare.html"])
    try:
...
def main() -> int:
    ...
    if not SCORES.is_file():
        raise SystemExit(f"Missing scores: {SCORES}")
    scores = load_scores(SCORES)
    packed = [run_system(s, scores) for s in SYSTEMS]
    meta = {"n_scores": int(len(scores))}

    write_metrics_csv(OUT / "metrics_all.csv", packed)
    write_keep_lists(OUT, packed)
    write_html(OUT / "compare.html", packed, meta)
    write_baseline(OUT / "BASELINE.md", packed, meta)
    write_summary(OUT / "SUMMARY.md", packed)

    ntfy([OUT / "compare.html"])
