#!/usr/bin/env python3
"""One-knob A/Bs under CONTROL = Neutral-after-OB + exit RSI≥75 + min ATR%≥4.

Control freezes the two research leans from rsi_ob_neutral_tune_ab_20260910
(EXIT_rsi75 + ENTRY_min_atr4). Arms differ by exactly one knob. Research only.

Usage:
  python tools/rsi_ob_neutral_rsi75_atr4_ab_20260910.py
"""
...
STAMP = "rsi_ob_neutral_rsi75_atr4_20260910"
OUT_DIR = ROOT / "drive" / "paul_experiments" / STAMP
PRIOR_TUNE = "rsi_ob_neutral_tune_ab_20260910"
...
def fixed_arms() -> list[Arm]:
    """Pre-declared one-knob arms vs rsi75+atr4 control (no corr pick)."""
    return [
        Arm(
            "CONTROL",
            "none",
            "Frozen combo: Neutral-after-OB buy, min ATR%≥4 at trigger, sell RSI≥75, next open, no time stop.",
        ),
        Arm(
            "EXIT_rsi70",
            "exit_rsi",
            "Ablate rsi75 lean: sell at classic OB≥70 (still ATR%≥4).",
            exit_rsi=70.0,
        ),
        Arm(
            "EXIT_rsi80",
            "exit_rsi",
            "One notch higher exit: sell when RSI≥80.",
            exit_rsi=80.0,
        ),
        Arm(
            "ENTRY_min_atr3",
            "min_atr_pct",
            "Ablate atr4 downward: require ATR%≥3 at trigger (still exit 75).",
            min_atr_pct=3.0,
        ),
        Arm(
            "ENTRY_min_atr5",
            "min_atr_pct",
            "Nudge atr4 upward: require ATR%≥5 at trigger (still exit 75).",
            min_atr_pct=5.0,
        ),
        Arm(
            "ENTRY_no_atr_gate",
            "min_atr_pct",
            "Ablate atr4 entirely: no ATR% gate (still exit 75).",
            min_atr_pct=0.0,
        ),
        Arm(
            "TIME_stop_20d",
            "time_stop_days",
            "Flatten at 20 calendar days if still not exit-RSI (control exit 75 + atr4).",
            time_stop_days=20,
        ),
        Arm(
            "TIME_stop_40d",
        ...
        return intro + "".join(
            [
                book_table_for_slice(
                    "IS",
                    "IS (entry &lt; 2024-01-01)",
                    "In-sample book under the rsi75+atr4 control.",
                ),
                book_table_for_slice(
                    "OOS",
...
<body>
<h1>RSI Neutral-after-OB — CONTROL = exit RSI≥75 + min ATR%≥4 — one-knob A/Bs</h1>
...
<p>Last time we tested one change at a time against a plain “buy after too-hot cools,
sell when too-hot again” book. Two ideas looked better on quality: sell only when RSI
hits <strong>75</strong> (not 70), and only take names that are <strong>moving enough</strong>
(ATR% at least 4 on the signal day). You asked to freeze <em>both</em> of those as the
new baseline and re-run one-change tests around that combo — not mix more knobs.
Later years (2024+) are a check only; we did not retune to make them look better.
This is research only — not gold, not wired to DailyRun.</p>
</div>
<div class="insight">
...
<p class="caveat"><strong>Selection honesty:</strong> this control combines two leans chosen after
an in-sample one-knob horse-race in <code>{PRIOR_TUNE}</code>. That is in-sample selection.
We re-scored FULL/IS/OOS under the frozen combo; OOS was not used to pick or retune.</p>
</div>
<h2>1. Arms (one knob each vs rsi75+atr4 control)</h2>
...
def write_baseline(path: Path, arms: list[Arm]) -> None:
    lines = [
        ...
        "## In plain English",
        "",
        "Freeze both prior leans (sell RSI≥75 and require ATR%≥4 at trigger) as the new control,",
        "then re-run one-change A/Bs around that combo. Judge quality, not trade count.",
        "",
        "## Control freeze",
        "",
        ...
        "",
        f"Control is the **combination** of two research leans from `{PRIOR_TUNE}` ",
        "(`EXIT_rsi75` + `ENTRY_min_atr4`). Those leans were chosen after an in-sample ",
        "one-knob horse-race (selection bias). This stamp **re-scores** FULL/IS/OOS under ",
        "the frozen combo and runs new one-knob arms around it. OOS is report-only — ",
        "do not retune on OOS. Not gold. Not DailyRun.",
        "",
...
def write_summary(path: Path, arms: list[Arm], books: dict[str, list[dict]]) -> None:
    ...
    lines = [
        f"# SUMMARY — {STAMP}",
        "",
        "Research only. Not gold. Not DailyRun. Control = exit RSI≥75 + min ATR%≥4 ",
        f"(combo of two leans from `{PRIOR_TUNE}`; in-sample selection, then re-scored).",
        "",
        "## What you asked",
        "",
        ORIGINAL_REQUEST,
        ...
        "## In plain English",
        "",
        "We froze both prior leans as the baseline and re-tested one change at a time. ",
...
def enrich_control_closed() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    enrich_script = ROOT / "tools" / "rsi_ob_neutral_rsi75_atr4_enrich_20260910.py"
    import subprocess
    r = subprocess.run(
...
def main() -> int:
    ...
    arms = fixed_arms()
    print(f"Phase 1: {len(arms)} fixed arms (control = rsi75 + atr4)", flush=True)
    by_arm = run_all(arms)
