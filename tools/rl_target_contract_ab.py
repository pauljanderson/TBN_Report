#!/usr/bin/env python3
"""RL target-contract A/B on tradable 764 tape (EXIT, turnover).

Evidence: ImprovePriority 260828112205 slow_target_grind (156 sym / 190 trades)
and target_pct_tension contract side: 190/902 TARGET exits (21%) held ≥100d.

Expand (1.25 / 1.30) already DISMISS on WR / host DD / OOS AnnROR.
This stamp is the unused opposite: closer SMA50 target to recycle capital.

One EXIT knob: lower `rl_target_pct` only. Do not move `rl_expansion`.
Do not pile on partial / time-stop / trails on this stamp.

Control: target=1.20 reuse tradable Closed 260828112205.
Candidates (pre-agreed, not a grid): 1.18 and 1.15.
Stop stays 0.934. Trails off. post_target_reentry_bars=0.

Judge quality (Avg/WO_MAX/PF/WR) first; AnnROR / hold days are the claimed
mechanism — KEEP only if quality holds AND recycling improves. Do not KEEP
on AnnROR alone if Avg collapses (same trap as partial).

Research-only. Not gold. Not DailyRun. Do not overwrite RL_universe.csv.

Usage:
  python tools/rl_target_contract_ab.py
  python tools/rl_target_contract_ab.py --summarize-only
"""
...
DRIVE = ROOT / "drive"
STAMP = "20260828"
OUT_DIR = DRIVE / "paul_experiments" / f"rl_target_contract_tradable_{STAMP}"
CONTROL_SRC = (
...
ARM_ORDER = {"control": 0, "tgt118": 1, "tgt115": 2}
...
def build_cmd(py: str, outdir: Path, workers: int, symbols: str, extra_v: list[str]) -> list[str]:
    ...
    i = 0
    while i < len(cmd):
        if cmd[i] == "-v" and i + 1 < len(cmd) and cmd[i + 1].startswith("rl_target_pct="):
            i += 2
            continue
        out.append(cmd[i])
        i += 1
...
def _decision(verdicts: dict[str, dict[str, tuple[str, str]]]) -> str:
    vis_a, vis_b = verdicts["tgt118"]["is"][0], verdicts["tgt115"]["is"][0]
    voos_a, voos_b = verdicts["tgt118"]["oos"][0], verdicts["tgt115"]["oos"][0]
    keepish = {"KEEP", "LEAN KEEP"}
    if vis_a not in keepish and vis_b not in keepish:
        return (
            f"**DISMISS** both. IS 1.18={vis_a}; 1.15={vis_b}. "
            f"OOS report-only 1.18={voos_a}; 1.15={voos_b}. "
            "Do not search more target values. Do not pile on partial / time-stop on this stamp."
        )
    if (vis_a in keepish and voos_a == "DISMISS") or (vis_b in keepish and voos_b == "DISMISS"):
        return (
            f"**HOLD**. IS quality looked better (1.18: {vis_a}; 1.15: {vis_b}) but OOS softened "
            f"(1.18: {voos_a}; 1.15: {voos_b}). Do not retune OOS. Research-only."
        )
    return (
        f"IS 1.18={vis_a}; 1.15={vis_b}. OOS report-only 1.18={voos_a}; 1.15={voos_b}. "
        "Research candidate ≠ gold ≠ DailyRun. KEEP only if quality holds AND recycling "
        "(AnnROR / hold days) improves — do not KEEP on AnnROR alone if Avg collapses."
    )
...
def write_compare_html(
    ...
    for split_key, title in (("m_is", "IS"), ("m_oos", "OOS (report-only)"), ("m_full", "FULL book")):
        ...
        note = (
            ...
            "Turnover hypothesis: also read Ann ROR / avg days / cap days."
        )
        sections.append(
            f'<section><h2>Target contract — {title}</h2>'
            f'<p class="muted">Δ vs control (rl_target_pct=1.20). {note} Click column headers to sort.</p>'
    ...
    pairs = [
        ("1.18 − control", by_id[CONTROL_ID], by_id["tgt118"]),
        ("1.15 − control", by_id[CONTROL_ID], by_id["tgt115"]),
        ("1.15 − 1.18", by_id["tgt118"], by_id["tgt115"]),
    ]
    pw_sections = []
...
<title>RL target contract A/B — tradable {STAMP}</title>
<style>
...
<body>
<header>
<h1>RL target contract — tradable 764 (EXIT, turnover)</h1>
<p class="muted">Stamp <code>rl_target_contract_tradable_{STAMP}</code>. One knob: <code>rl_target_pct</code> contract only
(1.20 → 1.18 / 1.15). Freeze: dip=1.055, expansion=1.163, stop=0.934, trails off, post-TARGET bars=0.
Not gold / not DailyRun. IS = entry &lt; 2024-01-01; OOS report-only. Click column headers to sort.</p>
</header>
<main>
<div class="callout">
<strong>Evidence:</strong> ImprovePriority <code>260828112205</code> <em>slow_target_grind</em>
(190 TARGET holds ≥100d). Expand 1.25/1.30 already <strong>DISMISS</strong>.
Hypothesis: a closer SMA50 target recycles capital (higher AnnROR, fewer hold days)
without collapsing trade quality (Avg%, WO_MAX, PF, WR). Pre-agreed:
<strong>1.18</strong> and <strong>1.15</strong> vs control <strong>1.20</strong>.
Do not move expansion. Do not A/B partial / time-stop on this stamp.
KEEP only if quality holds <em>and</em> recycling improves.
<ul>{"".join(v_lis)}</ul>
...
<p class="muted">Host Max DD = EquityMeta passive. Overlay Max DD ≠ host account DD. Click column headers to sort.</p>
...
{sortable_th("Avg days (FULL)", "num")}{sortable_th("Host Max DD%", "num")}
...
def write_docs(packed: list[dict[str, Any]], verdicts: dict[str, dict[str, tuple[str, str]]]) -> None:
    by_id = {p["arm"]["id"]: p for p in packed}
    hyp = f"""# HYPOTHESIS — RL target contract (EXIT, turnover)

| Field | Fill in |
|-------|---------|
| System / prefix | RL |
| Baseline stamp | tradable Closed `260828112205` (`rl_target_pct=1.20`) |
| Universe | tradable 2010 / ADV$2m (764) |
| **Evidence** | ImprovePriority `260828112205` **slow_target_grind**: 190 TARGET holds ≥100d; tension contract 21% of TARGETs. Expand 1.25/1.30 DISMISS |
| **Hypothesis** | A closer SMA50 target recycles capital (AnnROR / hold days) without collapsing IS quality (Avg%, WO_MAX, PF, WR) |
| **Single knob** | `rl_target_pct` contract only (EXIT) |
| Frozen settings | dip=1.055, expansion=1.163, stop=0.934, trails off, `rl_post_target_reentry_bars=0` |
| Alternatives | control **1.20**; **1.18**; **1.15**. Not 1.25/1.30. Not expansion / partial / time-stop on this stamp |
| **Decision** | (fill after compare) |

OOS report-only. Do not retune. Research-only ≠ gold ≠ DailyRun.
KEEP only if quality holds AND recycling improves.
"""
    ...
    lines = [
        f"# BASELINE — `rl_target_contract_tradable_{STAMP}`",
        "",
        "**Status:** RESEARCH only. Not gold. Not DailyRun. One EXIT knob (target contract).",
        "",
        "| Field | Value |",
        "|---|---|",
        "| Universe | tradable 2010 / ADV$2m (764) |",
        "| Control | reuse `RL_Closed_260828112205` (`rl_target_pct=1.20`) |",
        "| Candidates | 1.18; 1.15 |",
        "| Frozen | dip=1.055, expansion=1.163, stop=0.934, trails off, post-TARGET bars=0 |",
        "| Split | IS entry < 2024-01-01; OOS report-only |",
        "",
        "Do **not** retune on OOS. Do **not** pile on partial / time-stop in this stamp.",
        "Do **not** overwrite `RL_universe.csv`. Judge quality first; AnnROR is the mechanism, not a KEEP-alone.",
        "",
    ...
    slines = [
        f"# SUMMARY — `rl_target_contract_tradable_{STAMP}`",
        "",
        "Target contract (`rl_target_pct` 1.18 / 1.15 vs 1.20) on tradable 764. EXIT turnover. Research only.",
        "",
        "## IS",
        "",
    ]
...
def summarize(packed: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    print(f"[RL-TC] Wrote {OUT_DIR / 'compare.html'}", flush=True)
...
def main() -> int:
    ...
    if not trad:
        print("[RL-TC] Missing tradable universe", flush=True)
        return 1
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    arms = [
        {
            "id": "control",
            "label": "Control (target=1.20)",
            "role": "control",
            "symbols": trad,
            "extra_v": ["rl_target_pct=1.2"],
            "live": False,
        },
        {
            "id": "tgt118",
    ...
    if args.summarize_only:
        for arm in arms:
            ...
            if not closed:
                print(f"[RL-TC] Missing Closed for {arm['id']}", flush=True)
                return 1
            trades = load_trades(closed)
            runs.append(
                {
        ...
        print(
            f"[RL-TC] control reuse ok={ctrl_run['ok']} n={len(ctrl_run['trades'])} stamp={ctrl_run.get('stamp')}",
            flush=True,
        )
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as ex:
            ...
            for fut in as_completed(futs):
                ...
                run = fut.result()
                print(
                    f"[RL-TC] {arm['id']} ok={run['ok']} n={len(run['trades'])} "
    ...
    if not all(r.get("ok") for r in runs):
        print("[RL-TC] One or more arms failed", flush=True)
        for r in runs:
    ...
    if ntfy.is_file():
        a = next(p for p in packed if p["arm"]["id"] == "tgt118")
        b = next(p for p in packed if p["arm"]["id"] == "tgt115")
        subprocess.run(
            [
                py,
                str(ntfy),
                ...
                "-t",
                "RL target contract tradable",
                "-m",
                (
                    f"1.18 IS {result['verdicts']['tgt118']['is'][0]} "
                    f"Avg={a['m_is']['avg_pnl']:.2f}% AnnROR={a['m_is']['ann_ror']:.1f} · "
                    f"1.15 IS {result['verdicts']['tgt115']['is'][0]} "
