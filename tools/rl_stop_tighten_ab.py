#!/usr/bin/env python3
"""RL stop-tighten A/B on tradable 764 tape (EXIT).

Evidence: ImprovePriority 260828112205 fat_stops (139 sym / 205 trades):
large STOP losses dominate. Tension card also had 306/1242 STOPs (25%)
never-worked (MFE<2%) as the contract side of stop_pct_tension.

One EXIT knob: raise `rl_stop_pct` only (tighter vs signal low).
Do not re-run expand (0.92 / 0.90 already DISMISS on AnnROR).
Do not pile on time-stop / cut_the_losers on this stamp.
Do not reuse tools/rl_stop_ab.py (old dip 1.041, house 59, multi-arm grid).

Control: 0.934 (~6.6% below signal low) reuse tradable Closed 260828112205.
...
Usage:
  python tools/rl_stop_tighten_ab.py
  python tools/rl_stop_tighten_ab.py --summarize-only
"""
...
DRIVE = ROOT / "drive"
STAMP = "20260828"
OUT_DIR = DRIVE / "paul_experiments" / f"rl_stop_tighten_tradable_{STAMP}"
CONTROL_SRC = (
...
ARM_ORDER = {"control": 0, "stop0940": 1, "stop0945": 2}
...
def _decision(verdicts: dict[str, dict[str, tuple[str, str]]]) -> str:
    vis_a, vis_b = verdicts["stop0940"]["is"][0], verdicts["stop0945"]["is"][0]
    voos_a, voos_b = verdicts["stop0940"]["oos"][0], verdicts["stop0945"]["oos"][0]
    keepish = {"KEEP", "LEAN KEEP"}
    if vis_a not in keepish and vis_b not in keepish:
        return (
            f"**DISMISS** both. IS 0.940={vis_a}; 0.945={vis_b}. "
            f"OOS report-only 0.940={voos_a}; 0.945={voos_b}. "
            "Do not search more stop values. Do not pile on time-stop on this stamp."
        )
    if (vis_a in keepish and voos_a == "DISMISS") or (vis_b in keepish and voos_b == "DISMISS"):
        return (
            f"**HOLD**. IS quality looked better (0.940: {vis_a}; 0.945: {vis_b}) but OOS softened "
            f"(0.940: {voos_a}; 0.945: {voos_b}). Do not retune OOS. Research-only."
        )
    return (
        f"IS 0.940={vis_a}; 0.945={vis_b}. OOS report-only 0.940={voos_a}; 0.945={voos_b}. "
        "Research candidate ≠ gold ≠ DailyRun. Judge quality over N; also read IS AnnROR and host DD."
    )
...
def write_compare_html(
    ...
    for split_key, title in (("m_is", "IS"), ("m_oos", "OOS (report-only)"), ("m_full", "FULL book")):
        ...
        sections.append(
            f'<section><h2>Stop tighten — {title}</h2>'
            f'<p class="muted">Δ vs control (rl_stop_pct=0.934, ~6.6% below signal low). {note} Click column headers to sort.</p>'
    ...
    pairs = [
        ("0.940 − control", by_id[CONTROL_ID], by_id["stop0940"]),
        ("0.945 − control", by_id[CONTROL_ID], by_id["stop0945"]),
        ("0.945 − 0.940", by_id["stop0940"], by_id["stop0945"]),
    ]
    pw_sections = []
...
<title>RL stop tighten A/B — tradable {STAMP}</title>
<style>
...
<body>
<header>
<h1>RL stop tighten — tradable 764 (EXIT)</h1>
<p class="muted">Stamp <code>rl_stop_tighten_tradable_{STAMP}</code>. One knob: <code>rl_stop_pct</code> tighten only
(0.934 → 0.940 / 0.945). Freeze: dip=1.055, expansion=1.163, target=1.20, trails off, post-TARGET bars=0.
Not gold / not DailyRun. IS = entry &lt; 2024-01-01; OOS report-only. Click column headers to sort.</p>
</header>
<main>
<div class="callout">
<strong>Evidence:</strong> ImprovePriority <code>260828112205</code> <em>fat_stops</em>
(139 sym / 205 trades) — large STOP losses dominate. Expand (0.92 / 0.90) already
<strong>DISMISS</strong> on IS AnnROR. Hypothesis: a tighter stop (higher multiplier vs signal low)
cuts fat losses and lifts IS quality (Avg%, WO_MAX, PF, WR, AnnROR) without collapsing N.
Pre-agreed: <strong>0.940</strong> (~6.0% below signal low) and <strong>0.945</strong> (~5.5%)
vs control <strong>0.934</strong> (~6.6%). Do not A/B time-stop on this stamp.
<ul>{"".join(v_lis)}</ul>
...
def write_docs(packed: list[dict[str, Any]], verdicts: dict[str, dict[str, tuple[str, str]]]) -> None:
    by_id = {p["arm"]["id"]: p for p in packed}
    hyp = f"""# HYPOTHESIS — RL stop tighten (EXIT)
...
| **Evidence** | ImprovePriority `260828112205` **fat_stops**: 139 sym / 205 trades; tension contract side 306/1242 STOPs never-worked. Expand 0.92/0.90 already DISMISS on AnnROR |
| **Hypothesis** | A tighter stop (higher `rl_stop_pct`) cuts fat STOP losses and lifts IS quality (Avg%, WO_MAX, PF, WR, AnnROR) without collapsing N |
| **Single knob** | `rl_stop_pct` tighten only (EXIT) |
| Frozen settings | dip=1.055, expansion=1.163, target=1.20, trails off, `rl_post_target_reentry_bars=0` |
| Alternatives | control **0.934** (~6.6%); **0.940** (~6.0%); **0.945** (~5.5%). Not 0.92/0.90. Not time-stop on this stamp |
...
"""
    ...
    lines = [
        f"# BASELINE — `rl_stop_tighten_tradable_{STAMP}`",
        "",
        "**Status:** RESEARCH only. Not gold. Not DailyRun. One EXIT knob (stop tighten).",
        "",
        "| Field | Value |",
        "|---|---|",
        "| Universe | tradable 2010 / ADV$2m (764) |",
        "| Control | reuse `RL_Closed_260828112205` (`rl_stop_pct=0.934`, ~6.6%) |",
        "| Candidates | 0.940 (~6.0%); 0.945 (~5.5%) |",
        "| Frozen | dip=1.055, expansion=1.163, target=1.20, trails off, post-TARGET bars=0 |",
        "| Split | IS entry < 2024-01-01; OOS report-only |",
        "",
        "Do **not** retune on OOS. Do **not** pile on time-stop in this stamp.",
        "Do **not** overwrite `RL_universe.csv`. Do **not** reuse `tools/rl_stop_ab.py`.",
        "",
    ...
    slines = [
        f"# SUMMARY — `rl_stop_tighten_tradable_{STAMP}`",
        "",
        "Stop tighten (`rl_stop_pct` 0.940 / 0.945 vs 0.934) on tradable 764. EXIT. Research only.",
        "",
        "## IS",
        "",
    ]
...
def summarize(packed: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    print(f"[RL-ST] Wrote {OUT_DIR / 'compare.html'}", flush=True)
...
def main() -> int:
    ...
    if not trad:
        print("[RL-ST] Missing tradable universe", flush=True)
        return 1
    ...
    if args.summarize_only:
        for arm in arms:
            ...
            if not closed:
                print(f"[RL-ST] Missing Closed for {arm['id']}", flush=True)
                return 1
            trades = load_trades(closed)
            runs.append(
                {
        ...
        print(
            f"[RL-ST] control reuse ok={ctrl_run['ok']} n={len(ctrl_run['trades'])} stamp={ctrl_run.get('stamp')}",
            flush=True,
        )
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as ex:
            ...
            for fut in as_completed(futs):
                ...
                run = fut.result()
                print(
                    f"[RL-ST] {arm['id']} ok={run['ok']} n={len(run['trades'])} "
    ...
    if not all(r.get("ok") for r in runs):
        print("[RL-ST] One or more arms failed", flush=True)
        for r in runs:
    ...
    if ntfy.is_file():
        a = next(p for p in packed if p["arm"]["id"] == "stop0940")
        b = next(p for p in packed if p["arm"]["id"] == "stop0945")
        subprocess.run(
            [
                py,
                str(ntfy),
                ...
                "-t",
                "RL stop tighten tradable",
                "-m",
                (
                    f"0.940 IS {result['verdicts']['stop0940']['is'][0]} "
                    f"Avg={a['m_is']['avg_pnl']:.2f}% AnnROR={a['m_is']['ann_ror']:.1f} · "
                    f"0.945 IS {result['verdicts']['stop0945']['is'][0]} "
