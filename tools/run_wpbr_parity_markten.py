#!/usr/bin/env python3
"""Compare WPBR zone/BO/entry output to TOS reference tables (tos/gen_*_ts.py)."""
...
def run_wpbr(sym: str, *, band_pct: float, breakout_confirmation: float, min_date: str) -> tuple[dict, pd.DataFrame]:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="WPBR parity vs TOS gen_*_ts reference")
    ...
    for sym in args.symbols:
        ...
        try:
            ...
            continue
        out, df = run_wpbr(sym, band_pct=args.band_pct, breakout_confirmation=bc, min_date=args.min_date)
        wpbr_map = {ev["pivot_monday"].replace("-", ""): ev for ev in out["wpbr_zone_events"]}

        zone_ok = 0
        bo_ok = 0
        for pivot, lo, hi, bo in ref_z:
            ev = wpbr_map.get(str(pivot))
            if not ev:
        ...
        signals = sorted(int(df.index[b].strftime("%Y%m%d")) for b in out.get("wpbr_entry_signal_bars") or [])
        fills = sorted(int(df.index[b].strftime("%Y%m%d")) for b in out.get("wpbr_entry_fill_bars") or [])
        ...
        print(
            ...
            f"(wpbr zones since {args.min_date}: {len(out['wpbr_zone_events'])}, bc={bc})"
        )
        if entry_hit != ref_e:
