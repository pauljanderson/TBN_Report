#!/usr/bin/env python3
"""Post-run (no charts, no Yahoo) on rsi80+atr5 CONTROL Closed book."""
...
OUT = ROOT / "drive" / "paul_experiments" / "rsi_ob_neutral_rsi80_atr5_20260910"
...
LAYMAN = (
    "This is the after-the-run checkup on the new baseline that freezes exit RSI≥80 "
    "plus ATR%≥5 (promoting the prior ENTRY_min_atr5 lean into the control). "
...
def _prepend_ask(path: Path) -> None:
    ...
    block = (
        ...
        "control = prior lean ENTRY_min_atr5 from "
        "<code>rsi_ob_neutral_rsi80_atr4_20260910</code> (on top of rsi80+atr4), "
        "chosen in-sample then re-scored under freeze. "
