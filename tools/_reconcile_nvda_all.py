"""Reconcile NVDA BRT sheet (transcript paste) vs engine stamp 260720194240.

Layers: zones (+/- $0.02), breakouts/retests, closed trades (+/- $0.05).
Also accepts optional stamps via --zones-stamp / --bo-stamp / --trades-stamp.
"""
...
NVDA_CSV = ROOT / "data" / "newdata" / "data" / "NVDA.csv"
...
def extract_paste() -> str:
    """Prefer the latest full NVDA paste (OHLC + zones + BOs + trades)."""
    latest = None
    for line in TRANSCRIPT.read_text(encoding="utf-8").splitlines():
        ...
        is_nvda = (
            "NVDA\nDate\tOpen" in blob
            or "NVDA\r\nDate\tOpen" in blob
            or ("1/4/2010\t$0.4600" in blob and has_sections)
        )
        if has_sections and is_nvda:
            latest = blob
    if not latest:
        raise SystemExit("NVDA paste not found in transcript")
    if "<user_query>" in latest:
        latest = latest[latest.find("<user_query>") + len("<user_query>") :]
    if "</user_query>" in latest:
        latest = latest[: latest.find("</user_query>")]
    latest = latest.strip()
    if latest.startswith("NVDA\n") or latest.startswith("NVDA\r\n"):
        first, _, rest = latest.partition("\n")
        if first.strip() == "NVDA" and rest:
            latest = rest
...
def ensure_exports(text: str):
    ...
    (TOOLS / "nvda_brt_sheet_zones.tsv").write_text(zones.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "nvda_brt_sheet_breakouts.tsv").write_text(bos.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "nvda_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")
    (OUT / "nvda_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")

    zero_dates = []
    with (OUT / "NVDA_sheet_ohlc.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "NVDA_sheet_zones.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "NVDA_sheet_breakouts.csv").open("w", newline="", encoding="utf-8") as f:
        if bo_rows:
    ...
    with (OUT / "NVDA_sheet_trades.csv").open("w", newline="", encoding="utf-8") as f:
        if tr_rows:
...
def reconcile_zones(unique_sheet, stamp: str):
    eng_path = ROOT / "drive" / f"BRT_ZONES_NVDA_{stamp}.csv"
    rows = []
    ...
    with (OUT / "NVDA_zones_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        f"# NVDA zone reconcile (sheet paste 2026-07-20)",
        "",
        f"- Engine: `BRT_ZONES_NVDA_{stamp}.csv`",
        "- Sheet: `tools/nvda_brt_sheet_zones.tsv` / `NVDA_sheet_zones.csv`",
    ...
    (OUT / "NVDA_zones_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
...
def reconcile_breakouts(bo_rows, stamp: str):
    ...
    with (OUT / "NVDA_breakouts_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# NVDA breakout/retest reconcile",
        "",
        "- Sheet TSV: `tools/nvda_brt_sheet_breakouts.tsv`",
        f"- Engine: `BRT_breakout_and_retest_{stamp}.csv` (NVDA filter)",
        f"- Window: {WIN_START} .. {WIN_END}",
        ...
        f"- Engine BO (NVDA total): **{len(eng_all)}**",
        f"- Engine BO (NVDA in window): **{len(eng_bo)}**",
    ...
    (OUT / "NVDA_breakouts_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "sheet": len(sheet_bo),
        "engine": len(eng_bo),
        "exact": len(exact),
...
def reconcile_trades(tr_rows, stamp: str):
    ...
    with (OUT / "NVDA_trades_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# NVDA BRT trades reconcile — sheet vs engine",
        "",
        f"- Sheet: `tools/nvda_brt_sheet_trades.tsv` ({len(load_sheet_trades(tr_rows))} closed rows pasted)",
        ...
        f"- Engine NVDA trades in window: **{len(eng_tr)}**",
        "",
        "## Match summary",
    ...
    if len(eng_tr) == 0:
        note = (
            f"**NOTE:** Preferred multi-symbol stamp `{STAMP}` has **0** NVDA closed trades "
            f"(zones/BOs present). This trades layer used stamp `{stamp}`. "
            "Root cause hypothesis: post-2026-07-19 multi-symbol runs drop NVDA (and ~12 other symbols) "
            "from closed/open outputs despite writing zones/breakouts — likely portfolio/resume path "
            "or symbol-loop skip; NVDA-only re-run recommended for trade parity."
        )
        lines += ["## Engine availability note", "", note, ""]

    (OUT / "NVDA_trades_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "stamp": stamp,
        "sheet": len(sheet_tr),
        "sheet_after": len(sheet_after),
...
def write_summary(exports, z, b, t, zero_dates, zones_stamp: str, bo_stamp: str, ohlc_check: dict):
    lines = [
        "# NVDA BRT sheet vs engine — reconcile summary",
        "",
        f"- Sheet paste: transcript `f301f0a6-...` user message 2026-07-20 18:15 (NVDA OHLC + zones + BOs + trades)",
        ...
        f"- Engine NVDA.csv: present, trading-day-only (0 zero-OHLC bars)",
    ...
    if t["engine"] == 0:
        lines.append(
            f"- **Trades:** stamp has **0** NVDA closed trades despite BOs. "
            "Often `max_market_cap` default wipes NVDA when yfinance leaves market_cap=None — use `max_market_cap=0` or checkpoint rewrite. "
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `NVDA_sheet_ohlc.csv`, `NVDA_sheet_zones.csv`, `NVDA_sheet_breakouts.csv`, `NVDA_sheet_trades.csv`",
        "- `NVDA_zones_diff.md`, `NVDA_breakouts_diff.md`, `NVDA_trades_diff.md`",
        "- Match detail CSVs: `NVDA_*_match_detail.csv`",
        "- Four-scenario cut-paste: `NVDA_four_scenario_stats.md`",
        "",
    ]
    (OUT / "NVDA_reconcile_summary.md").write_text("\n".join(lines), encoding="utf-8")
    print("\n".join(lines))


def check_critical_bars():
    """Spot-check a few sheet OHLC bars vs engine NVDA.csv."""
    want_dates = ["2010-01-04", "2013-07-11", "2016-05-19", "2022-08-26", "2024-06-18"]
    sheet = {}
    with (OUT / "NVDA_sheet_ohlc.csv").open(encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            d = r.get("date")
            if d in want_dates:
                sheet[d] = (
    ...
    eng = {}
    if NVDA_CSV.exists():
        with NVDA_CSV.open(encoding="utf-8", newline="") as f:
            for r in csv.DictReader(f):
...
def pick_trades_stamp(cli_stamp: str | None) -> str:
    if cli_stamp:
        return cli_stamp
    # Prefer preferred stamp if it has NVDA trades; else newest prior stamp with NVDA.
    ...
    print(f"[trades] preferred {STAMP} has 0 NVDA; using {cands[0][0]} ({cands[0][1]} trades)")
    return cands[0][0]
...
def main():
    ...
    tstamp = pick_trades_stamp(args.trades_stamp or STAMP)
