#!/usr/bin/env python3
"""WPBR MarkTen reconcile: variant C + SC ON + stop_pct=0.91 + start_date=2016-01-01."""
...
STAMP_DIR = REPO / "drive" / "wpbr_sheet_reconcile" / "_markten_variantC_SC_stop91_2016_20260722151842"
STAMP = "260722151857"
...
def nflx_focus(r: dict) -> dict:
    ...
    if may13["in_sheet"] and may13["in_ser"]:
        may13_status = (
            "RESOLVED (ser) â€” sheet 2022-05-13 is in SC-on closed trades "
        ...
        may13_status = "RAW ONLY â€” still not serialized"
    elif may13["in_sheet"] and not may13["in_raw"] and not may13["in_ser"]:
        may13_status = "STILL ORPHAN â€” not in raw fills and not in ser"
    else:
        may13_status = "N/A (not in current sheet paste)"

    if oct16["in_sheet"] and oct16["in_ser"]:
        oct_status = "RESOLVED â€” sheet entry 2023-10-16 matches eng ser"
    elif oct16["in_sheet"] and oct17["in_ser"] and not oct16["in_ser"]:
        oct_status = (
            "TIMING GAP REMAINS â€” sheet entry 2023-10-16 vs eng fill/ser 2023-10-17 "
...
def write_ticker_status(r: dict, eng: dict) -> Path:
    ...
    lines: list[str] = []
    lines.append(f"# {sym} WPBR reconcile â€” variant C + SC-on (`{STAMP}`)")
    lines.append("")
    lines.append(
        f"**Engine:** `drive/wpbr_sheet_reconcile/_markten_variantC_SC_stop91_2016_20260722151842/` "
        f"(`{STAMP}`)"
    )
    lines.append(
    ...
    lines.append(f"| Sheet trades â‰¥2016 | {r['n_sheet_trades']} |")
    lines.append("")
    if r["raw_orphans"]:
    ...
    if fair["n_eng_only"]:
        ...
        lines.append("")
    lines.append(
        f"*Generated by `tools/_variantC_SC_stop91_2016_wpbr_reconcile.py` vs stamp `{STAMP}`.*"
    )
    lines.append("")
...
def write_status(results: list[dict], eng: dict, nflx: dict) -> Path:
    out = BASE / "VARIANT_C_SC_STOP91_2016_MARKTEN_STATUS.md"
    lines: list[str] = []
    lines.append("# WPBR MarkTen â€” variant C + SC-on + stop_pct=0.91 + start_date=2016-01-01")
    lines.append("")
    lines.append(
        f"**Engine outdir:** `drive/wpbr_sheet_reconcile/_markten_variantC_SC_stop91_2016_20260722151842/`"
    )
    ...
    lines.append(
        ...
        "`start_date=2016-01-01`, `target_pct=1.22`, `stop_pct=0.91`, WPBR-only, growth off, "
    ...
    lines.append(
        ...
        f"(doc: **{eng['doc_variant_C']}**); sample â†’ "
        f"{s['tp']}/{s['zl']}/{s['zh']} (expected {e['tp']}/{e['zl']}/{e['zh']}) â†’ "
    ...
    for sym in MARKTEN:
        if sym not in by:
            lines.append(
                f"| {sym} | â€” | â€” | â€” | â€” | â€” | â€” | â€” | **NEED fresh breakout+trade paste** |"
            )
            continue
        r = by[sym]
        fair = r["fair"]
    ...
    for sym in MARKTEN:
        ...
        if r["raw_orphans"] or r["ser_orphans"]:
            residual_rows.append(
                f"- **{sym}:** raw orphans `{', '.join(r['raw_orphans']) or 'â€”'}`; "
                f"ser orphans `{', '.join(r['ser_orphans']) or 'â€”'}`"
            )
    if residual_rows:
    ...
    for sym in MARKTEN:
        if sym in by:
            ...
            lines.append(
                f"| {sym} | **NEED breakout+trade repaste** (old paste present â€” replace; OHLC/weekly skip OK) |"
            )
    lines.append("")
    if NEED:
        lines.append(f"**Still need pastes from user:** {', '.join(NEED)}")
    else:
        lines.append("**Still need pastes from user:** _(none â€” all MarkTen pastes reconciled)_")
    lines.append("")
    ...
    lines.append(
        ...
        f"(`_markten_variantC_SC_stop91_2016_20260722151842`). Do **not** turn second chance off."
    )
    lines.append("")
    lines.append("Helper: `python tools/_variantC_SC_stop91_2016_wpbr_reconcile.py`")
    lines.append("")
    lines.append("## Artifacts")
    lines.append("")
    lines.append(
        f"- Engine: `drive/wpbr_sheet_reconcile/_markten_variantC_SC_stop91_2016_20260722151842/` (`{STAMP}`)"
    )
    lines.append("- Status: `drive/wpbr_sheet_reconcile/VARIANT_C_SC_STOP91_2016_MARKTEN_STATUS.md`")
    lines.append("- Payload: `drive/wpbr_sheet_reconcile/_variantC_SC_stop91_2016_reconcile_payload.json`")
    lines.append("- Script: `tools/_variantC_SC_stop91_2016_wpbr_reconcile.py`")
    lines.append("")
    out.write_text("\n".join(lines), encoding="utf-8")
    return out
...
def main() -> int:
    ...
    for r in payload["results"]:
        ...
        r["fair"] = fair
        # drop bulky raw_fills from payload copy already in results â€” trim
    for r in payload["results"]:
        if "raw_fills" in r:
