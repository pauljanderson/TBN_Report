#!/usr/bin/env python3
"""Load WPBR spreadsheet ground-truth zones and trades from tab-separated paste files."""
...
PBR_HDR = "Break out upper"  # spreadsheet column header (literal paste text)
WPBR_HDR = PBR_HDR
...
def parse_wpbr_paste(text: str) -> dict[str, PbrSymbolGroundTruth]:
...
def load_wpbr_ground_truth(path: Path | None = None) -> dict[str, PbrSymbolGroundTruth]:
    path = path or (TOOLS / "wpbr_sheet_paste.txt")
    if not path.is_file():
        path = TOOLS / "wpbr_meta_msft_paste.txt"
    return parse_wpbr_paste(path.read_text(encoding="utf-8-sig"))


PBR_SHEET_GROUND_TRUTH: dict[str, Path] = {
    "AAPL": TOOLS / "wpbr_sheet_paste.txt",
    "AMZN": TOOLS / "wpbr_sheet_paste.txt",
    "META": TOOLS / "wpbr_sheet_paste.txt",
    "MSFT": TOOLS / "wpbr_sheet_paste.txt",
}
WPBR_SHEET_GROUND_TRUTH = PBR_SHEET_GROUND_TRUTH
