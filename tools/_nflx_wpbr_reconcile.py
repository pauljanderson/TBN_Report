#!/usr/bin/env python3
"""NFLX WPBR sheet vs engine stamp 260722105625 (retest_mode=stop_looking)."""
...
ENG_ZONES = ENG_DIR / f"WPBR_ZONES_NFLX_{STAMP}.csv"
...
def main() -> int:
    ...
    closed = closed[closed["SYMBOL"] == "NFLX"].reset_index(drop=True)
    eng_trades = []
    ...
    P("# NFLX WPBR Sheet <-> Engine Reconcile")
    P("")
    ...
    P(f"**Artifacts:** `drive/wpbr_sheet_reconcile/_markten_retest_2016/` "
      f"(incl. `WPBR_ZONES_NFLX_{STAMP}.csv`, `WPBR_Closed_{STAMP}.csv`).")
    P("**Sheet source:** user paste recovered from parent transcript "
      f"(saved under `drive/wpbr_sheet_reconcile/NFLX/`).")
    P("")
    P("| Table | Rows | File |")
    P("|---|---|---|")
    ...
    P(f"| Engine zones (≥{MIN_DATE}) | **{len(eng)}** | `WPBR_ZONES_NFLX_{STAMP}.csv` + live recompute |")
    ...
    P(f"| Engine serialized closed trades (NFLX) | **{len(eng_trades)}** |")
    P("")
    P("### Sheet trades")
    P("")
    ...
    P("")
    P(f"**NFLX engine files for stamp `{STAMP}`:** present "
      f"(`WPBR_ZONES_NFLX_{STAMP}.csv`, `WPBR_ZONES_ENTRIES_NFLX_{STAMP}.csv`, "
      f"NFLX rows in `WPBR_Closed_{STAMP}.csv`).")
    P("")
    P("---")
    P(f"*Generated by `tools/_nflx_wpbr_reconcile.py` against stamp `{STAMP}`.*")

    report = "\n".join(lines) + "\n"
    status = OUT / "NFLX_wpbr_reconcile_status.md"
