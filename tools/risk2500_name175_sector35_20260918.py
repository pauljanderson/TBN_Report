#!/usr/bin/env python3
"""17.5% name + 35% sector sell-winner rotate (research only).

Stamp: drive/paul_experiments/risk2500_name175_sector35_20260918/

Same live-style freeze as official 17.5% name (min(1% BOM, $50k) + 1% ADV20
+ 17.5% per-name + $7,500 + 10.5% + 2× + sell-winner on empty BP) plus one
knob: 35% per-sector notional. If a new buy would push sector S over 35%,
sell the winner in S, then buy. If still over, clip; skip if leftover
room < $1.

Controls: (1) 10% name + 25% sector DISMISS sibling; (2) 17.5% name, no sector
(official live-style name lid). Not gold. Not DailyRun. Do not retune on OOS.
"""
...
STAMP = "risk2500_name175_sector35_20260918"
OUT_DIR = REPO / "drive" / "paul_experiments" / STAMP
SIBLING_17 = REPO / "drive" / "paul_experiments" / "risk_1pct_50k_adv_17name_20260917"
SIBLING_1025 = REPO / "drive" / "paul_experiments" / "risk2500_name10_sector25_20260918"
...
# Official live-style 17.5% name, no sector (docs/live_style.html + compare).
PUB_17NAME = {
    "end": 58_700_020.87,
    "eq_2010": 284_646.36,
    "eq_2011": 392_517.79,
    ...
    "n_name_capped": 516,
    "n_skip_name": 1,
    "n_risk_50k_bound": 3575,
    "n_risk_1pct_bound": 2378,
    "n_adv_clipped": 3064,
    "tag": "ADOPT research / HOLD gold",
}

# Just-published DISMISS 10% name + 25% sector (Paul note; re-run pins exact).
PUB_10_25 = {
    "end": 5_849.0,
    "eq_2012": 195_000.0,
    "max_dd_pct": 99.9,
...
# Per-arm name lid. Sector frac lives on ARM_SPEC[1].
NAME_BY_ARM = {
    "name175": NAME_CAP,
    "sec1025": NAME_CAP_1025,
    "sec35": NAME_CAP,
}

ORIGINAL_REQUEST = "how about 17.5% and 35% sector?"
PLAIN_ENGLISH = (
    "Same $250,000 five-sleeve wallet as the just-dismissed 10% name + 25% "
    "sector book, except the two lids are looser: one ticker may hold 17.5% "
    "of the account (the official live-style name lid), and one Yahoo sector "
    "may hold 35%. Five sleeves — StockBee (SB), Relative Strength Index "
    "(RSI), Volume Zone (VZ), Magic Touch (MTS), Rocket Launcher (RL). "
    ...
    "daily volume (ADV20); one ticker may not hold more than 17.5% of whatever "
    "the account is worth right then (clip the new fill — do not sell a winner "
    "just because the name lid binds). One Yahoo sector (a GICS-like industry "
    "group — Global Industry Classification Standard is the official name; we "
    "use the checked-in Yahoo sector map, not official GICS codes) may not "
    "hold more than 35% of the account. If a new buy in sector S would go "
    "over 35%, sell the most profitable open lot already in S (last close "
    "versus entry), then take the new one. If you are still over 35% after "
    ...
    "the book-wide winner, then fill (same as the sibling stamps). Tickers "
    ...
    "tracker (SPY), dividends reinvested. We also show the dead 10%/25% book "
    "and the official 17.5% name-only book so you can see what the lid change "
    "and the sector rotate each did. Not gold. Not DailyRun."
)
...
ARM_SPEC: list[tuple[str, Optional[float], str, str, str]] = [
    (
        "name175",
        None,
        "5-sys · 1%+$50k · 1% ADV · 17.5% name (no sector cap)",
        "Official live-style name lid — published sibling risk_1pct_50k_adv_17name_20260917. Same freeze minus the 35% sector lid.",
    ...
    (
        ...
        "Just-published DISMISS — tighter lids. Re-run so this page shares one engine.",
        "ALL_overlay_5sys_1pct_50k_adv_10name_sector25.csv",
    ),
    (
        "sec35",
        0.35,
        "5-sys · 1%+$50k · 1% ADV · 17.5% name · 35% sector",
        "Candidate — 17.5% name plus 35% per-sector notional. Overflow → sell sector winner, then buy; clip leftover room.",
        "ALL_overlay_5sys_1pct_50k_adv_17name_sector35.csv",
    ),
]
...
def _verdict(
    ...
    name_only = _led_pack(books["name175"])
    dead = _led_pack(books["sec1025"])
    cand = _led_pack(books["sec35"])
    spy_asof = float(spy["tr_end"])
    spy_2012 = float(spy_dd["eq_2012"])
    changed = _sector_changed(cand, name_only)
    qw = _quality_worse(cand, name_only)
    ...
    extras: dict[str, Any] = {
        ...
        "ye2012_vs_name175": _rel_change(cand["eq_2012"], name_only["eq_2012"]),
        "asof_vs_name175": _rel_change(cand["end"], name_only["end"]),
        "ye2012_vs_1025": _rel_change(cand["eq_2012"], dead["eq_2012"]),
        "asof_vs_1025": _rel_change(cand["end"], dead["end"]),
        "pin_note": pin_note,
        "sector_meta": sector_meta,
    }
    lids = (
        f"Lids on the 17.5% name + 35% sector book — $50k bound fills {cand['n_risk_50k_bound']} "
        ...
        f"17.5% name-capped {cand['n_name_capped']} "
    ...
    path = (
        f"2010–2012 path — 17.5/35 {format_money(cand['eq_2010'])} / "
        f"{format_money(cand['eq_2011'])} / {format_money(cand['eq_2012'])}; "
        f"17.5% name-only {format_money(name_only['eq_2010'])} / "
        f"{format_money(name_only['eq_2011'])} / {format_money(name_only['eq_2012'])}; "
        f"10/25 DISMISS {format_money(dead['eq_2010'])} / "
        f"{format_money(dead['eq_2011'])} / {format_money(dead['eq_2012'])}; "
        f"SPY {format_money(spy['years'].get(2010, {}).get('tr', 0))} / "
        f"{format_money(spy['years'].get(2011, {}).get('tr', 0))} / "
        f"{format_money(spy_2012)}."
    )
    vs_name = (
        f"17.5/35 vs 17.5% name-only: year-end 2012 {format_money(cand['eq_2012'])} vs "
        f"{format_money(name_only['eq_2012'])} ({_rel_change(cand['eq_2012'], name_only['eq_2012']):+.1%}); "
        f"as-of {format_money(cand['end'])} vs {format_money(name_only['end'])} "
        f"({_rel_change(cand['end'], name_only['end']):+.1%}). "
        + (
            "The 35% sector lid + sector-winner rotate bound the book — it is not slack. "
            if changed
            else "The 35% sector lid barely moved the 17.5% name path. "
        )
    )
    vs_1025 = (
        f"17.5/35 vs 10/25 DISMISS: year-end 2012 {format_money(cand['eq_2012'])} vs "
        f"{format_money(dead['eq_2012'])} ({_rel_change(cand['eq_2012'], dead['eq_2012']):+.1%}); "
        f"as-of {format_money(cand['end'])} vs {format_money(dead['end'])} "
        f"({_rel_change(cand['end'], dead['end']):+.1%})."
    )
    vs_spy = (
        f"17.5/35 year-end 2012 {format_money(cand['eq_2012'])} vs SPY "
    ...
    if still_huge:
        cap_note = (
            ...
            "fiction once overlay dollars dwarf the seed. HOLD not adopt."
        )
    tail = (
        f"{lids} {vs_name} {vs_1025} {vs_spy} {path} {pin_note} Sector map coverage "
    ...
    if cand["died"] or not cand["wires_full"]:
        ...
        prose = (
            f"DISMISS 17.5% name + 35% sector versus SPY — {why} "
    ...
    if oos_soft:
        tag = "HOLD"
        prose = (
            f"HOLD versus gold / DailyRun and versus adopting 35% sector — "
    ...
    if still_huge:
        tag = "HOLD"
        prose = (
            f"HOLD versus gold / DailyRun — 17.5% name + 35% sector is in this book "
            f"and the $7,500 wires were paid, but as-of {format_money(cand['end'])} is "
            f"still capacity fiction (survives because 1% of a growing pile prints "
            f"huge dollars). vs SPY: year-end 2012 "
    ...
    if qw:
        tag = "HOLD"
        prose = (
            f"HOLD versus the 17.5% name-only sibling — quality softened and we "
            f"do not adopt 35% sector from this one page. {tail}"
        )
        return tag, prose, extras
    ...
    if (
        ...
        and cand["max_dd_pct"] <= name_only["max_dd_pct"] + 2.0
        and not qw
    ):
        tag = "KEEP"
        prose = (
            f"KEEP 17.5% name + 35% sector + sector-winner rotate as a research "
            f"candidate versus SPY "
    ...
    prose = (
        f"HOLD versus gold / DailyRun. 17.5% name + 35% sector is in this book. "
        f"vs SPY / 17.5% name-only / 10/25 the picture is mixed — do not adopt "
        f"from this one page. {tail}"
    )
    return tag, prose, extras
...
def _headline_box(extras: dict[str, Any], tag: str) -> str:
    ...
    cand = extras["sec35"]
    name_only = extras["name175"]
    dead = extras["sec1025"]
    died = "YES" if cand["died"] else "NO"
    return f"""
<div class="lead">
  <h2>Headline — 17.5% name + 35% sector, start $250k</h2>
  <p><strong class="{cls}">{html_mod.escape(tag)}</strong> · research only · not gold · not DailyRun</p>
  <p class="small">Same official live-style freeze as 17.5% name (no sector), plus a 35%
  Yahoo-sector lid. Overflow → sell the winner in that sector, then buy.
  If still over, clip. Buying-power sell-winner is a separate book-wide rule.
  10%/25% is the just-dismissed tighter pair.</p>
  <div class="cards">
    <div class="card card-shared">
      <h3>17.5/35 · year-end 2012 vs SPY</h3>
    ...
    <div class="card card-shared">
      <h3>17.5/35 · as-of {ASOF.isoformat()} vs SPY</h3>
    ...
    <div class="card">
      <h3>vs 10%/25% DISMISS</h3>
      <div class="metric">{format_money(cand['eq_2012'])}</div>
      <div class="small">10/25 YE2012 {format_money(dead['eq_2012'])} ·
      as-of {format_money(dead['end'])} · Max DD {_fmt_pct(dead['max_dd_pct'])}</div>
      <div class="small">YE2012 {extras['ye2012_vs_1025']:+.1%} ·
      as-of {extras['asof_vs_1025']:+.1%}</div>
    </div>
    <div class="card">
      <h3>vs 17.5% name-only</h3>
      <div class="metric">{"YES bound" if extras["sector_changed"] else "NO / slack"}</div>
      <div class="small">As-of {format_money(cand['end'])} vs
      {format_money(name_only['end'])} ({extras['asof_vs_name175']:+.1%})</div>
      <div class="small">YE2012 {format_money(cand['eq_2012'])} vs
      {format_money(name_only['eq_2012'])} ({extras['ye2012_vs_name175']:+.1%})</div>
    ...
    <div class="card">
      ...
      <div class="small">17.5% name-only {format_money(name_only['peak_name'])}
      {html_mod.escape(name_only['peak_name_symbol'])} · {name_only['peak_name_frac']:.1%}</div>
    </div>
    <div class="card">
      ...
      <div class="small">17.5% name-only (no sector lid) {format_money(name_only['peak_sector'])}
      {html_mod.escape(name_only['peak_sector_name'])} · {name_only['peak_sector_frac']:.1%}</div>
    </div>
    <div class="card">
...
def _lead_table(
    ...
    last = packs["sec35"]
    last_row = (
        '<tr class="total-row"><th>Pinned (17.5% name + 35% sector)</th>'
    ...
    return (
        ...
        "Dollar fields are uncapped ($nnn,nnn.nn). 17.5% name-only is official "
        "live-style (no sector). 10/25 is the DISMISS sibling. Dies = Max DD ≥ 90% or as-of &lt; 10% of "
...
def _lid_table(books: dict[str, dict[str, Any]]) -> str:
    ...
    last = _led_pack(books["sec35"])
    last_row = (
        '<tr class="total-row"><th>Pinned (17.5/35)</th>'
        f"<td>{last['n_fill']}</td>"
...
def _year_table(
    ...
    labels = [
        ("name175", "17.5% name-only"),
        ("sec1025", "10% name + 25% sector"),
        ("sec35", "17.5% name + 35% sector"),
    ]
    head = f._sortable_head(
        [("Year-end", "num")]
...
def _path_2010_2012(
    ...
    for key, lab in (
        ("name175", "17.5% name-only (official live-style)"),
        ("sec1025", "10% name + 25% sector (DISMISS)"),
        ("sec35", "17.5% name + 35% sector"),
    ):
        led = books[key]["led"]
        rows.append(
...
def _month_path_table(
    ...
    keys_order = ["name175", "sec1025", "sec35"]
    labs = {
        "name175": "17.5% name-only EOM",
        "sec1025": "10/25 DISMISS EOM",
        "sec35": "17.5/35 EOM",
    }
    head = f._sortable_head(
        [("Month", "month")]
...
def _compare_sections(books: dict[str, dict[str, Any]], verdict: str) -> str:
    ...
    notes = {
        ...
        "OOS": "Out-of-Sample — report-only. Do not pick 17.5% / 35% from this slice.",
        "FULL": verdict,
    }
    html = ""
    for sl in ("IS", "OOS", "FULL"):
        cols = [
            ("17.5% name-only (official)", books["name175"][col_map[sl]]),
            ("10% name + 25% sector", books["sec1025"][col_map[sl]]),
            ("17.5% name + 35% sector", books["sec35"][col_map[sl]]),
        ]
        html += f"""
<section>
<h2>Canonical compare · {sl}</h2>
<p class="small">{html_mod.escape(notes[sl])} Sheet / Total PnL $ omitted. Click headers to sort. Δ vs 17.5% name-only.</p>
...
<title>5-sys 17.5% name + 35% sector — {STAMP}</title>
<style>{f._css()}</style></head><body>
<h1>Monthly Backtest Report — {ASOF.year} · 17.5% name + 35% sector · start $250k</h1>
<p class="sub">
...
Official live-style 17.5% name-only is the one-knob sibling. 10%/25% is the DISMISS pair.
Out-of-Sample is report-only. Do not retune 17.5% or 35% on OOS.
</div>
<section>
...
Sibling 17.5% name (no sector):
<a href="../risk_1pct_50k_adv_17name_20260917/compare.html">risk_1pct_50k_adv_17name_20260917</a>.
10%/25% DISMISS:
<a href="../risk2500_name10_sector25_20260918/compare.html">risk2500_name10_sector25_20260918</a>.</p>
{_acronyms()}
</section>
...
def build_compare(
    *,
    ...
    return f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Compare — 17.5% name + 35% sector vs 17.5% name-only vs 10/25 vs SPY — {STAMP}</title>
<style>{f._css()}</style></head><body>
<h1>Compare — 17.5% name + 35% sector · start $250k</h1>
<p class="sub">
Stamp <code>{STAMP}</code>. Research only. <strong>Not gold. Not DailyRun.</strong>
...
def write_baseline(
    *,
    ...
    lines = [
        ...
        "## Selection",
        "",
        "- One stamp, one added knob-pair vs official live-style: **35% per-sector notional + sell the sector winner, then buy** on top of the published 17.5% name freeze (`risk_1pct_50k_adv_17name_20260917`).",
        "- Also shown: just-published DISMISS `risk2500_name10_sector25_20260918` (10% name + 25% sector) so Paul sees the lid change.",
        ...
        "- Others: shares = risk_dollar / (entry − stop), then ADV 1%, name 17.5% (clip), sector 35% (sell-winner then clip), then remaining buying power.",
        "- Name overflow: **clip** the new fill. Same-ticker open lots share the 17.5% room. Do not sell-winner just for name.",
        "- Sector overflow: **sell the winner in that sector** (most profitable open lot in S, last close vs entry), then buy. If still over 35% after one sale, clip to leftover sector room; skip if room < $1.",
        ...
        "- Name lid (candidate): **open notional ≤ 17.5% of current (cash + reserved) equity** after ADV, before sector / buying-power. Same-ticker lots share the room. CLIP, do not rotate.",
        "- Sector lid (candidate): **open notional ≤ 35% of current equity** after name. One sale of the sector winner, then clip leftover room.",
        ...
        f"- Unmapped: **{sector_meta['n_unknown']}** — each ticker is its own `UNKNOWN:{{TICKER}}` bucket (they do not share a 35% pile; we do not skip the fill for a missing sector).",
        ...
        "## Books",
        "",
        _book_bullet("17.5% name-only (official live-style, no sector)", books["name175"]),
        _book_bullet("10% name + 25% sector (DISMISS sibling)", books["sec1025"]),
        _book_bullet("17.5% name + 35% sector (candidate)", books["sec35"]),
        "",
        "## Headline vs SPY / 17.5% name-only / 10/25",
        "",
        f"- 17.5/35 year-end 2012: **${cand['eq_2012']:,.2f}** vs SPY **${extras['spy_2012']:,.2f}** ({_vs_spy(cand['eq_2012'], extras['spy_2012'])}).",
        f"- 17.5/35 as-of {ASOF.isoformat()}: **${cand['end']:,.2f}** vs SPY **${extras['spy_asof']:,.2f}** ({_vs_spy(cand['end'], extras['spy_asof'])}).",
        f"- vs 17.5% name-only YE2012 **${cand['eq_2012']:,.2f}** vs **${name_only['eq_2012']:,.2f}** ({extras['ye2012_vs_name175']:+.1%}); as-of **${cand['end']:,.2f}** vs **${name_only['end']:,.2f}** ({extras['asof_vs_name175']:+.1%}). Sector lid changed the path? **{'YES' if extras['sector_changed'] else 'NO'}**.",
        f"- vs 10/25 DISMISS YE2012 **${cand['eq_2012']:,.2f}** vs **${dead['eq_2012']:,.2f}** ({extras['ye2012_vs_1025']:+.1%}); as-of **${cand['end']:,.2f}** vs **${dead['end']:,.2f}** ({extras['asof_vs_1025']:+.1%}).",
        ...
        "## Honesty",
        "",
        "- This page is “how does THIS book perform over time vs SPY starting $250k.” 17.5% name-only is the one-knob sibling; 10/25 is the tighter DISMISS pair.",
        "- Judge 2010–2012 and whether the $7,500 wire survives — not later-year billions.",
        "- OOS is report-only. Do not retune 17.5% / 35% / $50k / ADV on OOS.",
        "- If the account survives only because 1% of a growing pile prints huge dollars, HOLD not adopt.",
...
**Product owner (PO)–aligned process:** one added knob (35% sector + sector-winner rotate) on the frozen official 17.5% name live-style book. Also shown: 10%/25% DISMISS pair. See `docs/HYPOTHESIS_TEST.md`.
...
| Baseline stamp | Official 17.5% name sibling `risk_1pct_50k_adv_17name_20260917`. DISMISS pair `risk2500_name10_sector25_20260918`. Wallet engine `risk2500_five_sys_20260917` |
| Universe | House DailyRun 5-sys. Indicators (IND) excluded |
| **Evidence** | Paul: “how about 17.5% and 35% sector?” after 10%/25% DISMISS; start $250k; $7,500/mo; 1% ADV20; min(1% risk, $50k); if a new buy would put a sector over 35%, sell the winner, then buy the new one |
| **Hypothesis** | Looser lids (17.5% name + 35% Yahoo-sector + sell sector winner, then buy; clip leftover) keep the account alive versus SPY at year-end 2012 / as-of and pay the $7,500 wire, unlike 10%/25% |
| **Single knob vs 17.5% name** | Sector cap on (35% + sector-winner rotate) vs off. Name 17.5%, min(1%,$50k), 1% ADV, $7,500, 10.5%, 2× already on both |
| Frozen settings | $250k start. $7,500 on the 1st. 10.5% actual/365. Open ≤ 2×. Risk min(1% BOM, $50k). ADV 1% of ADV20. Name 17.5% clip. RSI 6.51% IS freeze. BP sell-winner when empty |
| Alternatives | 17.5% name-only official live-style. 10%/25% DISMISS. SPY $250k total return |
| Candidate stamps | `{STAMP}` monthly.html / compare.html |
| Metrics | YE2012 vs SPY; as-of vs SPY; Max DD; dies?; vs 10/25; vs 17.5% name-only; peak name / peak sector; wires; sector-winner sales; wallet CAGR vs book Ann ROR; canonical IS/OOS (report-only) |
| **Trade-diff HTML** | N/A — size / wallet overlay (not an entry A/B) |
| ToS before path | N/A |
| ToS after path | N/A |
| **Decision** | {tag} — {verdict} |
| Reviewer | AI job risk2500_name175_sector35_20260918 |
| PO sign-off | no |
...
- [x] Evidence was the PO ask (17.5% name + 35% sector + sell sector winner + $250k + $7,500 + 1% ADV + min(1%,$50k))
- [x] One added knob versus the published 17.5% name sibling; 10/25 shown as the prior lid pair
- [x] Same Closed pins as the sibling $2,500 / live-style stamps
...
def _pin_match(name175_led: Any, sec1025_led: Any) -> str:
    bits: list[str] = []
    end_ok = abs(float(name175_led.end_equity) - PUB_17NAME["end"]) < 1.0
    ye_ok = abs(float(name175_led.eq_2012) - PUB_17NAME["eq_2012"]) < 1.0
    dd_ok = abs(float(name175_led.max_dd_pct) - PUB_17NAME["max_dd_pct"]) < 0.05
    if end_ok and ye_ok and dd_ok:
        bits.append(
            f"17.5% name-only re-run matched published {SIBLING_17.name} "
            f"(as-of {format_money(name175_led.end_equity)} / YE2012 "
            f"{format_money(name175_led.eq_2012)} / Max DD {_fmt_pct(name175_led.max_dd_pct)})."
        )
    else:
        bits.append(
            f"17.5% name-only re-run vs published {SIBLING_17.name}: as-of "
            f"{format_money(name175_led.end_equity)} vs {format_money(PUB_17NAME['end'])}; "
            f"YE2012 {format_money(name175_led.eq_2012)} vs {format_money(PUB_17NAME['eq_2012'])}; "
            f"Max DD {_fmt_pct(name175_led.max_dd_pct)} vs {_fmt_pct(PUB_17NAME['max_dd_pct'])}. "
            "This page uses the re-run so all arms share one engine."
        )
    dead_end = float(sec1025_led.end_equity)
    dead_ye = float(sec1025_led.eq_2012)
    dead_dd = float(sec1025_led.max_dd_pct)
    bits.append(
        f"10/25 DISMISS re-run: as-of {format_money(dead_end)} "
        f"(Paul note ~{format_money(PUB_10_25['end'])}); "
        f"YE2012 {format_money(dead_ye)} (Paul note ~{format_money(PUB_10_25['eq_2012'])}); "
        f"Max DD {_fmt_pct(dead_dd)} (Paul note ~{_fmt_pct(PUB_10_25['max_dd_pct'])})."
    )
    return " ".join(bits)
...
def run() -> int:
    ...
    sources = [
        ...
        "name_cap_frac=0.175 max_risk_dollar=50000 adv_frac=0.01 sector_cap_frac=0.35 "
        "(defaults unchanged — older stamps keep identity)",
        f"one-knob sibling: {SIBLING_17.name} 17.5% name, no sector (re-run)",
        f"lid-change sibling: {SIBLING_1025.name} 10% name + 25% sector DISMISS (re-run)",
        ...
        "sector lid: sell winner in S, then buy; if still over, clip leftover room < $1",
        "BP empty: book-wide sell-winner (separate from sector)",
        "official live-style 17.5% name is the no-sector control — not mutated",
        DD_DEF,
    ]
    ...
    pin_note = _pin_match(books["name175"]["led"], books["sec1025"]["led"])
    ...
    summary = {
        ...
        "years": YEARS,
        "name175": _jsonable(_led_pack(books["name175"])),
        "sec1025": _jsonable(_led_pack(books["sec1025"])),
        "sec35": _jsonable(_led_pack(books["sec35"])),
