#!/usr/bin/env python3
"""RL time-stop A/B on tradable 764 tape (EXIT, after profit gate).

Evidence: ImprovePriority 260828112205 fat_stops / slow_target_grind alts:
RL_EXIT_DAYS after a profit gate recycles sitters. Stop tighter/wider and
target contract already DISMISS (or HOLD-only on AnnROR with OOS veto).

One EXIT knob: lower `rl_exit_days` (bars after +29% MTM). Profit gate
`rl_exit_percent=0.29` stays frozen (engine default). Prod days=10000 ≈ off.

Control: 10000 (off) reuse tradable Closed 260828112205.
Candidates (pre-agreed, not a grid): 40 and 80 bars after +29%.

Do not move stop / target / partial / trails on this stamp.

Research-only. Not gold. Not DailyRun. Do not overwrite RL_universe.csv.

Usage:
  python tools/rl_time_stop_ab.py
  python tools/rl_time_stop_ab.py --summarize-only
"""
...
DRIVE = ROOT / "drive"
STAMP = "20260828"
OUT_DIR = DRIVE / "paul_experiments" / f"rl_time_stop_tradable_{STAMP}"
CONTROL_SRC = (
...
ARM_ORDER = {"control": 0, "days40": 1, "days80": 2}
...
def _timed_n(p: dict[str, Any]) -> int:
    return int((p["m_full"].get("exits") or {}).get("RL_EXIT_DAYS", 0))


def _decision(verdicts: dict[str, dict[str, tuple[str, str]]]) -> str:
    # PO 2026-08-28: CONSIDER 40d (AnnROR + host DD). House auto DISMISS on Avg.
    _ = verdicts
    return (
        "**CONSIDER 40d** (PO 2026-08-28, research candidate). House auto DISMISS on IS Avg/PF "
        "(4.42→3.54, PF 1.69→1.59). Under AnnROR + host DD: IS 16.4→21.0 and 17.7→7.8; "
        "OOS Avg flat 3.77→3.74, AnnROR 31.7→34.1. Picked 40 after seeing 40 vs 80 "
        "(in-sample selection). Freeze still `rl_exit_days=10000` until adopt. "
        "Not gold / not DailyRun. Do not search more day counts. Do not mix with target 1.18 on this stamp."
    )
...
def write_compare_html(
    ...
    for split_key, title in (("m_is", "IS"), ("m_oos", "OOS (report-only)"), ("m_full", "FULL book")):
        ...
        note = (
            ...
            "Turnover: also read Ann ROR / avg days."
        )
        sections.append(
            f'<section><h2>Time-stop — {title}</h2>'
            f'<p class="muted">Δ vs control (rl_exit_days=10000, off). Profit gate frozen at +29%. {note} Click column headers to sort.</p>'
    ...
    pairs = [
        ("40d − control", by_id[CONTROL_ID], by_id["days40"]),
        ("80d − control", by_id[CONTROL_ID], by_id["days80"]),
        ("80d − 40d", by_id["days40"], by_id["days80"]),
    ]
    pw_sections = []
    ...
    for aid in CANDIDATES:
        ...
        v_lis.append(
            ...
            f"RL_EXIT_DAYS rows={_timed_n(by_id[aid])}</li>"
        )
    html = f"""<!DOCTYPE html>
<html lang="en"><head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1"/>
<title>RL time-stop A/B — tradable {STAMP}</title>
<style>
...
<body>
<header>
<h1>RL time-stop — tradable 764 (EXIT, after +29%)</h1>
<p class="muted">Stamp <code>rl_time_stop_tradable_{STAMP}</code>. One knob: <code>rl_exit_days</code>
(10000 → 40 / 80) after profit gate <code>rl_exit_percent=0.29</code> frozen.
Freeze: dip=1.055, stop=0.934, target=1.20, trails off. Not gold / not DailyRun.
...
<em>fat_stops</em> alt: cut sitters after a profit print without moving SMA target.
Hypothesis: 40d or 80d after +29% MTM recycles capital (AnnROR / hold days / host DD)
without collapsing trade quality. Do not move the 29% gate. Do not pile on partial.
KEEP only if quality holds <em>and</em> recycling improves (same rule as target contract).
<ul>{"".join(v_lis)}</ul>
...
{sortable_th("RL_EXIT_DAYS", "num")}{sortable_th("N", "num")}
...
def write_docs(packed: list[dict[str, Any]], verdicts: dict[str, dict[str, tuple[str, str]]]) -> None:
    by_id = {p["arm"]["id"]: p for p in packed}
    hyp = f"""# HYPOTHESIS — RL time-stop after +29% (EXIT)

| Field | Fill in |
|-------|---------|
| System / prefix | RL |
| Baseline stamp | tradable Closed `260828112205` (`rl_exit_days=10000`, off) |
| Universe | tradable 2010 / ADV$2m (764) |
| **Evidence** | ImprovePriority `260828112205` **slow_target_grind** / **fat_stops** alt: RL_EXIT_DAYS after profit gate. Stop/target contract already DISMISS |
| **Hypothesis** | After +29% MTM, exit in 40 or 80 bars recycles sitters (AnnROR / hold days / host DD) without collapsing IS quality |
| **Single knob** | `rl_exit_days` (EXIT). Profit gate `rl_exit_percent=0.29` frozen |
| Frozen settings | dip=1.055, expansion=1.163, stop=0.934, target=1.20, trails off, bars=0, exit_percent=0.29 |
| Alternatives | control **10000** (off); **40**; **80**. Not 17 (RL100). Not a new profit-gate % |
...
"""
    ...
    lines = [
        f"# BASELINE — `rl_time_stop_tradable_{STAMP}`",
        "",
        "**Status:** RESEARCH only. One EXIT knob (time-stop after +29%).",
        "",
        "| Field | Value |",
        "|---|---|",
        "| Universe | tradable 2010 / ADV$2m (764) |",
        "| Control | reuse `RL_Closed_260828112205` (`rl_exit_days=10000`) |",
        "| Candidates | 40 bars; 80 bars after +29% |",
        "| Frozen | dip=1.055, stop=0.934, target=1.20, exit_percent=0.29, trails off |",
        "| Split | IS entry < 2024-01-01; OOS report-only |",
        "",
        "| Arm | Stamp | N_full | RL_EXIT_DAYS rows | Host Max DD% | OK |",
        "|-----|-------|--------|-------------------|---------------|-----|",
    ]
    for p in packed:
        lines.append(
            f"| `{p['arm']['id']}` | `{p.get('stamp','')}` | {p['m_full']['n']} | "
            f"{_timed_n(p)} | {_host_dd(p)} | {'yes' if p.get('ok') else 'no'} |"
        )
    ...
    slines = [
        f"# SUMMARY — `rl_time_stop_tradable_{STAMP}`",
        "",
        "Time-stop (`rl_exit_days` 40 / 80 vs 10000) after +29% on tradable 764. EXIT. Research only.",
        "",
        "## IS",
        "",
    ]
    ...
    for aid in ("control", *CANDIDATES):
        slines.append(f"- **{aid}**: {_host_dd(by_id[aid])}%  RL_EXIT_DAYS rows={_timed_n(by_id[aid])}")
...
def summarize(packed: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    timed = {k: _timed_n(by_id[k]) for k in ("control", *CANDIDATES)}
    print(f"[RL-TS] Wrote {OUT_DIR / 'compare.html'} timed={timed}", flush=True)
...
def main() -> int:
    ...
    if not trad:
        print("[RL-TS] Missing tradable universe", flush=True)
        return 1
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    arms = [
        {
            "id": "control",
            "label": "Control (exit_days=10000, off)",
            "role": "control",
            "symbols": trad,
            "extra_v": [],
            "live": False,
        ...
        {
            "id": "days80",
            "label": "80 bars after +29%",
            "role": "candidate",
            "symbols": trad,
            "extra_v": ["rl_exit_days=80"],
            "live": True,
        },
    ]
    py = _resolve_python()
    ...
    if args.summarize_only:
        for arm in arms:
            ...
            if not closed:
                print(f"[RL-TS] Missing Closed for {arm['id']}", flush=True)
                return 1
            trades = load_trades(closed)
            runs.append(
                {
        ...
        print(
            f"[RL-TS] control reuse ok={ctrl_run['ok']} n={len(ctrl_run['trades'])} stamp={ctrl_run.get('stamp')}",
            flush=True,
        )
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as ex:
            ...
            for fut in as_completed(futs):
                ...
                run = fut.result()
                print(
                    f"[RL-TS] {arm['id']} ok={run['ok']} n={len(run['trades'])} "
    ...
    if not all(r.get("ok") for r in runs):
        print("[RL-TS] One or more arms failed", flush=True)
        for r in runs:
    ...
    if ntfy.is_file():
        a = next(p for p in packed if p["arm"]["id"] == "days40")
        b = next(p for p in packed if p["arm"]["id"] == "days80")
        subprocess.run(
            [
                py,
                str(ntfy),
                ...
                "-t",
                "RL time-stop tradable",
                "-m",
                (
                    f"40d IS {result['verdicts']['days40']['is'][0]} "
                    f"Avg={a['m_is']['avg_pnl']:.2f}% AnnROR={a['m_is']['ann_ror']:.1f} · "
                    f"80d IS {result['verdicts']['days80']['is'][0]} "
