"""WPBR merge-overlapping-zones on vs off — MarkTen parity compare.

Baseline: wpbr_merge_overlapping_zones=false (default, sheet parity).
...
# run_wpbr.bat parity; merge flag overridden per arm
WPBR_COMMON = (
    "wpbr_zones=true",
    "brt_zones=false",
    "yh_zones=false",
...
def build_arms() -> list[Arm]:
    return [
        Arm(
            "merge_off",
            "wpbr_merge_overlapping_zones=false (default / sheet parity)",
            ("wpbr_merge_overlapping_zones=false",),
        ),
        Arm(
            "merge_on",
            "wpbr_merge_overlapping_zones=true (ZONE_STRENGTH = member count)",
            ("wpbr_merge_overlapping_zones=true",),
        ),
    ]
...
def strength_breakdown(closed: Path) -> list[str]:
    """Breakdown of merge-on trades by ZONE_STRENGTH: n, win%, avg PNL%, total $."""
    import pandas as pd

    df = pd.read_csv(closed)
    cols = {c.upper(): c for c in df.columns}
    strength_c = cols.get("ZONE_STRENGTH") or cols.get("WPBR_MERGE_COUNT")
    pnl_c = cols.get("PNL_DOLLARS") or cols.get("PNL")
    pct_c = cols.get("PNL_PCT")
    if strength_c is None:
        return ["(no ZONE_STRENGTH / WPBR_MERGE_COUNT column)"]
    lines = [
        "ZONE_STRENGTH breakdown (merge-on)",
        "strength | n | win% | avg_PNL% | total_$",
    ]
    strengths = []
    for v in df[strength_c]:
        fv = _fp(v)
        strengths.append(int(fv) if fv is not None else 1)
    df = df.copy()
    df["_zs"] = strengths
    for zs in sorted(df["_zs"].unique()):
        s = df[df["_zs"] == zs]
        n = len(s)
        pcts = [_fp(x) for x in s[pct_c]] if pct_c else []
        pcts = [p for p in pcts if p is not None]
        ...
        if pnl_c:
            for x in s[pnl_c]:
                ...
                if v is not None:
                    dol += v
        lines.append(f"{zs} | {n} | {wr:.1f}% | {avg:.1f}% | ${dol:,.2f}")
    return lines
...
def write_paste(results: list[dict]) -> Path:
    ...
    titles = {
        "merge_off": "Baseline (merge=off)",
        "merge_on": "Treatment (merge=on)",
    }
    lines: list[str] = []
    ...
    for arm_id, title in titles.items():
        ...
        lines.append("")
        if arm_id == "merge_on":
            lines.extend(strength_breakdown(closed))
            lines.append("")
            # Confirm column presence
            import pandas as pd

            cols = list(pd.read_csv(closed, nrows=0).columns)
            has_zs = "ZONE_STRENGTH" in cols
            lines.append(f"Closed CSV has ZONE_STRENGTH: {has_zs}")
            lines.append("")
    out = ROOT / "_paste_wpbr_merge_zones.txt"
    out.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
    return out
...
def write_report(results: list[dict]) -> Path:
    ...
    lines = [
        "# WPBR merge overlapping zones — MarkTen compare",
        "",
        ...
        "- Arms: `wpbr_merge_overlapping_zones=false` (baseline) vs `true`",
        "- Enable: `-v wpbr_merge_overlapping_zones=true` (not in run_wpbr.bat)",
        "- Closed column: `ZONE_STRENGTH` = merge member count (1=unmerged)",
        "",
        "## Ranking (by score)",
        "",
    ...
    lines += [
        "",
        f"Artifacts: `{ROOT}` — `comparison.csv`, `comparison.md`, `_paste_wpbr_merge_zones.txt`",
    ]
...
def main() -> int:
    ...
    print(
        f"[wpbr_merge_zones] arms={len(arms)} jobs={args.jobs} workers={args.workers}",
        flush=True,
    )
    print(f"[wpbr_merge_zones] outdir={ROOT}", flush=True)
