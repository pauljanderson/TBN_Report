"""WPBR stop_loss_based anchors on MarkTen — davey compare.
...
# run_wpbr.bat parity; stop_loss_based overridden per arm
WPBR_COMMON = (
    "wpbr_zones=true",
    "brt_zones=false",
    "yh_zones=false",
...
ANCHORS = (
    ("trigger_low", "trigger_low (signal/trigger Low × stop_pct) — WPBR production default"),
    ("entry_open", "entry_open (entry/open × stop_pct)"),
    ("zone_low", "zone_low (zone lower bound × stop_pct)"),
)


def build_arms() -> list[Arm]:
    return [
        Arm(aid, label, (f"stop_loss_based={aid}",))
...
def write_paste(results: list[dict]) -> Path:
    ...
    titles = {
        "trigger_low": "trigger_low (WPBR default / parity)",
    ...
    for arm_id, title in titles.items():
        ...
        lines.append(agg)
        lines.append("")
    out = ROOT / "_paste_stop_anchor_markten.txt"
    out.write_text("\n".join(lines).rstrip() + "\n", encoding="utf-8")
    return out
...
def write_report(results: list[dict]) -> Path:
    ...
    lines = [
        "# WPBR stop_loss_based MarkTen compare",
        "",
        ...
        "- Arms: `stop_loss_based` ∈ {trigger_low, entry_open, zone_low}",
        f"- Production / parity default: **`{DEFAULT_ANCHOR}`** (`run_wpbr.bat` does not override)",
        "- Primary score: davey `score` = 2*PF + 0.02*PPCD - 0.03*MaxDD - 0.002*max_symbol% "
        "(requires >=30 trades)",
        "",
        "## Ranking (by score)",
        "",
        "| rank | arm | default? | trades | PNL | PF | DD | PPCD | AnnROR | Expectancy | score |",
        "|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for i, r in enumerate(ranked, 1):
        ...
        sc = score(m)
        sc_s = f"{sc:.3f}" if math.isfinite(sc) else "n/a"
        is_def = "yes" if r["id"] == DEFAULT_ANCHOR else ""
        lines.append(
            f"| {i} | {r['id']} | {is_def} | {int(m.get('Total_Trades', 0) or 0)} | "
    ...
    lines += [
        ...
        "## Notes",
        "",
        "- `trigger_low` ≈ sheet AM / signal bar Low × stop_pct (legacy alias `signal_low`)",
        "- `entry_open` = entry/open × stop_pct (legacy alias `entry`)",
        "- `zone_low` = zone lower bound × stop_pct (alias `zone_bottom`)",
        "",
        f"Artifacts: `{ROOT}` — `comparison.csv`, `comparison.md`, `status.json`, "
        "`_paste_stop_anchor_markten.txt`, `runs/<phase>__<arm>/`",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--jobs", type=int, default=3, help="Concurrent MarkTen backtests (default 3)")
    parser.add_argument(
        "--workers",
        "-w",
        type=int,
    ...
    print(f"[wpbr_stop_anchor] arms={len(arms)} jobs={args.jobs} workers={args.workers}", flush=True)
    print(f"[wpbr_stop_anchor] outdir={ROOT}", flush=True)
    print(f"[wpbr_stop_anchor] default={DEFAULT_ANCHOR}", flush=True)
