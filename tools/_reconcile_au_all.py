"""Reconcile AU BRT sheet (transcript paste) vs engine stamp 260720143523.
...
AU_CSV = ROOT / "data" / "newdata" / "data" / "AU.csv"
...
def extract_paste() -> str:
    ...
    for line in TRANSCRIPT.read_text(encoding="utf-8").splitlines():
        ...
        is_au = (
            "AU\nDate\tOpen" in blob
            or "AU\r\nDate\tOpen" in blob
            or ("AU" in blob[:200] and has_sections)
        )
        if has_sections and is_au:
            latest = blob
    if not latest:
        raise SystemExit("AU paste not found in transcript")
    if "<user_query>" in latest:
        latest = latest[latest.find("<user_query>") + len("<user_query>") :]
    if "</user_query>" in latest:
        latest = latest[: latest.find("</user_query>")]
    latest = latest.strip()
    if latest.startswith("AU\n") or latest.startswith("AU\r\n"):
        first, _, rest = latest.partition("\n")
        if first.strip() == "AU" and rest:
            latest = rest
...
def ensure_exports(text: str):
    ...
    (TOOLS / "au_brt_sheet_zones.tsv").write_text(zones.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "au_brt_sheet_breakouts.tsv").write_text(bos.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "au_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")

    zero_dates = []
    trading_dates = []
    with (OUT / "AU_sheet_ohlc.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "AU_sheet_zones.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "AU_sheet_breakouts.csv").open("w", newline="", encoding="utf-8") as f:
        if bo_rows:
    ...
    with (OUT / "AU_sheet_trades.csv").open("w", newline="", encoding="utf-8") as f:
        if tr_rows:
...
def reconcile_zones(unique_sheet, stamp: str):
    eng_path = ROOT / "drive" / f"BRT_ZONES_AU_{stamp}.csv"
    rows = []
    ...
    with (OUT / "AU_zones_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# AU zone reconcile (sheet paste 2026-07-20)",
        "",
        f"- Engine: `BRT_ZONES_AU_{stamp}.csv`",
        "- Sheet: `tools/au_brt_sheet_zones.tsv` / `AU_sheet_zones.csv`",
    ...
    (OUT / "AU_zones_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
...
def reconcile_breakouts(bo_rows, stamp: str):
    ...
    with (OUT / "AU_breakouts_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# AU breakout/retest reconcile",
        "",
        "- Sheet TSV: `tools/au_brt_sheet_breakouts.tsv`",
        f"- Engine: `BRT_breakout_and_retest_{stamp}.csv` (AU filter)",
        f"- Window: {WIN_START} .. {WIN_END}",
    ...
    (OUT / "AU_breakouts_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "sheet": len(sheet_bo),
        "engine": len(eng_bo),
        "exact": len(exact),
...
def reconcile_trades(tr_rows, stamp: str):
    ...
    with (OUT / "AU_trades_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
...
def check_ohlc(zero_dates, trading_dates):
    """Compare sheet trading-day OHLC vs engine AU.csv."""
    sheet = {}
    with (OUT / "AU_sheet_ohlc.csv").open(encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            if float(r["open"]) == 0:
                continue
            sheet[r["date"]] = (
    ...
    eng = {}
    if AU_CSV.exists():
        with AU_CSV.open(encoding="utf-8", newline="") as f:
            for r in csv.DictReader(f):
...
def analyze_zero_impact(zero_dates, bo_result, z_result):
    ...
    lines = [
        ...
        f"- Engine AU.csv: trading-day only, **0** zero-OHLC bars",
        "",
        "### Impact mechanism",
        "",
        ...
        "### Recommended fix",
        "",
        f"**Delete all {len(zero_dates)} `$0` rows** from the AU sheet date/OHLC grid.",
...
def four_scenario_stats(stamp: str):
    """Pull AU stats from four-scenario run if available."""
    ...
    with path.open(encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            if (r.get("symbol") or r.get("SYMBOL") or "").upper() == "AU":
                rows.append(r)
    if not rows:
        return None
    lines = ["## Four-scenario Default stats (AU)", ""]
    for r in rows:
        lines.append(
...
def write_summary(exports, z, b, t, ohlc_check, zero_analysis, stamp: str):
    ...
    lines = [
        "# AU BRT sheet vs engine — reconcile summary",
        "",
        f"- Sheet paste: transcript `f301f0a6-...` user message 2026-07-20 (AU OHLC + zones + BOs + trades)",
        ...
        f"- Engine AU.csv: {ohlc_check['engine_bars']} trading-day bars, 0 zero-OHLC",
    ...
    if ohlc_check["mismatch_count"]:
        causes.append(
            (
                2,
                f"**{ohlc_check['mismatch_count']} non-zero OHLC mismatches** vs engine AU.csv — "
    ...
    lines += [
        ...
        "",
        f"1. **Delete all {ohlc_check['zero_count']} `$0` OHLC rows** from the AU sheet date grid first. "
        "This is the highest-impact fix (same as AMZN 4-row fix, but AU has far more).",
        "2. After row deletes, re-paste zones/BOs/trades and re-reconcile — expect row-index alignment and possible zone/BO drift.",
    ]
    if ohlc_check["mismatch_count"]:
        lines.append(
            "3. Review non-zero OHLC mismatches; add AU-specific overrides to `OHLC_override_formula.md` only for confirmed bad prints."
        )
    else:
        lines.append(
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `AU_sheet_ohlc.csv`, `AU_sheet_zones.csv`, `AU_sheet_breakouts.csv`, `AU_sheet_trades.csv`",
        "- `AU_zones_diff.md`, `AU_breakouts_diff.md`, `AU_trades_diff.md`",
        "- Match detail CSVs: `AU_*_match_detail.csv`",
        "",
    ]
    (OUT / "AU_reconcile_summary.md").write_text("\n".join(lines), encoding="utf-8")
    print("\n".join(lines))
