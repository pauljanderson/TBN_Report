"""Reconcile NVDA zone_low sheet trades+BOs vs MarkTen stamp 260721155448."""
...
ENTRY_TOL = 0.05
EXIT_PX_TOL = 0.05
SHEET_TRADES = OUT / "NVDA_zone_low_sheet_trades.csv"
SHEET_BOS = OUT / "NVDA_zone_low_sheet_breakouts.csv"
...
def main():
    ...
    detail = OUT / "NVDA_zone_low_trades_match_detail.csv"
    ...
    lines = [
        "# NVDA zone_low sheet vs engine — reconcile summary",
        "",
        f"- Sheet trades: `NVDA_zone_low_sheet_trades.csv` ({len(sheet)} closed, paste 2026-07-21 16:43)",
        f"- Sheet BOs: `NVDA_zone_low_sheet_breakouts.csv` ({len(sheet_bos)} rows)",
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `NVDA_zone_low_sheet_trades.csv`",
        "- `NVDA_zone_low_sheet_breakouts.csv`",
        "- `NVDA_zone_low_trades_match_detail.csv`",
        "- `NVDA_zone_low_reconcile_summary.md`",
        "",
    ]
    summary = OUT / "NVDA_zone_low_reconcile_summary.md"
    summary.write_text("\n".join(lines), encoding="utf-8")

    print(f"NVDA zone_low: matched={len(matched)}/{len(sheet)} so={len(sheet_only)} eo={len(engine_only)}")
