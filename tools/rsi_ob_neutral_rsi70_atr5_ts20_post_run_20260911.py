#!/usr/bin/env python3
"""Post-run (no charts, no Yahoo) on rsi70+atr5+ts20 CONTROL Closed book."""
...
OUT = ROOT / "drive" / "paul_experiments" / "rsi_ob_neutral_rsi70_atr5_ts20_20260911"
...
LAYMAN = (
    ...
    "COMBO exit Relative Strength Index (RSI) ≥70 plus Average True Range percent "
    "(ATR%) ≥5 plus a 20-day calendar time stop — despite that multi-knob arm being "
    "DISMISS on quality under the combo look / atr5 parent. Research only — not gold, "
    "not DailyRun. One-knob A/B compare lives in <code>compare.html</code> in this same stamp."
)
...
def _prepend_ask(path: Path) -> None:
    ...
    block = (
        ...
        "COMBO_EXIT_rsi70_TIME_20d was a multi-knob DISMISS on quality under "
        "<code>rsi_ob_neutral_rsi80_atr5_combo_rsi70_ts20_20260910</code>; singles were "
        "also DISMISS under the atr5 parent / prior ts20 preference freeze. Paul chose "
        "the combo by preference into control — not a KEEP from metrics. "
