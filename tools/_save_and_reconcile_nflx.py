#!/usr/bin/env python3
"""Save NFLX breakout/trade paste and reconcile vs variant C 2016 stamp."""
...
def update_markten_status(results: dict[str, dict], vc: dict) -> Path:
    ...
    lines.append("|---|---|---|---|---|---|---|---:|---|")
    for sym in ("AU", "NVDA", "NFLX"):
        r = results[sym]
        fair = r["fair"] if "fair" in r else None
        if fair is None:
            # from summary json shape
            piv = r.get("pivots", "?")
            zones = r.get("zones", "?")
            retest = r.get("retest", "?")
            rocket = r.get("rocket_where_sheet", "?")
            raw = r.get("raw", "?")
            ser = r.get("ser", "?")
            ...
            lines.append(
                f"| {sym} | {piv} | {zones} | {retest} | {rocket} | **{raw}** | **{ser}** | "
                f"{closed} | {'; '.join(note_bits) or 'OK'} |"
            )
            continue
        note = []
    ...
    lines.append("")
    lines.append("- Sheet/engine: zh **4.27**, retest **2017-09-25** under variant C (confirmed prior).")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("- NFLX: `drive/wpbr_sheet_reconcile/NFLX/NFLX_wpbr_reconcile_status.md`")
    lines.append("")
    out.write_text("\n".join(lines), encoding="utf-8")
    print(f"wrote {out}")
    return out
...
def main() -> int:
    ...
    if "Break out upper" not in body or "11.36" not in body:
        raise SystemExit("line does not look like NFLX paste")
    save_zones_trades("NFLX", body)

    vc = confirm_variant_c()
    print("=== reconcile NFLX ===")
    r = analyze("NFLX")
    fair = r["fair"]
    print(
    ...
    # ensure AU/NVDA have usable fields
