"""
Run IND ATR/VOL/DIAMOND weight experiment matrix v2 (fair positive-base design).

Queue policy (default): up to 3 concurrent backtests, each with -w 10
(~30 symbol workers total). Override with --jobs / --workers.

Reads experiments/ind_weight_exp_v2_manifest.json from setup_ind_weight_experiments_v2.py.

Outputs under drive/ind_weight_exp_v2/<candidate_id>/
Status:    drive/ind_weight_exp_v2/status.txt
Results:   drive/ind_weight_exp_v2/comparison.csv
           drive/ind_weight_exp_v2/comparison.md
"""
...
MANIFEST_PATH = EXP_DIR / "ind_weight_exp_v2_manifest.json"
MIN_TRADES = 350
MAX_DD = 22.0
...
def write_status(
    *,
    ...
    lines = [
        ...
        f"note:           {note}".rstrip(),
        "",
        "Watch live: Get-Content -Wait drive\\ind_weight_exp_v2\\status.txt",
    ]
...
def extract_metrics(outdir: Path) -> dict | None:
    ...
    return {
        ...
        "Pct_PNL_Top10": _safe_num(row.get("Pct_PNL_Top10", 0)),
    ...
    *,
    ...
    for r in results:
        ...
        rows.append({
            ...
            "Pct_PNL_Top10": round(float(m.get("Pct_PNL_Top10", 0) or 0), 2),
    ...
    lines = [
        "# IND Weight Experiment Comparison (v2)",
        "",
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "",
        "Method: **positive-base** treatment weights (non-targets=0) + empirical "
        "quantile `min_ind_score` calibrated on C0-accepted triggers. "
        "See `score_calibration.md`.",
        "",
        ...
        f"(~{jobs * workers} symbol workers).",
        "",
        "| id | trades | PNL | PF | Max_DD | PPCD | Exp | AnnROR | MaxSym% | gate | vsC0 | vsC1 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---|",
    ]
    for r in rows:
        lines.append(
            f"| {r['id']} | {r['Total_Trades']} | {r['Total_PNL']:.0f} | {r['Profit_Factor']:.3f} | "
            f"{r['Max_DD']:.1f} | {r['PPCD']:.4f} | {r['Expectancy']:.2f} | {r['Ann_ROR']:.1f} | "
            f"{r['Pct_PNL_Max_Symbol']:.1f} | {r['gate_pass']} | "
    ...
    zero_score = [r["id"] for r in rows if r["Total_Trades"] == 0]
    if zero_score:
        lines.extend([
            "",
            "## Notes",
            "",
            f"Zero-trade ids: {', '.join(zero_score)}. "
            "If score-gated arms are empty, re-check `score_calibration.md` quantiles.",
        ])
    lines.extend([
        "",
        "## Production note",
        "",
        "Do **not** update `run_ind.bat` unless a winner clearly beats C0 on PPCD+PNL+PF "
        "while passing hard gates **and** you explicitly approve.",
        "",
        "## Fair-run notes (v2 corrected)",
        "",
        "- Path resolver finds `experiments/ind_weights_v2_*.json` after basename snapshot.",
        "- Score gate active only for `min_ind_score > 0`; epsilon `1e-6` = any positive-base target BULL.",
        "- Production correlation scores are entirely negative → cannot gate with prod weights.",
        "- Integrated write-up: `drive/Followup_ABC_Integrated_Summary.md`.",
    ])
...
def main() -> int:
    ...
    setup = REPO / "tools" / "setup_ind_weight_experiments_v2.py"
