#!/usr/bin/env python3
"""Post-run (no charts, no Yahoo) on maxrsi60+rsi70+atr5+ts20 CONTROL Closed book."""
...
OUT = ROOT / "drive" / "paul_experiments" / "rsi_ob_neutral_rsi70_atr5_ts20_maxrsi60_20260911"
...
LAYMAN = (
    ...
    "ENTRY_max_rsi_60 (trigger Relative Strength Index / RSI &lt; 60) on top of "
    "Neutral-after-Overbought (OB) + exit RSI≥70 + Average True Range percent (ATR%) ≥5 "
    "+ 20-day calendar time stop. Under the prior freeze that arm was HOLD (quality up, "
    "N cut hard) — preference adopt, not a KEEP from metrics. Research only — not gold, "
    "not DailyRun. One-knob A/B compare lives in <code>compare.html</code>; per-stock "
    "rollup in <code>STOCK_SUMMARY.html</code>; universe advice in "
    "<code>UNIVERSE_RECOMMENDATION.md</code>."
)
...
def _prepend_ask(path: Path) -> None:
    ...
    block = (
        ...
        "ENTRY_max_rsi_60 was <strong>HOLD</strong> under "
        "<code>rsi_ob_neutral_rsi70_atr5_ts20_20260911</code> (Avg% up, MaxDD down, "
        "N cut hard ~6948→948). Paul chose it by preference into control — not a KEEP "
        "from metrics. Research only · not gold · not DailyRun.</p></div>"
    )
