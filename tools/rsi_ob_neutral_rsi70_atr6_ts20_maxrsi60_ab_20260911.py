#!/usr/bin/env python3
"""One-knob A/Bs under CONTROL = Neutral-after-OB + exit RSI≥70 + ATR%≥6 + ts20 + max_rsi_trigger=60.

Control preference-adopts ENTRY_min_atr6 (LEAN KEEP FULL / OOS-softened HOLD under
maxrsi60+rsi70+atr5+ts20 parent) into the freeze — same preference-adopt pattern as
ENTRY_max_rsi_60. Research only. Not gold. Not DailyRun.

Usage:
  python tools/rsi_ob_neutral_rsi70_atr6_ts20_maxrsi60_ab_20260911.py
"""
...
STAMP = "rsi_ob_neutral_rsi70_atr6_ts20_maxrsi60_20260911"
OUT_DIR = ROOT / "drive" / "paul_experiments" / STAMP
PRIOR_FREEZE = "rsi_ob_neutral_rsi70_atr5_ts20_maxrsi60_20260911"
PAUL_TWENTY = ROOT / "drive" / "universes" / "PaulTwenty_universe.csv"
REC_UNIV = ROOT / "drive" / "universes" / "RSIN_HighFIT_ISgood_maxrsi60_20260911.csv"
IS_CUTOFF = pd.Timestamp("2024-01-01")
RSI_OB = 70.0
RSI_OS = 30.0
CTRL_EXIT_RSI = 70.0
CTRL_MIN_ATR = 6.0  # preference-adopted ATR% gate (was LEAN KEEP / OOS-softened HOLD)
CTRL_TIME_STOP = 20
CTRL_MAX_RSI = 60.0  # already frozen from prior preference adopt
NOTIONAL = 10_000.0
...
def fixed_arms() -> list[Arm]:
    """Pre-declared one-knob arms vs maxrsi60+rsi70+atr6+ts20 control (no corr pick)."""
    return [
        Arm(
            "CONTROL",
            "none",
            "Frozen: Neutral-after-OB buy with trigger RSI < 60, min ATR%≥6, sell RSI≥70, "
            "time stop 20 calendar days, next open. Preference-adopt of ENTRY_min_atr6 "
            "(LEAN KEEP FULL / OOS-softened HOLD under prior freeze — not a KEEP from metrics).",
        ),
        Arm(
            "ENTRY_no_max_rsi",
            "max_rsi_trigger",
            "Ablate max-RSI gate: restore max_rsi_trigger=70 (Neutral band top) — still atr6.",
        ...
        Arm(
            "EXIT_rsi75",
            "exit_rsi",
            "One notch higher exit: sell at RSI≥75 (still maxrsi60 + ATR%≥6 + ts20).",
            exit_rsi=75.0,
        ),
        Arm(
            "EXIT_rsi80",
            "exit_rsi",
            "Ablate exit upward: sell when RSI≥80 (still maxrsi60 + ATR%≥6 + ts20).",
            exit_rsi=80.0,
        ),
        Arm(
            "ENTRY_min_atr5",
            "min_atr_pct",
            "Ablate atr6 downward to prior freeze: require ATR%≥5 at trigger "
            "(still maxrsi60 + exit 70 + ts20) — proves the adopt.",
            min_atr_pct=5.0,
        ),
        Arm(
            "ENTRY_min_atr7",
            "min_atr_pct",
            "Nudge atr6 upward: require ATR%≥7 at trigger (still maxrsi60 + exit 70 + ts20).",
            min_atr_pct=7.0,
        ),
        Arm(
            "ENTRY_no_atr_gate",
            "min_atr_pct",
            "Ablate atr6 entirely: no ATR% gate (still maxrsi60 + exit 70 + ts20).",
            min_atr_pct=0.0,
        ),
        Arm(
            "TIME_stop_none",
            "time_stop_days",
            "Ablate ts20: no time stop (still maxrsi60 + exit 70 + atr6).",
            time_stop_days=0,
        ),
        Arm(
            "TIME_stop_40d",
            "time_stop_days",
            "Softer time stop at 40 calendar days (still maxrsi60 + exit 70 + atr6).",
            time_stop_days=40,
        ),
    ]
...
def pick_extra_arms(corr_is: pd.DataFrame) -> list[Arm]:
    ...
    Skip ATR% gates (already frozen at 6), time-stop / days-held, and RSI14_AT_TRIGGER
...
def write_html(
        ...
        return intro + "".join(
            [
                book_table_for_slice(
                    "IS",
                    "IS (entry &lt; 2024-01-01)",
                    "In-sample book under the maxrsi60+rsi70+atr6+ts20 control.",
                ),
                book_table_for_slice(
                    "OOS",
    ...
    def univ_compare_section() -> str:
        """FullUniverse vs RecommendedUniverse vs PaulTwenty under NEW freeze."""
        path_map = [
            ("FullUniverse", OUT_DIR / "compare_book_metrics.csv", "CONTROL"),
            ("RecommendedUniverse", OUT_DIR / "compare_recuniv_book.csv", None),
            ("PaulTwenty", OUT_DIR / "compare_paultwenty_book.csv", None),
        ]
        headers = [
            ("Universe", "text"), ("Slice", "text"), ("N", "num"), ("Win %", "num"),
            ("Avg PnL %", "num"), ("Max DD %", "num"), ("Ann ROR %", "num"),
            ("Calmar", "num"), ("Sharpe", "num"), ("Avg days", "num"),
        ]
        th = "".join(sortable_th(a, b) for a, b in headers)
        body = []
        for uname, path, arm_filter in path_map:
            if not path.is_file():
                body.append(
                    f"<tr><td>{html_mod.escape(uname)}</td><td colspan='9'>missing</td></tr>"
                )
                continue
            df = pd.read_csv(path)
            if arm_filter and "arm" in df.columns:
                df = df[df["arm"] == arm_filter]
            for sl in ("FULL", "IS", "OOS"):
                hit = df[df["slice"] == sl] if "slice" in df.columns else df.iloc[0:0]
                if hit.empty:
                    continue
                r = hit.iloc[0]
                body.append(
                    "<tr>"
                    f"<td>{html_mod.escape(uname)}</td>"
                    f"<td>{html_mod.escape(sl)}</td>"
                    f"<td>{fmt_int(r.get('n_trades'))}</td>"
                    f"<td>{fmt_pct_pts(float(r['win_pct']) if pd.notna(r.get('win_pct')) else float('nan'))}</td>"
                    f"<td>{fmt_pct_pts(float(r['avg_pnl_pct']) if pd.notna(r.get('avg_pnl_pct')) else float('nan'))}</td>"
                    f"<td>{fmt_pct_pts(float(r['max_dd']) if pd.notna(r.get('max_dd')) else float('nan'))}</td>"
                    f"<td>{fmt_pct_pts(float(r['ann_ror']) if pd.notna(r.get('ann_ror')) else float('nan'))}</td>"
                    f"<td>{fmt_num(float(r['calmar']) if pd.notna(r.get('calmar')) else float('nan'))}</td>"
                    f"<td>{fmt_num(float(r['sharpe']) if pd.notna(r.get('sharpe')) else float('nan'))}</td>"
                    f"<td>{fmt_num(float(r['avg_days']) if pd.notna(r.get('avg_days')) else float('nan'), 1)}</td>"
                    "</tr>"
                )
        return (
            "<h2>2b. Universe compare under NEW freeze (CONTROL)</h2>"
            "<p class='meta'>FullUniverse vs RecommendedUniverse (High-FIT / IS-good hybrid from "
            f"<code>{PRIOR_FREEZE}</code>) vs PaulTwenty. Rec-univ was selected on the "
            "<strong>prior</strong> Closed book — in-sample selection bias. Click headers to sort.</p>"
            f'<table class="sortable"><thead><tr>{th}</tr></thead><tbody>{"".join(body)}</tbody></table>'
        )
...
<body>
<h1>RSI Neutral-after-OB — CONTROL = max_rsi_trigger&lt;60 + exit RSI≥70 + ATR%≥6 + TIME_stop 20d</h1>
...
(a deeper cool-off), with a stricter Average True Range percent (ATR%) ≥6 gate, sell at
RSI≥70, and flatten by day 20 if needed. Under the prior freeze <code>{PRIOR_FREEZE}</code>,
the arm <code>ENTRY_min_atr6</code> was <strong>LEAN KEEP (research)</strong> on FULL
(Avg% +0.886, MaxDD better) but <strong>OOS softened</strong> vs control → rolled to HOLD.
You asked to adopt it anyway with the recommended High-FIT / IS-good universe. That is a
preference adoption — <em>not</em> a KEEP from metrics. This stamp re-scores FULL/IS/OOS
under the new freeze and runs one-change tests (ablate max-RSI, 55/65 notches, exit 75/80,
atr5/7/none, time-stop none/40d). Later years (2024+) are a check only; we did not retune.
Also: per-stock summary + universe recommendation in this folder. Research only — not gold,
not wired to DailyRun.</p>
</div>
<div class="insight">
...
<code>ENTRY_min_atr6</code> was <strong>LEAN KEEP FULL / OOS-softened HOLD</strong>.
Paul preference-adopts it into control — <em>not</em> a KEEP from metrics. The recommended
universe (High-FIT / IS-good hybrid) was selected on that prior Closed book — in-sample
selection. We re-scored FULL/IS/OOS under the frozen maxrsi60+rsi70+atr6+ts20 combo; OOS
was not used to pick or retune. Research only · not gold · not DailyRun.</p>
</div>
<h2>1. Arms (one knob each vs atr6+maxrsi60 control)</h2>
<ul class="meta">{hyps}</ul>
<h2>2. Book compare (canonical overlay)</h2>
{book_tables()}
{univ_compare_section()}
...
in the same stamp folder. Recommended universe CSV:
<code>drive/universes/RSIN_HighFIT_ISgood_maxrsi60_20260911.csv</code>.</p>
{SORT_JS}
</body>
</html>
"""
...
def write_baseline(path: Path, arms: list[Arm]) -> None:
    ...
    if REC_UNIV.is_file():
        try:
            n_rec = len(pd.read_csv(REC_UNIV))
        except Exception:
            n_rec = 0
    lines = [
        f"# BASELINE — {STAMP}",
        ...
        "## In plain English",
        "",
        "Relative Strength Index (RSI) Neutral-after-Overbought (OB) with deeper cool-off",
        "(trigger RSI < 60), exit RSI≥70, Average True Range percent (ATR%) ≥6, and a",
        "20-day time stop. Under the prior freeze, ENTRY_min_atr6 was LEAN KEEP on FULL",
        "but OOS softened → HOLD. Paul preference-adopts it into control with the",
        "High-FIT / IS-good recommended universe — not a KEEP from metrics. Re-score",
        "one-change A/Bs; also per-stock summary + universe recommendation.",
        "",
        "## Control freeze",
        "",
        "- Buy: prior RSI≥70 and today Neutral (30<RSI<70); next-open fill",
        f"- Entry gate: trigger RSI < {CTRL_MAX_RSI:g} (`max_rsi_trigger={CTRL_MAX_RSI:g}`) — already frozen",
        f"- **NEW entry gate: ATR% at trigger ≥ {CTRL_MIN_ATR:g}** — preference adopt of ENTRY_min_atr6",
        f"- Sell: RSI ≥ {CTRL_EXIT_RSI:g}; next-open fill",
        f"- Time stop: flatten at {CTRL_TIME_STOP} calendar days if still not exit-RSI",
        "- Primary research book: FullUniverse = all `data/newdata/data/*.csv` (N honest)",
        f"- Recommended research universe: `RSIN_HighFIT_ISgood_maxrsi60_20260911.csv` "
        f"(N={n_rec}) — High-FIT OR IS plain-English good from prior Closed; "
        "**in-sample selection** on `{PRIOR_FREEZE}` history",
        "- Also replay: PaulTwenty",
        "- Costs 0 · $10k notional · $500k overlay",
        "- IS: entry < 2024-01-01; OOS ≥ 2024-01-01 (report-only)",
        "",
        "## Provenance / selection honesty",
        "",
        f"Prior freeze: `{PRIOR_FREEZE}`. There, **ENTRY_min_atr6** was **LEAN KEEP "
        "(research)** on FULL (Avg% +0.886, MaxDD better) but **OOS softened** vs "
        "control → rolled into HOLD with OOS-softened note. Paul preference-adopts it "
        "into this control anyway — same pattern as ENTRY_max_rsi_60 preference adopt, "
        "**not** a KEEP from metrics. Recommended universe was built from that prior "
        "stamp's IS_STOCK_SUMMARY good / High-FIT hybrid — selection-biased. This stamp "
        "**re-scores** FULL/IS/OOS under the frozen maxrsi60+rsi70+atr6+ts20 combo and "
        "runs new one-knob arms around it. OOS is report-only — do not retune on OOS. "
    ...
    lines += [
        ...
        "(ATR frozen at 6; max_rsi_trigger frozen at 60). Cap ≤3 extras. No combinatorial grid.",
        "",
    ]
...
def write_summary(path: Path, arms: list[Arm], books: dict[str, list[dict]]) -> None:
    ...
    lines = [
        ...
        f"min ATR%≥6 + TIME_stop 20d (preference-adopted LEAN KEEP/OOS-softened HOLD arm from "
        f"`{PRIOR_FREEZE}` — not a KEEP from metrics).",
        "",
        "## What you asked",
        "",
        ORIGINAL_REQUEST,
        ...
        "sell at RSI 70, Average True Range percent (ATR%) ≥6, plus a 20-day calendar time stop — ",
        "frozen by preference after you liked the atr6 arm (despite OOS soft). One change at a ",
        "time around that freeze. Quality over count; OOS is a check only. Recommended High-FIT / ",
        "IS-good universe replayed under the same freeze (selection-biased on prior history).",
        "",
        "## Control headlines (FullUniverse)",
        "",
    ...
    rec_path = OUT_DIR / "compare_recuniv_book.csv"
    if rec_path.is_file():
        rdf = pd.read_csv(rec_path)
        lines.append("## Control headlines (RecommendedUniverse)")
        lines.append("")
        for sl in ("FULL", "IS", "OOS"):
            hit = rdf[rdf["slice"] == sl]
            if hit.empty:
                continue
            r = hit.iloc[0]
            lines.append(
                f"- **{sl}** N={int(r['n_trades']):,} Win%={float(r['win_pct']):.2f} "
                f"AvgPnL%={float(r['avg_pnl_pct']):.3f} MaxDD={float(r['max_dd']):.2f}"
            )
        lines.append("")
    lines += [
...
def enrich_control_closed() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    enrich_script = ROOT / "tools" / "rsi_ob_neutral_rsi70_atr6_ts20_maxrsi60_enrich_20260911.py"
    import subprocess
    r = subprocess.run(
...
def _replay_control_on_syms(syms: list[str], ctrl: Arm) -> list[dict]:
    trades: list[dict] = []
    for sym in syms:
        df = load_ohlcv(sym)
        if df is None or len(df) < 20:
...
def main() -> int:
    ...
    # Keep a copy of recommended universe inside the stamp
    if REC_UNIV.is_file():
        dest = OUT_DIR / REC_UNIV.name
        if not dest.is_file() or dest.read_bytes() != REC_UNIV.read_bytes():
            dest.write_bytes(REC_UNIV.read_bytes())
        # README note for N + selection bias
        n_rec = len(_load_symbol_list(REC_UNIV))
        (OUT_DIR / "REC_UNIV_NOTE.md").write_text(
            "\n".join(
                [
                    f"# Recommended universe — {STAMP}",
                    "",
                    f"- Path: `drive/universes/{REC_UNIV.name}` (copied into this stamp)",
                    f"- N symbols: **{n_rec}**",
                    f"- Rule: High-FIT OR IS plain-English good from `{PRIOR_FREEZE}` Closed book",
                    "- **In-sample selection** on prior history — research watchlist only",
                    "- Not gold · not DailyRun",
                    "",
                ]
            ),
    ...
    arms = fixed_arms()
    print(f"Phase 1: {len(arms)} fixed arms (control = maxrsi60 + rsi70 + atr6 + ts20)", flush=True)
    by_arm = run_all(arms)
    ...
    # PaulTwenty replay under SAME freeze (CONTROL arm)
    pt_syms = _load_symbol_list(PAUL_TWENTY)
    if pt_syms:
        print(f"PaulTwenty replay ({len(pt_syms)} symbols) under CONTROL freeze…", flush=True)
        pt_trades = _replay_control_on_syms(pt_syms, ctrl)
    ...
    # RecommendedUniverse replay under SAME freeze (CONTROL arm)
    rec_syms = _load_symbol_list(REC_UNIV)
    if rec_syms:
        print(
            f"RecommendedUniverse replay ({len(rec_syms)} symbols) under CONTROL freeze…",
            flush=True,
        )
        rec_trades = _replay_control_on_syms(rec_syms, ctrl)
        write_closed_csv(OUT_DIR / "closed_recuniv.csv", rec_trades)
        rec_books = [book_metrics(filter_slice(rec_trades, s), s) for s in ("FULL", "IS", "OOS")]
        pd.DataFrame([{"universe": "RecommendedUniverse", **b} for b in rec_books]).to_csv(
            OUT_DIR / "compare_recuniv_book.csv", index=False
        )
        print(f"  RecUniv FULL N={rec_books[0]['n_trades']}", flush=True)
