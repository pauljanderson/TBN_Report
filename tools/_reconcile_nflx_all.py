"""Reconcile NFLX BRT sheet (transcript paste) vs engine stamp 260720143523.

Layers: zones (+/- $0.02), breakouts/retests, closed trades (+/- $0.05).
Also accepts an optional NFLX-only re-run stamp via --trades-stamp for trades
when the multi-symbol preferred stamp has 0 NFLX closed rows.
"""
...
NFLX_CSV = ROOT / "data" / "newdata" / "data" / "NFLX.csv"
...
def extract_paste() -> str:
    """Prefer the latest full NFLX paste (OHLC + zones + BOs + trades)."""
    latest = None
    for line in TRANSCRIPT.read_text(encoding="utf-8").splitlines():
        ...
        is_nflx = (
            "NFLX\nDate\tOpen" in blob
            or "NFLX\r\nDate\tOpen" in blob
            or ("1/4/2010\t$0.7900" in blob and has_sections)
        )
        if has_sections and is_nflx:
            latest = blob
    if not latest:
        raise SystemExit("NFLX paste not found in transcript")
    if "<user_query>" in latest:
        latest = latest[latest.find("<user_query>") + len("<user_query>") :]
    if "</user_query>" in latest:
        latest = latest[: latest.find("</user_query>")]
    latest = latest.strip()
    if latest.startswith("NFLX\n") or latest.startswith("NFLX\r\n"):
        first, _, rest = latest.partition("\n")
        if first.strip() == "NFLX" and rest:
            latest = rest
...
def ensure_exports(text: str):
    ...
    (TOOLS / "nflx_brt_sheet_zones.tsv").write_text(zones.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "nflx_brt_sheet_breakouts.tsv").write_text(bos.rstrip() + "\n", encoding="utf-8")
    (TOOLS / "nflx_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")
    (OUT / "nflx_brt_sheet_trades.tsv").write_text(trades.rstrip() + "\n", encoding="utf-8")

    zero_dates = []
    with (OUT / "NFLX_sheet_ohlc.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "NFLX_sheet_zones.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
    ...
    with (OUT / "NFLX_sheet_breakouts.csv").open("w", newline="", encoding="utf-8") as f:
        if bo_rows:
    ...
    with (OUT / "NFLX_sheet_trades.csv").open("w", newline="", encoding="utf-8") as f:
        if tr_rows:
...
def reconcile_zones(unique_sheet, stamp: str):
    eng_path = ROOT / "drive" / f"BRT_ZONES_NFLX_{stamp}.csv"
    rows = []
    ...
    with (OUT / "NFLX_zones_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        f"# NFLX zone reconcile (sheet paste 2026-07-20)",
        "",
        f"- Engine: `BRT_ZONES_NFLX_{stamp}.csv`",
        "- Sheet: `tools/nflx_brt_sheet_zones.tsv` / `NFLX_sheet_zones.csv`",
    ...
    (OUT / "NFLX_zones_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
...
def reconcile_breakouts(bo_rows, stamp: str):
    ...
    with (OUT / "NFLX_breakouts_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# NFLX breakout/retest reconcile",
        "",
        "- Sheet TSV: `tools/nflx_brt_sheet_breakouts.tsv`",
        f"- Engine: `BRT_breakout_and_retest_{stamp}.csv` (NFLX filter)",
        f"- Window: {WIN_START} .. {WIN_END}",
        ...
        f"- Engine BO (NFLX total): **{len(eng_all)}**",
        f"- Engine BO (NFLX in window): **{len(eng_bo)}**",
    ...
    (OUT / "NFLX_breakouts_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "sheet": len(sheet_bo),
        "engine": len(eng_bo),
        "exact": len(exact),
...
def reconcile_trades(tr_rows, stamp: str):
    ...
    with (OUT / "NFLX_trades_match_detail.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(
            [
                "status",
    ...
    lines = [
        "# NFLX BRT trades reconcile — sheet vs engine",
        "",
        f"- Sheet: `tools/nflx_brt_sheet_trades.tsv` ({len(load_sheet_trades(tr_rows))} closed rows pasted)",
        ...
        f"- Engine NFLX trades in window: **{len(eng_tr)}**",
        "",
        "## Match summary",
    ...
    if len(eng_tr) == 0:
        note = (
            f"**NOTE:** Preferred multi-symbol stamp `{STAMP}` has **0** NFLX closed trades "
            f"(zones/BOs present). This trades layer used stamp `{stamp}`. "
            "Root cause hypothesis: post-2026-07-19 multi-symbol runs drop NFLX (and ~12 other symbols) "
            "from closed/open outputs despite writing zones/breakouts — likely portfolio/resume path "
            "or symbol-loop skip; NFLX-only re-run recommended for trade parity."
        )
        lines += ["## Engine availability note", "", note, ""]

    (OUT / "NFLX_trades_diff.md").write_text("\n".join(lines), encoding="utf-8")
    return {
        "stamp": stamp,
        "sheet": len(sheet_tr),
        "sheet_after": len(sheet_after),
...
def write_summary(exports, z, b, t, zero_dates, zones_stamp: str, bo_stamp: str, ohlc_check: dict):
    lines = [
        "# NFLX BRT sheet vs engine — reconcile summary",
        "",
        f"- Sheet paste: transcript `f301f0a6-...` user message 2026-07-20 18:15 (NFLX OHLC + zones + BOs + trades)",
        f"- Zones stamp: **{zones_stamp}**; BO/trades stamp: **{bo_stamp}** (`stop_loss_based=trigger_low`, SPY -1000, zone pick max)",
        f"- Trades stamp used: **{t['stamp']}**",
        f"- Sheet OHLC `$0` holiday placeholders: **{len(zero_dates)}** ({', '.join(zero_dates[:8])}{'...' if len(zero_dates)>8 else ''})",
        f"- Engine NFLX.csv: present, trading-day-only (0 zero-OHLC bars)",
    ...
    if t["engine"] == 0:
        lines.append(
            f"- **Trades:** stamp has **0** NFLX closed trades despite BOs. "
            "Often `max_market_cap` default wipes NFLX when yfinance leaves market_cap=None — use `max_market_cap=0` or checkpoint rewrite. "
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `NFLX_sheet_ohlc.csv`, `NFLX_sheet_zones.csv`, `NFLX_sheet_breakouts.csv`, `NFLX_sheet_trades.csv`",
        "- `NFLX_zones_diff.md`, `NFLX_breakouts_diff.md`, `NFLX_trades_diff.md`",
        "- Match detail CSVs: `NFLX_*_match_detail.csv`",
        "- Four-scenario cut-paste: `NFLX_four_scenario_stats.md`",
        "",
    ]
    (OUT / "NFLX_reconcile_summary.md").write_text("\n".join(lines), encoding="utf-8")
    print("\n".join(lines))


def check_critical_bars():
    """Spot-check a few sheet OHLC bars vs engine NFLX.csv."""
    want_dates = ["2010-01-04", "2013-06-24", "2022-01-31", "2024-02-26"]
    sheet = {}
    with (OUT / "NFLX_sheet_ohlc.csv").open(encoding="utf-8", newline="") as f:
        for r in csv.DictReader(f):
            d = r.get("date")
            if d in want_dates:
                sheet[d] = (
    ...
    eng = {}
    if NFLX_CSV.exists():
        with NFLX_CSV.open(encoding="utf-8", newline="") as f:
            for r in csv.DictReader(f):
...
def pick_trades_stamp(cli_stamp: str | None) -> str:
    if cli_stamp:
        return cli_stamp
    # Prefer preferred stamp if it has NFLX trades; else newest prior stamp with NFLX.
    ...
    print(f"[trades] preferred {STAMP} has 0 NFLX; using {cands[0][0]} ({cands[0][1]} trades)")
    return cands[0][0]
