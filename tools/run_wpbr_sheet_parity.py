#!/usr/bin/env python3
"""Compare WPBR engine output to spreadsheet ground truth (zones + trades)."""
...
def compare_symbol(
    ...
    fills = sorted(_ymd(df.index[b]) for b in out.get("wpbr_entry_fill_bars") or [])
    trade_hits = 0
    trade_miss: list[str] = []
    for tr in gt.trades:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="WPBR parity vs spreadsheet ground truth")
    ...
    gt_all = load_wpbr_ground_truth(args.paste)
    exit_code = 0
