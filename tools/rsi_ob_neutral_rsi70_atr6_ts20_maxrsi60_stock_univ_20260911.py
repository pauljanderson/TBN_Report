#!/usr/bin/env python3
"""STOCK_SUMMARY + UNIVERSE_RECOMMENDATION for atr6+maxrsi60 stamp (after enrich/post-run)."""
...
PRIOR = "rsi_ob_neutral_rsi70_atr5_ts20_maxrsi60_20260911"
...
REC_CLOSED = OUT / "closed_recuniv.csv"
REC_BOOK = OUT / "compare_recuniv_book.csv"
...
def write_stock_summary(df: pd.DataFrame) -> None:
    ...
    md = [
        ...
        "ATR% ≥6, 20-day time stop). **IS columns** (entry before 2024-01-01) are the ones to",
...
<p class="caveat">Selection honesty: ENTRY_min_atr6 was LEAN KEEP FULL / OOS-softened HOLD under <code>{PRIOR}</code>;
preference-adopted — not a KEEP from metrics. Filtering later by FIT/IS on this same history
is in-sample selection bias. RecommendedUniverse was selected on that prior Closed book.</p>
</div>
...
def write_universe_rec(stock_df: pd.DataFrame) -> str:
    ...
    recu = book_from_closed_csv(REC_CLOSED)
    if REC_BOOK.is_file() and not recu:
        bdf = pd.read_csv(REC_BOOK)
        recu = {}
        for _, r in bdf.iterrows():
            recu[str(r["slice"])] = {
                "n_trades": int(r["n_trades"]),
    ...
    if r_is:
        verdict_bits.append(
            f"RecommendedUniverse IS (prior High-FIT/IS-good hybrid, selection-biased): "
            f"N={int(r_is.get('n_trades', 0)):,} "
            f"Win%={_pct_str(r_is.get('win_pct'))} Avg%={_pct_str(r_is.get('avg_pnl_pct'), 3)} "
            f"MaxDD={_pct_str(r_is.get('max_dd'))}."
        )
    if p_is:
        verdict_bits.append(
    ...
    lines = [
        ...
        "## In plain English",
        "",
        "Same system freeze (trigger RSI < 60, ATR% ≥6, sell RSI ≥70, 20-day time stop) on",
        "the full CSV universe vs the prior High-FIT/IS-good recommended list vs PaulTwenty.",
        "Quality (Avg PnL%, win%, Max DD) matters more than how many trades show up.",
        "The recommended list was selected on the PRIOR stamp Closed book — selection bias.",
        "",
        "## Books under the SAME freeze (atr6 + maxrsi60 + rsi70 + ts20)",
        "",
    ...
    lines += ["", "### RecommendedUniverse (High-FIT / IS-good from prior stamp)", ""]
    if not r_full:
        lines.append("- Missing `closed_recuniv.csv` / book metrics.")
    else:
        for sl in ("FULL", "IS", "OOS"):
            b = recu.get(sl, {})
            if not b:
    ...
    lines += [
        "",
        "## FIT / fit rollup (FullUniverse control Closed under NEW freeze)",
        "",
    ...
    lines += [
        ...
        "### Honesty / process",
        "",
        f"- Prior freeze `{PRIOR}`: ENTRY_min_atr6 was **LEAN KEEP FULL / OOS-softened HOLD**; "
        "this stamp is preference adopt (not a KEEP from metrics).",
        "- RecommendedUniverse CSV was selected on that prior Closed book (High-FIT OR IS-good) "
        "— **in-sample selection**; re-scored here under the new atr6 freeze only.",
        "- OOS is report-only — do not retune the gate or universe on OOS lifts.",
        "- High-FIT-only lists picked on Closed history are **in-sample selection**; "
...
<h2>In plain English</h2>
<p>Same atr6 freeze on FullUniverse vs RecommendedUniverse (prior High-FIT/IS-good hybrid)
vs PaulTwenty. Judge quality over trade count. Rec-univ selection is biased on prior history
— research watchlist only.</p>
</div>
<div class="insight">
<p><strong>Verdict: {html_mod.escape(rec)}</strong></p>
<ul>{"".join(f"<li>{html_mod.escape(b)}</li>" for b in verdict_bits)}</ul>
<p class="caveat">Research only · not gold · not DailyRun · OOS report-only · preference adopt of ENTRY_min_atr6.</p>
</div>
