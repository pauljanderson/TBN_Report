"""Reconcile AMZN zone_low sheet trades+BOs vs MarkTen stamp 260721155448."""
...
ENTRY_TOL = 0.05
EXIT_PX_TOL = 0.05
SHEET_TRADES = OUT / "AMZN_zone_low_sheet_trades.csv"
SHEET_BOS = OUT / "AMZN_zone_low_sheet_breakouts.csv"
...
def main():
    ...
    detail = OUT / "AMZN_zone_low_trades_match_detail.csv"
    ...
    lines = [
        "# AMZN zone_low sheet vs engine — reconcile summary",
        "",
        f"- Sheet trades: `AMZN_zone_low_sheet_trades.csv` ({len(sheet)} closed, paste 2026-07-21 16:43)",
        f"- Sheet BOs: `AMZN_zone_low_sheet_breakouts.csv` ({len(sheet_bos)} rows)",
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        "- `AMZN_zone_low_sheet_trades.csv`",
        "- `AMZN_zone_low_sheet_breakouts.csv`",
        "- `AMZN_zone_low_trades_match_detail.csv`",
        "- `AMZN_zone_low_reconcile_summary.md`",
        "",
    ]
    summary = OUT / "AMZN_zone_low_reconcile_summary.md"
    summary.write_text("\n".join(lines), encoding="utf-8")

    print(f"AMZN zone_low: matched={len(matched)}/{len(sheet)} so={len(sheet_only)} eo={len(engine_only)}")
