#!/usr/bin/env python3
"""RL target-expand A/B on tradable 764 tape.

Evidence: ImprovePriority 260828112205 target_pct_tension (lean expand):
579/902 TARGET exits (64%) continued ≥5% higher within 15 bars.

One EXIT knob: raise `rl_target_pct` only. Do not move `rl_expansion`.
Do not run slow-grind contract / partial exit on this stamp.

Control: target=1.20, stop=0.934 (reuse tradable Closed 260828112205).
Stop expand was DISMISS (AnnROR drop) — this stamp keeps 0.934.
Candidates (pre-agreed, not a grid): 1.25 and 1.30.
Trails off. post_target_reentry_bars=0.

Research-only. Not gold. Not DailyRun. Do not overwrite RL_universe.csv.

Usage:
  python tools/rl_target_expand_ab.py
  python tools/rl_target_expand_ab.py --summarize-only
"""
...
DRIVE = ROOT / "drive"
STAMP = "20260828"
OUT_DIR = DRIVE / "paul_experiments" / f"rl_target_expand_tradable_{STAMP}"
CONTROL_SRC = (
...
ARM_ORDER = {"control": 0, "tgt125": 1, "tgt130": 2}
...
def _decision(verdicts: dict[str, dict[str, tuple[str, str]]]) -> str:
    vis_a, vis_b = verdicts["tgt125"]["is"][0], verdicts["tgt130"]["is"][0]
    voos_a, voos_b = verdicts["tgt125"]["oos"][0], verdicts["tgt130"]["oos"][0]
    keepish = {"KEEP", "LEAN KEEP"}
    if vis_a not in keepish and vis_b not in keepish:
        return (
            f"**DISMISS** both. IS did not beat target=1.20 (1.25: {vis_a}; 1.30: {vis_b}). "
            f"OOS report-only (1.25: {voos_a}; 1.30: {voos_b}). Stop stays 0.934. Do not move rl_expansion. Do not retune."
        )
    if (vis_a in keepish and voos_a == "DISMISS") or (vis_b in keepish and voos_b == "DISMISS"):
        return (
            f"**HOLD**. IS quality looked better (1.25: {vis_a}; 1.30: {vis_b}) but OOS softened "
            f"(1.25: {voos_a}; 1.30: {voos_b}). Do not retune OOS. Research-only. Stop stays 0.934."
        )
    return (
        f"IS 1.25={vis_a}; 1.30={vis_b}. OOS report-only 1.25={voos_a}; 1.30={voos_b}. "
        "Research candidate ≠ gold ≠ DailyRun. Stop stays 0.934."
    )
...
def write_compare_html(
    ...
    for split_key, title in (("m_is", "IS"), ("m_oos", "OOS (report-only)"), ("m_full", "FULL book")):
        ...
        sections.append(
            f'<section><h2>Target expand — {title}</h2>'
            f'<p class="muted">Δ vs control (rl_target_pct=1.20). Stop frozen at 0.934. {note} '
    ...
    pairs = [
        ("1.25 − control", by_id[CONTROL_ID], by_id["tgt125"]),
        ("1.30 − control", by_id[CONTROL_ID], by_id["tgt130"]),
        ("1.30 − 1.25", by_id["tgt125"], by_id["tgt130"]),
    ]
    pw_sections = []
...
<title>RL target expand A/B — tradable {STAMP}</title>
<style>
...
<body>
<header>
<h1>RL target expand — tradable 764</h1>
<p class="muted">Stamp <code>rl_target_expand_tradable_{STAMP}</code>. One knob: <code>rl_target_pct</code> expand only
(1.20 → 1.25 / 1.30). Freeze: dip=1.055, expansion=1.163, <strong>stop=0.934</strong> (stop expand DISMISS),
trails off, post-TARGET bars=0. Not gold / not DailyRun.
IS = entry &lt; 2024-01-01; OOS report-only. Click column headers to sort.</p>
</header>
<main>
<div class="callout">
<strong>Evidence:</strong> ImprovePriority <code>260828112205</code> <em>target_pct_tension</em> lean expand —
579/902 TARGET exits (64%) continued ≥5% within 15 bars.
Hypothesis: a higher target captures leftover run without changing dip/stop/expansion.
Pre-agreed: <strong>1.25</strong> and <strong>1.30</strong> vs control <strong>1.20</strong>.
Do not move <code>rl_expansion</code>. Do not A/B slow-grind contract on this stamp.
Stop expand stayed DISMISS (AnnROR); this freeze uses existing stop 0.934.
<ul>{"".join(v_lis)}</ul>
...
def write_docs(packed: list[dict[str, Any]], verdicts: dict[str, dict[str, tuple[str, str]]]) -> None:
    by_id = {p["arm"]["id"]: p for p in packed}
    hyp = f"""# HYPOTHESIS — RL target expand on tradable tape

| Field | Fill in |
|-------|---------|
| System / prefix | RL |
| Baseline stamp | tradable Closed `260828112205` (`rl_target_pct=1.20`, `rl_stop_pct=0.934`) |
| Universe | tradable 2010 / ADV$2m (764) |
| **Evidence** | ImprovePriority `RL_ImprovePriority_260828112205.html` **target_pct_tension** lean expand: 579/902 TARGET exits (64%) continued ≥5% within 15 bars |
| **Hypothesis** | If we raise `rl_target_pct`, leftover post-TARGET run is captured and book quality (Avg%, WO_MAX, PF, WR, AnnROR, DD) improves |
| **Single knob** | `rl_target_pct` expand only |
| Frozen settings | dip=1.055, expansion=1.163, **stop=0.934** (stop expand DISMISS on AnnROR), trails off, `rl_post_target_reentry_bars=0` |
| Alternatives | control **1.20**; **1.25**; **1.30**. Not `rl_expansion`. Not slow-grind contract / partial exit on this stamp |
...
"""
    ...
    lines = [
        f"# BASELINE — `rl_target_expand_tradable_{STAMP}`",
        "",
        "**Status:** RESEARCH only. Not gold. Not DailyRun. One knob (target expand).",
        "",
        "| Field | Value |",
        "|---|---|",
        "| Universe | tradable 2010 / ADV$2m (764) |",
        "| Control | reuse `RL_Closed_260828112205` (`rl_target_pct=1.20`, `rl_stop_pct=0.934`) |",
        "| Candidates | 1.25; 1.30 |",
        "| Frozen | dip=1.055, expansion=1.163, **stop=0.934**, trails off, post-TARGET bars=0 |",
        "| Stop expand | DISMISS (`rl_stop_expand_tradable_20260828`) — AnnROR drop; not used here |",
        "| Split | IS entry < 2024-01-01; OOS report-only |",
        "",
        "Do **not** retune on OOS. Do **not** move `rl_expansion`. Do **not** overwrite `RL_universe.csv`.",
        "",
        "| Arm | Stamp | N_full | OK |",
        "|-----|-------|--------|-----|",
    ]
    ...
    slines = [
        f"# SUMMARY — `rl_target_expand_tradable_{STAMP}`",
        "",
        "Target expand (`rl_target_pct` 1.25 / 1.30 vs 1.20) on tradable 764. Stop frozen at 0.934. Research only.",
        "",
        "## IS",
        "",
    ]
...
def summarize(packed: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    print(f"[RL-TE] Wrote {OUT_DIR / 'compare.html'}", flush=True)
...
def main() -> int:
    ...
    if not trad:
        print("[RL-TE] Missing tradable universe", flush=True)
        return 1
    ...
    if args.summarize_only:
        for arm in arms:
            ...
            if not closed:
                print(f"[RL-TE] Missing Closed for {arm['id']}", flush=True)
                return 1
            trades = load_trades(closed)
            runs.append(
                {
        ...
        print(
            f"[RL-TE] control reuse ok={ctrl_run['ok']} n={len(ctrl_run['trades'])} stamp={ctrl_run.get('stamp')}",
            flush=True,
        )
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as ex:
            ...
            for fut in as_completed(futs):
                ...
                run = fut.result()
                print(
                    f"[RL-TE] {arm['id']} ok={run['ok']} n={len(run['trades'])} "
    ...
    if not all(r.get("ok") for r in runs):
        print("[RL-TE] One or more arms failed", flush=True)
        for r in runs:
    ...
    if ntfy.is_file():
        a = next(p for p in packed if p["arm"]["id"] == "tgt125")
        b = next(p for p in packed if p["arm"]["id"] == "tgt130")
        subprocess.run(
            [
                py,
                str(ntfy),
                ...
                "-t",
                "RL target expand tradable",
                "-m",
                (
                    f"1.25 IS {result['verdicts']['tgt125']['is'][0]} "
                    f"Avg={a['m_is']['avg_pnl']:.2f}% AnnROR={a['m_is']['ann_ror']:.1f} · "
                    f"1.30 IS {result['verdicts']['tgt130']['is'][0]} "
                    f"Avg={b['m_is']['avg_pnl']:.2f}% AnnROR={b['m_is']['ann_ror']:.1f}"
                ),
            ],
            cwd=str(ROOT),
        )
