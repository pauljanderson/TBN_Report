#!/usr/bin/env python3
"""One-knob A/Bs under CONTROL = Neutral-after-OB + exit RSI≥80 + min ATR%≥5.

Control freezes ENTRY_min_atr5 (lean from rsi_ob_neutral_rsi80_atr4_20260910) on top of
the prior rsi80+atr4 combo. Arms differ by exactly one knob. Research only.

Usage:
  python tools/rsi_ob_neutral_rsi80_atr5_ab_20260910.py
"""
...
STAMP = "rsi_ob_neutral_rsi80_atr5_20260910"
OUT_DIR = ROOT / "drive" / "paul_experiments" / STAMP
PRIOR_TUNE = "rsi_ob_neutral_rsi80_atr4_20260910"
...
ORIGINAL_REQUEST = (
    "lets take this report and implement min atr 5. RSI Neutral-after-OB — "
    "CONTROL = exit RSI≥80 + min ATR%≥4 — one-knob A/Bs"
)
...
def fixed_arms() -> list[Arm]:
    """Pre-declared one-knob arms vs rsi80+atr5 control (no corr pick)."""
    return [
        Arm(
            "CONTROL",
            "none",
            "Frozen combo: Neutral-after-OB buy, min ATR%≥5 at trigger, sell RSI≥80, next open, no time stop.",
        ),
        Arm(
            "EXIT_rsi75",
            "exit_rsi",
            "Ablate exit toward prior rsi75 freeze: sell at RSI≥75 (still ATR%≥5).",
            exit_rsi=75.0,
        ),
        Arm(
            "EXIT_rsi70",
            "exit_rsi",
            "Ablate to classic OB: sell when RSI≥70 (still ATR%≥5).",
            exit_rsi=70.0,
        ),
        Arm(
            "EXIT_rsi85",
            "exit_rsi",
            "One notch higher exit: sell when RSI≥85 (still ATR%≥5).",
            exit_rsi=85.0,
        ),
        Arm(
            "ENTRY_min_atr4",
            "min_atr_pct",
            "Ablate atr5 downward to prior freeze: require ATR%≥4 at trigger (still exit 80).",
            min_atr_pct=4.0,
        ),
        Arm(
            "ENTRY_min_atr6",
            "min_atr_pct",
            "Nudge atr5 upward: require ATR%≥6 at trigger (still exit 80).",
            min_atr_pct=6.0,
        ),
        Arm(
            "ENTRY_no_atr_gate",
            "min_atr_pct",
            "Ablate atr5 entirely: no ATR% gate (still exit 80).",
            min_atr_pct=0.0,
        ),
        Arm(
            "TIME_stop_20d",
            "time_stop_days",
            "Flatten at 20 calendar days if still not exit-RSI (control exit 80 + atr5).",
            time_stop_days=20,
        ),
        Arm(
            "TIME_stop_40d",
            "time_stop_days",
            "Softer time stop at 40 calendar days.",
            time_stop_days=40,
        ),
    ]
...
def pick_extra_arms(corr_is: pd.DataFrame) -> list[Arm]:
    ...
    Skip ATR% gates (already frozen at 5). Cap extras to avoid combinatorial explode.
    """
    skip = {
...
<body>
<h1>RSI Neutral-after-OB — CONTROL = exit RSI≥80 + min ATR%≥5 — one-knob A/Bs</h1>
...
<p>We already froze “sell at RSI 80” plus “only buy livelier names (ATR%≥4)” as a baseline.
On that book, requiring a bit more liveliness — ATR% <strong>≥5</strong> — looked like a quality lean
(alongside a couple of other one-knob leans). You asked to put min ATR 5 into the freeze itself
and re-run one-change tests around the new combo (exit 80 + ATR%≥5). Ablations include going
back to atr4 (prior freeze), nudging to atr6, removing the ATR gate, exit 75/70/85, and optional
time stops. Later years (2024+) are a check only; we did not retune to make them look better.
Research only — not gold, not wired to DailyRun.</p>
</div>
<div class="insight">
...
<p class="caveat"><strong>Selection honesty:</strong> control = prior lean <code>ENTRY_min_atr5</code>
from <code>{PRIOR_TUNE}</code> (itself built on the rsi80+atr4 combo), chosen after an
in-sample one-knob horse-race. That is in-sample selection. We re-scored FULL/IS/OOS under
the frozen rsi80+atr5 combo; OOS was not used to pick or retune.</p>
</div>
<h2>1. Arms (one knob each vs rsi80+atr5 control)</h2>
...
def write_baseline(path: Path, arms: list[Arm]) -> None:
    lines = [
        ...
        "## In plain English",
        "",
        "Promote the prior lean ENTRY_min_atr5 into the freeze (on top of exit RSI≥80), then re-run",
        "one-change A/Bs around exit RSI≥80 + min ATR%≥5. Judge quality, not trade count.",
        "",
        "## Control freeze",
        "",
        ...
        "",
        f"Control adopts **ENTRY_min_atr5** as a research lean from `{PRIOR_TUNE}` ",
        "(which itself froze EXIT_rsi80 + ENTRY_min_atr4 from an earlier tune stamp). ",
        "ENTRY_min_atr5 was chosen after an in-sample one-knob horse-race (selection bias). ",
        "This stamp **re-scores** FULL/IS/OOS under the frozen rsi80+atr5 combo and runs ",
        "new one-knob arms around it. OOS is report-only — do not retune on OOS. ",
    ...
    lines += [
        ...
        "inspection-only. ATR% arms skipped (already frozen). Cap ≤3 extras. No combinatorial grid.",
        "",
    ]
...
def write_summary(path: Path, arms: list[Arm], books: dict[str, list[dict]]) -> None:
    ...
    lines = [
        ...
        f"(prior lean ENTRY_min_atr5 from `{PRIOR_TUNE}` on top of rsi80+atr4 combo; ",
        "in-sample selection, then re-scored).",
        "",
        "## What you asked",
        "",
        ORIGINAL_REQUEST,
        ...
        "## In plain English",
        "",
        "We put min ATR%≥5 into the freeze with exit RSI≥80 and re-tested one change at a time. ",
        "Quality over count; OOS is a check only.",
        "",
        "## Control headlines",
        "",
...
def enrich_control_closed() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    enrich_script = ROOT / "tools" / "rsi_ob_neutral_rsi80_atr5_enrich_20260910.py"
    import subprocess
    r = subprocess.run(
...
def main() -> int:
    ...
    arms = fixed_arms()
    print(f"Phase 1: {len(arms)} fixed arms (control = rsi80 + atr5)", flush=True)
    by_arm = run_all(arms)
