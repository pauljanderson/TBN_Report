#!/usr/bin/env python3
"""Zone-exits A/B: static target/stop vs nearest-zone target/stop (+ RR gates).

Hypothesis test (docs/HYPOTHESIS_TEST.md): for zone systems, does using nearest
matured zone above/below as target/stop beat production static knobs?

Arms per system:
  control | zone_exits | zone_rr2 | zone_rr3 | zone_rr4

Usage (repo root)::

  python tools/run_zone_exits_ab.py
  python tools/run_zone_exits_ab.py --systems BRT,YH,WPBR --jobs 3
  python tools/run_zone_exits_ab.py --skip-existing
  run_zone_exits_ab.bat

Writes under ``drive/paul_experiments/zone_exits_ab/``.
"""
...
DRIVE = REPO / "drive"
OUT_ROOT = DRIVE / "paul_experiments" / "zone_exits_ab"
...
# Production-parity commons (mirror run_*.bat; do not edit bats here).
...
ARMS: list[Arm] = [
    ...
    Arm("zone_exits", "Nearest zone above=target, below=stop", ("zone_exits=true",)),
    Arm("zone_rr2", "Zone exits + RR>2", ("zone_exits=true", "zone_rr_min=2")),
    Arm("zone_rr3", "Zone exits + RR>3", ("zone_exits=true", "zone_rr_min=3")),
    Arm("zone_rr4", "Zone exits + RR>4", ("zone_exits=true", "zone_rr_min=4")),
]

ARM_ORDER = [a.id for a in ARMS]

# Canonical-ish book fields for the compare table (CANONICAL_COMPARE_METRICS.md core).
COMPARE_COLS: list[tuple[str, str, str]] = [
    # (key, label, fmt)  fmt: money|pct|num|int
...
def recommend_r(rows: list[dict[str, Any]]) -> str:
    """Per-system best RR arm vs control on Ann_ROR then Total_PNL; aggregate vote."""
    by_sys: dict[str, list[dict]] = {}
    for r in rows:
        if not r.get("ok"):
            continue
        by_sys.setdefault(r["system"], []).append(r)
    votes: dict[str, int] = {"zone_exits": 0, "zone_rr2": 0, "zone_rr3": 0, "zone_rr4": 0, "control": 0}
    details: list[str] = []
    for sys, arms in sorted(by_sys.items()):
        ...
        best_ror = c_ror
        best_pnl = c_pnl
        for aid in ("zone_exits", "zone_rr2", "zone_rr3", "zone_rr4"):
            a = by_id.get(aid)
            if not a:
                continue
            m = a.get("metrics") or {}
    ...
    # Prefer an RR that beats control; else zone_exits; else none.
    rr_ranked = sorted(
        (("zone_rr2", 2), ("zone_rr3", 3), ("zone_rr4", 4)),
        key=lambda x: (-votes.get(x[0], 0), x[1]),
    )
    top_rr, r_val = rr_ranked[0]
    if votes.get(top_rr, 0) > 0 and votes.get(top_rr, 0) >= votes.get("control", 0) and votes.get(top_rr, 0) >= votes.get("zone_exits", 0):
        return f"R={r_val} ({top_rr}) - {'; '.join(details)}"
    if votes.get("zone_exits", 0) > votes.get("control", 0):
        return f"zone_exits (no RR gate) - {'; '.join(details)}"
...
def write_html(rows: list[dict[str, Any]], path: Path, recommendation: str) -> None:
    ...
    for system in systems:
        ...
        sections.append(
            ...
            f'<p class="note">Click column headers to sort. Deltas vs control.</p>'
...
<title>Zone exits A/B</title>
<style>
body{{font-family:Segoe UI,system-ui,sans-serif;margin:24px;max-width:1400px;color:#1e293b}}
...
<h1>Zone exits A/B</h1>
<p>Control = production static target%/stop%. Candidates use <code>-v zone_exits=true</code>
and optional <code>-v zone_rr_min</code>. Generated {html.escape(stamp)}.</p>
<p><strong>Recommendation:</strong> {html.escape(recommendation)}</p>
<p class="note">Decision log: <code>drive/paul_experiments/zone_exits_ab/DECISION_LOG.md</code></p>
{"".join(sections)}
{SORTABLE_TABLE_SCRIPT}
</body></html>
"""
...
def update_decision_log(recommendation: str, rows: list[dict[str, Any]]) -> None:
    ...
    for r in sorted(rows, key=lambda x: (x.get("system", ""), ARM_ORDER.index(x["id"]) if x.get("id") in ARM_ORDER else 99)):
        ...
        summary_lines.append(
            f"- `{r.get('system')}` / `{r.get('id')}`: ok={r.get('ok')} trades={int(safe_num(m.get('Total_Trades')))} "
    ...
    # Replace pending answers if present
    repl = {
        "Zone exits beat control on Ann ROR / sheet PnL / PF / DD? | _pending_": f"Zone exits beat control on Ann ROR / sheet PnL / PF / DD? | see recommendation / table ({stamp})",
        "Best R in {2,3,4} or none? | _pending_": f"Best R in {{2,3,4}} or none? | {recommendation}",
        "Adopt / reject / hold | _pending_": "Adopt / reject / hold | hold (research; PO sign-off required)",
        "Phase-2 union worth a follow-up? | _pending_": "Phase-2 union worth a follow-up? | only if phase-1 zone_exits shows edge",
    }
    for a, b in repl.items():
        lines = lines.replace(a, b)
    if "## Run results" in lines:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Zone exits A/B across zone systems")
    ap.add_argument("--systems", default="BRT,YH,WPBR", help="Comma list: BRT,YH,WPBR,MTS")
    ...
    if args.dry_run:
        for s, a in specs:
            print(f"DRY {s} {a.id}: {list(SYSTEM_COMMON[s][:3])}... + {list(a.values)}")
        return 0

    results: list[dict[str, Any]] = []
    jobs = max(1, int(args.jobs))
    print(f"[zone_exits_ab] systems={systems} arms={len(ARMS)} jobs={jobs} symbols={args.symbols}", flush=True)
    ...
    update_decision_log(recommendation, results)
    ...
    readme.write_text(
        "\n".join(
            [
                ...
                "- `runs/<system>__<arm>/`",
                "",
                "Re-run: `python tools/run_zone_exits_ab.py` or `run_zone_exits_ab.bat`",
                "",
            ]
        ),
        encoding="utf-8",
    )
    print(f"[zone_exits_ab] recommendation: {recommendation}", flush=True)
    print(f"[zone_exits_ab] wrote {OUT_ROOT / 'comparison.html'}", flush=True)
