#!/usr/bin/env python3
"""RL too-high fill gate A/B AFTER house adopt 40_30d (tradable 764).

One knob: rl_too_high. Control is the NEW house freeze (cut OFF + +40%/30d).
Must re-run control — prior rl_too_high_ab_20260831 used exit_days=10000.

Control: rl_too_high=0, rl_cut_the_losers=1000, rl_exit_percent=0.40, rl_exit_days=30.
Candidates: rl_too_high in {1.10, 1.12, 1.14, 1.16}.
...
Usage:
  python tools/rl_too_high_ab_after_40_30d.py
  python tools/rl_too_high_ab_after_40_30d.py --summarize-only
  python tools/rl_too_high_ab_after_40_30d.py --skip-existing --jobs 3 --workers 12
"""
...
DRIVE = ROOT / "drive"
STAMP = "20260831"
OUT_DIR = DRIVE / "paul_experiments" / f"rl_too_high_ab_after_40_30d_{STAMP}"
...
def _freeze_v(too_high: float) -> list[str]:
    return [
        f"rl_too_high={too_high}",
...
def _arm_defs(trad: list[str]) -> list[dict[str, Any]]:
    arms: list[dict[str, Any]] = [
        {
            "id": CONTROL_ID,
            "label": "Control (too_high=0, cut off, +40%/30d)",
            "role": "control",
            "symbols": trad,
            "extra_v": _freeze_v(0),
        },
...
def build_cmd(py: str, outdir: Path, workers: int, symbols: str, extra_v: list[str]) -> list[str]:
    ...
    i = 0
    while i < len(cmd):
        if cmd[i] == "-v" and i + 1 < len(cmd) and any(cmd[i + 1].startswith(p) for p in strip):
            i += 2
            continue
        out.append(cmd[i])
        i += 1
...
def write_compare_html(
    ...
    for split_key, title in (("m_is", "IS"), ("m_oos", "OOS (report-only)"), ("m_full", "FULL book")):
        ...
        sections.append(
            ...
            f'<p class="muted">Δ vs control (too_high=0, cut off, +40%/30d). {note} Click column headers to sort.</p>'
...
<title>RL too_high A/B after 40_30d — tradable {STAMP}</title>
<style>
...
<body>
<header>
<h1>RL too_high after house 40_30d — tradable 764</h1>
<p class="muted">Stamp <code>rl_too_high_ab_after_40_30d_{STAMP}</code>. One knob: <code>rl_too_high</code>.
<strong>New control freeze:</strong> cut OFF (1000), <code>rl_exit_percent={HOUSE_EXIT_PCT}</code>,
<code>rl_exit_days={HOUSE_EXIT_DAYS}</code>, too_high=0. All arms re-run (cannot reuse prior exit_days=10000 control).
Fill gate when on: next_open ≤ signal_low × too_high × stop_pct ({HOUSE_STOP}).
Also: dip=1.055, expansion=1.163, target=1.20, trails off, flush=0.
...
def write_docs(packed: list[dict[str, Any]], verdicts: dict[str, dict[str, tuple[str, str]]]) -> None:
    ...
    freeze_lines = [
        f"# BASELINE — `rl_too_high_ab_after_40_30d_{STAMP}`",
        "",
        "**Status:** RESEARCH only. One ENTRY fill-gate knob (`rl_too_high`) vs **new house control**.",
        "Control = adopted 40_30d freeze (`rl_adopt_exit_40_30d_20260831`). Not gold. Not DailyRun.",
        "",
        ...
        "| `rl_target_pct` | **1.20** | SMA50 — stays live |",
        "| trails | **off** | |",
        "| `rl_flush_days` | **0** (off) | |",
        f"| `rl_exit_percent` | **{HOUSE_EXIT_PCT}** | +40% entry MTM |",
        ...
        *cap_lines,
        ...
        "| Reruns | **All arms live** — prior too_high AB had exit_days=10000 |",
    ...
    slines = [
        f"# SUMMARY — `rl_too_high_ab_after_40_30d_{STAMP}`",
        "",
        "RL too_high fill gate A/B on tradable 764 **after** house adopt +40%/30d. Research only.",
        "",
        "## Plain English (Paul)",
        "",
        _best_candidate(packed, verdicts),
        "",
        f"- **Control** = too_high off (0), cut off (1000), exit +{HOUSE_EXIT_PCT:.0%}/{HOUSE_EXIT_DAYS}d.",
        "- **Candidates** = reject fills when next open gaps above signal_low × too_high × stop.",
        "- **One knob only** (`rl_too_high`). Do not retune OOS.",
        "",
        "## IS",
        "",
    ]
    ...
    slines.extend(
        [
            ...
            "## Paths",
            "",
            f"- HTML: `drive/paul_experiments/rl_too_high_ab_after_40_30d_{STAMP}/compare.html`",
            f"- BASELINE: `drive/paul_experiments/rl_too_high_ab_after_40_30d_{STAMP}/BASELINE.md`",
            f"- Adopt freeze: `drive/paul_experiments/rl_adopt_exit_40_30d_{STAMP}/`",
            "",
        ]
    )
    ...
    if ntfy.is_file():
        ...
        subprocess.run(
            [
                ...
                "-t",
                "RL too_high AB after 40/30d",
                "-m",
                " · ".join(bits) + " · ctrl +40%/30d cut OFF",
            ],
            cwd=str(ROOT),
        )
    return 0
