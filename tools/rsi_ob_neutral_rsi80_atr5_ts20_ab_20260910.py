#!/usr/bin/env python3
"""One-knob A/Bs under CONTROL = Neutral-after-OB + exit RSI≥80 + min ATR%≥5 + TIME_stop 20d.

Control preference-adopts TIME_stop_20d (DISMISS on quality under rsi80+atr5 / combo
look) into the freeze — same promotion pattern as EXIT_rsi80 / min atr 5. Research only.

Usage:
  python tools/rsi_ob_neutral_rsi80_atr5_ts20_ab_20260910.py
"""
...
STAMP = "rsi_ob_neutral_rsi80_atr5_ts20_20260910"
...
ORIGINAL_REQUEST = (
    "of those 2 i like time atop 20d. can we add that to control?"
)
...
def fixed_arms() -> list[Arm]:
    """Pre-declared one-knob arms vs rsi80+atr5+ts20 control (no corr pick)."""
    return [
        Arm(
            "CONTROL",
            "none",
            "Frozen combo: Neutral-after-OB buy, min ATR%≥5 at trigger, sell RSI≥80, "
            "time stop 20 calendar days, next open.",
        ),
        Arm(
            "EXIT_rsi75",
            "exit_rsi",
            "Ablate exit toward prior rsi75: sell at RSI≥75 (still ATR%≥5 + ts20).",
            exit_rsi=75.0,
        ),
        Arm(
            "EXIT_rsi70",
            "exit_rsi",
            "Ablate to classic OB: sell when RSI≥70 (still ATR%≥5 + ts20).",
            exit_rsi=70.0,
        ),
        Arm(
            "EXIT_rsi85",
        ...
        Arm(
            "ENTRY_min_atr4",
            "min_atr_pct",
            "Ablate atr5 downward: require ATR%≥4 at trigger (still exit 80 + ts20).",
            min_atr_pct=4.0,
        ),
        Arm(
            "ENTRY_min_atr6",
            "min_atr_pct",
            "Nudge atr5 upward: require ATR%≥6 at trigger (still exit 80 + ts20).",
            min_atr_pct=6.0,
        ),
        Arm(
            "ENTRY_no_atr_gate",
            "min_atr_pct",
            "Ablate atr5 entirely: no ATR% gate (still exit 80 + ts20).",
            min_atr_pct=0.0,
        ),
        Arm(
            "TIME_stop_none",
            "time_stop_days",
            "Ablate the new freeze: no time stop (back to prior atr5 control exit 80 + atr5).",
            time_stop_days=0,
        ),
        Arm(
            "TIME_stop_40d",
            "time_stop_days",
            "Softer time stop at 40 calendar days (still exit 80 + atr5).",
            time_stop_days=40,
        ),
    ]
        ...
        return intro + "".join(
            [
                book_table_for_slice(
                    "IS",
                    "IS (entry &lt; 2024-01-01)",
                    "In-sample book under the rsi80+atr5+ts20 control.",
                ),
                book_table_for_slice(
                    "OOS",
...
<body>
<h1>RSI Neutral-after-OB — CONTROL = exit RSI≥80 + min ATR%≥5 + TIME_stop 20d — one-knob A/Bs</h1>
...
hot (RSI was ≥70) into the middle band (30&lt;RSI&lt;70), filled next open. We already froze
“sell at RSI 80” plus “only buy livelier names” via Average True Range percent (ATR%) ≥5.
On that book, a 20-day time stop (flatten if still not exit-RSI after 20 calendar days) was
<strong>DISMISS</strong> on quality — and a combo look with EXIT_rsi70 also dismissed both.
You still preferred the 20-day time stop and asked to put it into the freeze itself
(same preference-adoption pattern as when we added exit 80 / min ATR 5). This stamp
re-scores under exit 80 + ATR%≥5 + time stop 20d and runs one-change tests around it
(ablate time stop back to none, softer 40d, exit 75/70/85, atr4/6/none). Later years
(2024+) are a check only; we did not retune. Research only — not gold, not wired to DailyRun.
Do <strong>not</strong> read time-stop-in-control as a KEEP from metrics.</p>
</div>
<div class="insight">
...
<p class="caveat"><strong>Selection honesty:</strong> <code>TIME_stop_20d</code> was
<strong>DISMISS</strong> on quality vs <code>{PRIOR_TUNE}</code> (and again under
<code>{COMBO_LOOK}</code>). Paul chose it by preference after the combo look — preference
adoption into the freeze, <em>not</em> a KEEP from metrics. We re-scored FULL/IS/OOS under
the frozen rsi80+atr5+ts20 combo; OOS was not used to pick or retune. Research only ·
not gold · not DailyRun.</p>
</div>
<h2>1. Arms (one knob each vs rsi80+atr5+ts20 control)</h2>
...
<code>RSIN_Closed_20260910.csv</code>. Post-run HTML in the same stamp folder.</p>
{SORT_JS}
</body>
</html>
"""
...
def write_baseline(path: Path, arms: list[Arm]) -> None:
    lines = [
        ...
        "## In plain English",
        "",
        "Relative Strength Index (RSI) Neutral-after-Overbought (OB) + exit RSI≥80 +",
        "Average True Range percent (ATR%) ≥5 was the prior control. A 20-day time stop",
        "(flatten if still not exit-RSI) was DISMISS on quality under that book / combo look,",
        "but Paul preferred it and asked to freeze it into control — preference adoption,",
        "not a KEEP from metrics. Re-score one-change A/Bs around exit 80 + ATR%≥5 + ts20.",
        "",
        "## Control freeze",
        "",
        ...
        "",
        f"Control preference-adopts **TIME_stop_20d** into the freeze on top of `{PRIOR_TUNE}` ",
        f"(exit RSI≥80 + min ATR%≥5, no time stop). Under that prior stamp and under ",
        f"`{COMBO_LOOK}`, TIME_stop_20d was **DISMISS** on quality. Paul chose it by preference ",
        "after the combo look — same promotion pattern as EXIT_rsi80 / min atr 5 preference ",
        "adoptions, **not** a KEEP from metrics. This stamp **re-scores** FULL/IS/OOS under ",
        "the frozen rsi80+atr5+ts20 combo and runs new one-knob arms around it. ",
    ...
    lines += [
        ...
        "control Closed only** (`RSIN_Correlation_IS_20260910.csv`). Full-sample corr is ",
...
def write_summary(path: Path, arms: list[Arm], books: dict[str, list[dict]]) -> None:
    ...
    lines = [
        f"# SUMMARY — {STAMP}",
        "",
        "Research only. Not gold. Not DailyRun. Control = exit RSI≥80 + min ATR%≥5 + ",
        f"TIME_stop 20d (preference-adopted DISMISS from `{PRIOR_TUNE}` / `{COMBO_LOOK}`; ",
        "re-scored under freeze — not a KEEP from metrics).",
        "",
        "## What you asked",
        "",
        ORIGINAL_REQUEST,
        ...
        "## In plain English",
        "",
        "Relative Strength Index (RSI) Neutral-after-OB with sell at RSI 80, Average True Range ",
        "percent (ATR%) ≥5, plus a 20-day calendar time stop now frozen by preference. ",
        "One change at a time around that freeze. Quality over count; OOS is a check only.",
        "",
        "## Control headlines",
        "",
...
def enrich_control_closed() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    enrich_script = ROOT / "tools" / "rsi_ob_neutral_rsi80_atr5_ts20_enrich_20260910.py"
    import subprocess
    r = subprocess.run(
    ...
    corr_full = load_corr(OUT_DIR / "RSIN_Correlation_20260910.csv")
    corr_is = load_corr(OUT_DIR / "RSIN_Correlation_IS_20260910.csv")
...
def main() -> int:
    ...
    arms = fixed_arms()
    print(f"Phase 1: {len(arms)} fixed arms (control = rsi80 + atr5 + ts20)", flush=True)
    by_arm = run_all(arms)
