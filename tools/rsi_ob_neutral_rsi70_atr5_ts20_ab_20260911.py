#!/usr/bin/env python3
"""One-knob A/Bs under CONTROL = Neutral-after-OB + exit RSI≥70 + min ATR%≥5 + TIME_stop 20d.

Control preference-adopts COMBO_EXIT_rsi70_TIME_20d (DISMISS on quality under combo look
/ atr5 parent) into the freeze — same pattern as rsi80+atr5+ts20 preference adopt.
Research only.

Usage:
  python tools/rsi_ob_neutral_rsi70_atr5_ts20_ab_20260911.py
"""
...
STAMP = "rsi_ob_neutral_rsi70_atr5_ts20_20260911"
OUT_DIR = ROOT / "drive" / "paul_experiments" / STAMP
PRIOR_TUNE = "rsi_ob_neutral_rsi80_atr5_20260910"
COMBO_LOOK = "rsi_ob_neutral_rsi80_atr5_combo_rsi70_ts20_20260910"
TS20_PREF = "rsi_ob_neutral_rsi80_atr5_ts20_20260910"
...
ORIGINAL_REQUEST = (
    "can you adopt this? COMBO_EXIT_rsi70_TIME_20d then run another test and "
    "post run analysis and run some ab tests and report back please."
)
...
def fixed_arms() -> list[Arm]:
    """Pre-declared one-knob arms vs rsi70+atr5+ts20 control (no corr pick)."""
    return [
        Arm(
            "CONTROL",
            "none",
            "Frozen combo: Neutral-after-OB buy, min ATR%≥5 at trigger, sell RSI≥70, "
            "time stop 20 calendar days, next open.",
        ),
        Arm(
            "EXIT_rsi75",
            "exit_rsi",
            "One notch higher exit: sell at RSI≥75 (still ATR%≥5 + ts20).",
            exit_rsi=75.0,
        ),
        Arm(
            "EXIT_rsi80",
            "exit_rsi",
            "Ablate toward prior atr5/ts20 control: sell when RSI≥80 (still ATR%≥5 + ts20).",
            exit_rsi=80.0,
        ),
        Arm(
            "EXIT_rsi85",
            "exit_rsi",
            "One notch higher exit: sell when RSI≥85 (still ATR%≥5 + ts20).",
            exit_rsi=85.0,
        ),
        Arm(
            "ENTRY_min_atr4",
            "min_atr_pct",
            "Ablate atr5 downward: require ATR%≥4 at trigger (still exit 70 + ts20).",
            min_atr_pct=4.0,
        ),
        Arm(
            "ENTRY_min_atr6",
            "min_atr_pct",
            "Nudge atr5 upward: require ATR%≥6 at trigger (still exit 70 + ts20).",
            min_atr_pct=6.0,
        ),
        Arm(
            "ENTRY_no_atr_gate",
            "min_atr_pct",
            "Ablate atr5 entirely: no ATR% gate (still exit 70 + ts20).",
            min_atr_pct=0.0,
        ),
        Arm(
            "TIME_stop_none",
            "time_stop_days",
            "Ablate ts20: no time stop (still exit 70 + atr5).",
            time_stop_days=0,
        ),
        Arm(
            "TIME_stop_40d",
            "time_stop_days",
            "Softer time stop at 40 calendar days (still exit 70 + atr5).",
            time_stop_days=40,
        ),
    ]
...
def pick_extra_arms(corr_is: pd.DataFrame) -> list[Arm]:
    ...
    Skip ATR% gates (already frozen at 5) and time-stop / days-held variables.
    Cap extras to avoid combinatorial explode.
    """
    skip = {
    ...
    for var, rp in useful:
        ...
        if "DIST_TO_52W" in vu and rp < 0:
            ...
            extra.append(Arm(
                "ENTRY_max_rsi_60", "max_rsi_trigger",
                f"IS corr (new control) {var} R_PNL={rp:+.3f}: deeper cool-off (trigger RSI < 60).",
                max_rsi_trigger=60.0,
            ))
            seen.add(vu)
        if len(extra) >= 3:
            break
        ...
        return intro + "".join(
            [
                book_table_for_slice(
                    "IS",
                    "IS (entry &lt; 2024-01-01)",
                    "In-sample book under the rsi70+atr5+ts20 control.",
                ),
                book_table_for_slice(
                    "OOS",
...
<body>
<h1>RSI Neutral-after-OB — CONTROL = exit RSI≥70 + min ATR%≥5 + TIME_stop 20d — one-knob A/Bs</h1>
...
hot (RSI was ≥70) into the middle band (30&lt;RSI&lt;70), filled next open. We already had a
preference-adopted freeze of exit RSI≥80 + Average True Range percent (ATR%) ≥5 + 20-day
time stop (<code>{TS20_PREF}</code>). Separately, under the combo look
<code>{COMBO_LOOK}</code>, the multi-knob arm <code>COMBO_EXIT_rsi70_TIME_20d</code>
(sell at RSI≥70 plus the same 20-day flatten) was <strong>DISMISS</strong> on quality —
and the singles that built it were also dismissed under the atr5 parent. You still asked to
freeze that combo into control. This stamp preference-adopts exit 70 + ATR%≥5 + time stop
20d, re-scores FULL/IS/OOS under that freeze, and runs one-change tests around it
(ablate time stop, softer 40d, exit 75/80/85, atr4/6/none). Later years (2024+) are a check
only; we did not retune. Research only — not gold, not wired to DailyRun.
Do <strong>not</strong> read combo-in-control as a KEEP from metrics.</p>
</div>
<div class="insight">
...
<p class="caveat"><strong>Selection honesty:</strong> <code>COMBO_EXIT_rsi70_TIME_20d</code>
was a <strong>multi-knob DISMISS</strong> on quality under <code>{COMBO_LOOK}</code>
(parent atr5: <code>{PRIOR_TUNE}</code>; immediate prior preference freeze:
<code>{TS20_PREF}</code>). Both singles and the combo were DISMISS. Paul chose the combo
by preference into the freeze — preference adoption, <em>not</em> a KEEP from metrics.
We re-scored FULL/IS/OOS under the frozen rsi70+atr5+ts20 combo; OOS was not used to pick
or retune. Research only · not gold · not DailyRun.</p>
</div>
<h2>1. Arms (one knob each vs rsi70+atr5+ts20 control)</h2>
...
<code>RSIN_Closed_20260911.csv</code>. Post-run HTML in the same stamp folder.</p>
{SORT_JS}
</body>
</html>
"""
...
def write_baseline(path: Path, arms: list[Arm]) -> None:
    lines = [
        ...
        "## In plain English",
        "",
        "Relative Strength Index (RSI) Neutral-after-Overbought (OB) + exit RSI≥70 +",
        "Average True Range percent (ATR%) ≥5 + 20-day time stop. That combo arm was",
        "DISMISS on quality under the combo look / atr5 parent, but Paul preferred it and",
        "asked to freeze it into control — multi-knob preference adoption, not a KEEP from",
        "metrics. Re-score one-change A/Bs around exit 70 + ATR%≥5 + ts20.",
        "",
        "## Control freeze",
        "",
        ...
        "",
        f"Control preference-adopts **COMBO_EXIT_rsi70_TIME_20d** into the freeze ",
        f"(exit RSI≥70 + min ATR%≥5 + TIME_stop 20d). Under `{COMBO_LOOK}` that multi-knob ",
        f"arm was **DISMISS** on quality; singles were also DISMISS under `{PRIOR_TUNE}` / ",
        f"combo. Immediate prior preference freeze was `{TS20_PREF}` (exit 80 + atr5 + ts20). ",
        "Paul chose the combo by preference — same promotion pattern as prior preference ",
        "adoptions, **not** a KEEP from metrics. This stamp **re-scores** FULL/IS/OOS under ",
        "the frozen rsi70+atr5+ts20 combo and runs new one-knob arms around it. ",
    ...
    lines += [
        ...
        "inspection-only. ATR% and time-stop / days-held variables skipped (already frozen / ",
        "confusing). Cap ≤3 extras. No combinatorial grid.",
        "",
    ]
...
def write_summary(path: Path, arms: list[Arm], books: dict[str, list[dict]]) -> None:
    ...
    lines = [
        f"# SUMMARY — {STAMP}",
        "",
        "Research only. Not gold. Not DailyRun. Control = exit RSI≥70 + min ATR%≥5 + ",
        f"TIME_stop 20d (preference-adopted multi-knob DISMISS from `{COMBO_LOOK}` / ",
        f"`{PRIOR_TUNE}`; prior pref freeze `{TS20_PREF}`; re-scored under freeze — not a ",
        "KEEP from metrics).",
        "",
        "## What you asked",
        "",
        ...
        "## In plain English",
        "",
        "Relative Strength Index (RSI) Neutral-after-OB with sell at RSI 70, Average True Range ",
        "percent (ATR%) ≥5, plus a 20-day calendar time stop now frozen by preference despite ",
        "DISMISS on the combo look. One change at a time around that freeze. Quality over count; ",
        "OOS is a check only.",
        "",
        "## Control headlines",
        "",
    ...
    for a in arms:
        ...
        if "KEEP" in v:
            ...
            holds.append(a.name)
    # Soften KEEP → HOLD if OOS softens materially vs control OOS
    soft: list[str] = []
...
def enrich_control_closed() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    enrich_script = ROOT / "tools" / "rsi_ob_neutral_rsi70_atr5_ts20_enrich_20260911.py"
    import subprocess
    r = subprocess.run(
...
def main() -> int:
    ...
    arms = fixed_arms()
    print(f"Phase 1: {len(arms)} fixed arms (control = rsi70 + atr5 + ts20)", flush=True)
    by_arm = run_all(arms)
    ...
    extra_note = (
        ...
        f"(ATR + time-stop vars skipped; cap 3): "
