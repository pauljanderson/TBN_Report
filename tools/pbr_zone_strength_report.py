#!/usr/bin/env python3
"""PBR zone strength report: zone metrics + trade outcome correlation."""
...
from pbr_zones import PBR_STRENGTH_FIELDS, compute_pbr_touch_stream
...
def _collect_zone_rows(symbols: list[str], data_dir: Path) -> pd.DataFrame:
    ...
    for sym in symbols:
        ...
        out = filter_pbr_output_for_compare(
            compute_pbr_touch_stream(df, **PBR_PARAMS),
            df,
        ...
        for ev in out.get("pbr_zone_events") or []:
            row = {
                "symbol": sym,
                "pbr_zone_id": ev.get("pbr_zone_id", ""),
...
def main() -> int:
    ap = argparse.ArgumentParser(description="PBR zone strength report")
    ...
    ap.add_argument("--out-dir", default=str(REPO / "output" / "pbr_strength_report"))
    args = ap.parse_args()
    ...
    _print_section("PBR ZONE STRENGTH — ZONE UNIVERSE")
    ...
    for label, mask in [
        ...
        sub = zones.loc[mask]
        print(f"\n{label} ({len(sub)})")
        for field in ("pbr_zone_strength", "pbr_pre_rise_pct", "pbr_poc_dist_pct", "pbr_conf_overshoot_pct", "pbr_retest_depth_pct"):
    ...
    if args.skip_backtest:
        out_csv = Path(args.out_dir) / "pbr_zone_strength_zones.csv"
    ...
    if closed_path is None or not closed_path.exists():
        print("Backtest failed or no PBR_Closed CSV produced.", file=sys.stderr)
        return 2
    ...
    if "pbr_zone_strength" in trades.columns and trades["pbr_zone_strength"].notna().any():
        trades = trades.dropna(subset=["pbr_zone_strength"])
        trades["strength_quartile"] = pd.qcut(trades["pbr_zone_strength"], 4, duplicates="drop")
        ...
        show_cols = ["SYMBOL", "DATE_OPENED", "PNL_PCT", "pbr_zone_strength", "pbr_conf_overshoot_pct", "pbr_retest_depth_pct"]
        show_cols = [c for c in show_cols if c in trades.columns]
        top = trades.nlargest(5, "pbr_zone_strength")[show_cols]
        bot = trades.nsmallest(5, "pbr_zone_strength")[show_cols]
        print("\nHighest strength:")
        print(top.to_string(index=False))
        print("\nLowest strength:")
        print(bot.to_string(index=False))

    out_zones = Path(args.out_dir) / "pbr_zone_strength_zones.csv"
    out_trades = Path(args.out_dir) / "pbr_zone_strength_trades.csv"
